...
PAGE = ROOT / "bitrix/templates/aspro_max/design-model/pages/uslugi-podderzhka-dorabotka.html"
...
class DorabotkaPageTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_manifest_registers_child_element(self):
        entry = next(page for page in self.manifest["pages"] if page["code"] == "dorabotka")
        self.assertEqual(entry["kind"], "element")
        self.assertEqual(entry["section"], "podderzhka")
        self.assertEqual(entry["html"], PAGE.name)
        self.assertIn("доработ", entry["meta_description"].lower())
    ...
    def test_uses_design_model_contract(self):
        ...
        self.assertIn("<h1>Доработка и развитие сайта под ключ</h1>", self.html)
    ...
    def test_url_contract_and_existing_links(self):
        self.assertTrue(
            '"detail" => "#SECTION_CODE#/#ELEMENT_CODE#/"' in self.sef
            or '"detail" => "#SECTION_CODE_PATH#/#ELEMENT_CODE#/"' in self.sef
        )
        self.assertIn('href="/services/podderzhka/dorabotka/"', self.parent)
        self.assertNotIn("/services/podderzhka/dorabotka//", self.parent)
        ...
        self.assertIn('href="/services/podderzhka/kontentnaya/"', self.html)
