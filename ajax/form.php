...
<?elseif($form_id === 'TABLES_SIZE'):?>
	...
	switch ($blockType) {
		case 'shoes':
			$baseFile = 'detail_shoes.php';
			break;
		default:
			$baseFile = 'detail_clothes.php';
			break;
	}

	if ($baseFile) {
		$baseDir = $_SERVER['DOCUMENT_ROOT'].SITE_DIR.'include/table_sizes/';
		$basePath = $baseDir.$baseFile;

		$brandCode = $request['brand_code'] ?? '';
		if ($brandCode && preg_match('/^[a-zA-Z0-9_-]+$/', $brandCode)) {
			$brandPath = $baseDir.str_replace('.php', '_'.$brandCode.'.php', $baseFile);
			if (file_exists($brandPath)) {
				$includeFile = $brandPath;
			}
		}

		if (!$includeFile && file_exists($basePath)) {
	...
	if($formType == 'CRM') {
		...
		if(!$bitrix24):?>
			<div class="form">
				<a href="#" class="close jqmClose"><?=CMax::showIconSvg('', SITE_TEMPLATE_PATH.'/images/svg/Close.svg')?></a>
				<div class="form_head">
					<h2><?=htmlspecialcharsbx($formName)?></h2>
				</div>
				<div class="form_body">
