...
$dbRes = CRubric::GetList(['ID' => 'ASC'], ['LID' => SITE_ID, 'ACTIVE' => 'Y']);
$arRubricsID = [];
...
$arFields = ['EMAIL' => $email, 'RUB_ID' => $arRubricsID, 'SITE_ID' => SITE_ID];

$res = $arSubscription
    ? $subscr->Update($arSubscription['ID'], $arFields)
    : $subscr->Add($arFields);

if (!$res && $exeptionObject = $APPLICATION->GetException()) {
    echo '<font class="errortext">' . $exeptionObject->GetString() . '</font>';
    die();
}
