...
<?if ($arMainPageOrder && is_array($arMainPageOrder)):?>
    ...
    <?foreach ($arMainPageOrder as $key => $optionCode):?>
        ...
        <?// Services?>
        <?if ($optionCode === 'SERVICES'):?>
            <?global $bShowServices, $bServicesIndexClass;?>
            <?if ($bShowServices):?>
                <div class="drag-block container <?=$optionCode;?> <?=$bServicesIndexClass;?>" data-class="<?=$subtype;?>_drag" data-order="<?=++$key;?>" style="--order-index: <?=$key;?>" data-file="<?=SITE_DIR;?>include/mainpage/components/<?=$subtype;?>/<?=$strTemplateName;?>.php">
                    <?=CMax::ShowPageType(
                        type: 'mainpage',
                        subtype: $subtype,
                        template: $strTemplateName,
