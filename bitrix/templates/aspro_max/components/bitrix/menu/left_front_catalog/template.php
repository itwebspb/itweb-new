		...
		<ul class="menu dropdown">
			<?foreach($arResult as $key => $arItem){?>
                ...
                if(!empty($arItem["CHILD"])){
                    $arItemClasses[] = 'has-child';
                }
                if(!empty($arItem["SELECTED"])){
                     $arItemClasses[] = 'current opened';
                }
				...
				?>
				<li class="full <?=TSolution\Utils::implodeClasses($arItemClasses);?> <?=$classMenuPosition?> v_<?=strtolower(CMax::GetFrontParametrValue("MENU_TYPE_VIEW"));?>">
					<a class="icons_fa<?=($arItem["CHILD"] ? " parent" : "");?> rounded2 bordered" href="<?=$arItem["SECTION_PAGE_URL"]?>" data-key='<?=$key?>'>
						<?if($arItem["CHILD"]):?>
					...
					<?if($arItem["CHILD"]):?>
						<ul class="dropdown scrollblock overscroll-behavior-contain">
							<?foreach($arItem["CHILD"] as $arChildItem){?>
								<?
                                $arChildItemClasses = [];

                                if(!empty($arChildItem["CHILD"])){
                                    $arChildItemClasses[] = 'has-child';
                                }
                                if(!empty($arChildItem["SELECTED"])){
                                    $arChildItemClasses[] = 'current opened';
                                }
								...
								?>
								<li class="<?=TSolution\Utils::implodeClasses($arChildItemClasses)?>">
									...
									<?if($arChildItem["CHILD"]):?>
										<ul class="dropdown">
											<?foreach($arChildItem["CHILD"] as $arChildItem1){?>
                                                ...
                                                if(!empty($arChildItem1["SELECTED"])){
                                                    $arChildItem1Classes[] = 'current';
                                                }
                                                ?>
												<li class="menu_item <?=TSolution\Utils::implodeClasses($arChildItem1Classes)?>">
