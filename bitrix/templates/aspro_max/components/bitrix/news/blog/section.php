...
if (TSolution\Author::isNeedShowByExistedInRequest()) {
    $author = new TSolution\Author();
    $author->setId(TSolution\Author::getIdFromRequest());
    $author->setTable($author->getHighloadTableNameByIblockId($arParams['IBLOCK_ID']));

    $arItemFilter += $author->makeSectionFilter();
    $GLOBALS[$arParams["FILTER_NAME"]][] = $author->makeSectionFilter();
}
?>
				...
				<div class="select_head_wrap">
					...
					<div class="head-block top bordered-block rounded3 clearfix srollbar-custom">
						<div class="item-link font_upper_md  <?=($bHasYear ? '' : 'active');?>">
							<div class="title">
								<?if($bHasYear):?>
									<a class="btn-inline dark_link" href="<?=$APPLICATION->GetCurPageParam("", ["year"]);?>"><?=GetMessage('ALL_TIME');?></a>
								<?else:?>
			...
			<?if (count($arAllSections) > 1):?>
				...
				<div class="categories_block menu_top_block">
					...
					<ul class="categories left_menu dropdown">
                        <li><a href="<?=(TSolution\Author::isNeedShowByExistedInRequest() ? $author->getFilterUrl($arParams['SEF_FOLDER']) : $arParams['SEF_FOLDER']);?>" class="categories_link bordered rounded2"><span class="categories_name darken"><?=GetMessage("ALL_SECTION")?></span></a></li>
						<?foreach($arAllSections as $arSection):
							if(isset($arSection['TEXT']) && $arSection['TEXT']):?>
								<li class="categories_item  item v_bottom <?=($arSection['CHILD'] ? 'has-child' : '')?> <?=($arSection['CURRENT'] ? 'current opened' : '');?>">
									<a href="<?=(TSolution\Author::isNeedShowByExistedInRequest() ? $author->getFilterUrl($arSection['LINK']) : $arSection['LINK']);?>" class="categories_link bordered rounded2">
												...
												<ul class="child">
													<?foreach ($arSection['CHILD'] as $arChild):?>
														<li class="menu_item hover_color_theme ">
															<a href="<?=(TSolution\Author::isNeedShowByExistedInRequest() ? $author->getFilterUrl($arChild['LINK']) : $arChild['LINK']);?>">
