				...
				<div class="items-inner">
					<?foreach($arResult['MAP_ITEMS'] as $arItem):?>
						<div class="item" data-coordinates="<?=$arItem['GPS_N'].','.$arItem['GPS_S'];?>" data-id="<?=$arItem['ID']?>">
							<div class="title option-font-bold font_sm"><?=TSolution\Utils::sanitizeHtml(htmlspecialcharsBack($arItem['NAME']));?></div>
							<?if($arItem['PHONE']):?>
								<div class="phones">
									<?if(is_array($arItem['PHONE'])):?>
										<?foreach($arItem['PHONE'] as $value):?>
											<div class="value"><a class="muted font_xs" rel= "nofollow" href="tel:<?=preg_replace('/[^+\d]+/', '', $value)?>"><?=TSolution\Utils::sanitizeHtml(htmlspecialcharsBack($value));?></a></div>
										<?endforeach;?>
									<?else:?>
										<div class="value"><a class="muted font_xs" rel= "nofollow" href="tel:<?=preg_replace('/[^+\d]+/', '', $arItem['PHONE']);?>"><?=TSolution\Utils::sanitizeHtml(htmlspecialcharsBack($arItem['PHONE']));?></a></div>
									<?endif;?>
								</div>
							<?endif;?>
						</div>
