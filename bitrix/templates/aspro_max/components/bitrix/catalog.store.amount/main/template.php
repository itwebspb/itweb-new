...
<?if(count($arResult['STORES']) > 0):?>
    ...
    $dbRes = CIBlock::GetList([], ['CODE' => 'aspro_max_shops', 'ACTIVE' => 'Y', 'SITE_ID' => SITE_ID]);
    if ($arShospIblock = $dbRes->Fetch()) {
        $dbRes = CIBlockElement::GetList([], ['ACTIVE' => 'Y', 'IBLOCK_ID' => $arShospIblock['ID']], false, false, ['ID', 'DETAIL_PAGE_URL', 'PROPERTY_STORE_ID']);
    ...
    $count_stores = count($arResult['STORES']); ?>
        ...
        <?foreach($arResult['STORES'] as $pid => $arProperty):
            ...
            <?if(!$bMapMode):?>
                <div class="stores_block bordered rounded3 <?= isset($arProperty['IMAGE_ID']) && !empty($arProperty['IMAGE_ID']) ? 'w_image' : 'wo_image'; ?>" <?= $arParams['SHOW_EMPTY_STORE'] == 'N' && $amount <= 0 ? 'style="display: none"' : ''; ?>>
                    <div class="stores_text_wrapp <?= isset($arProperty['IMAGE_ID']) && !empty($arProperty['IMAGE_ID']) ? 'image_block' : ''; ?>">
                        ...
                        <div class="main_info ">
                            <?if (isset($arProperty['TITLE'])):?>
                                <span>
                                    <a class="title_stores font_sm dark_link option-font-bold" href="<?=TSolution\Utils::sanitizeValue($arProperty['URL']); ?>" data-storehref="<?=TSolution\Utils::sanitizeValue($arProperty['URL']); ?>" data-iblockhref="<?=$arShops[$arProperty['ID']]['DETAIL_PAGE_URL']; ?>"> <?=TSolution\Utils::sanitizeHtml($storeName); ?></a>
                                </span>
                            <?endif; ?>
                            <?if($arParams['FIELDS'] && in_array('PHONE', $arParams['FIELDS']) && isset($arProperty['PHONE']) && $arProperty['PHONE']):?>
                                <div class="store_phone p10 muted777 font_xs">
                                    <?=GetMessage('S_PHONE'); ?> <?=TSolution\Utils::sanitizeHtml(htmlspecialcharsBack($arProperty['PHONE'])); ?>
                                </div>
                            <?endif; ?>
                            <?if(isset($arProperty['SCHEDULE']) && $arProperty['SCHEDULE']):?>
                                <div class="schedule p10 muted777 font_xs">
                                    <?=GetMessage('S_SCHEDULE'); ?>&nbsp;<?=nl2br(TSolution\Utils::sanitizeHtml(str_replace('&lt;br/&gt;', "\n", $arProperty['SCHEDULE']))); ?>
                                </div>
                            <?endif; ?>
                            <?if(isset($arProperty['EMAIL']) && $arProperty['EMAIL']):?>
                                <div class="email p10 muted777 font_xs">
                                    <?=GetMessage('S_EMAIL'); ?>&nbsp;<a href="<?=TSolution\Utils::sanitizeValue('mailto:'.$arProperty['EMAIL']); ?>"><?=TSolution\Utils::sanitizeHtml(htmlspecialcharsBack($arProperty['EMAIL'])); ?></a>
                                </div>
                            <?endif; ?>
                            <?if (!empty($arProperty['USER_FIELDS']) && is_array($arProperty['USER_FIELDS'])) {
                                foreach ($arProperty['USER_FIELDS'] as $userField) {
                                    if (isset($userField['CONTENT'])) {
                                        ?><span class="muted777 font_xs"><?=TSolution\Utils::sanitizeValue(htmlspecialcharsBack($userField['TITLE'])); ?> : <?=TSolution\Utils::sanitizeHtml(htmlspecialcharsBack($userField['CONTENT'])); ?></span><br /><?php
                                    }
                                }
                            }?>
                                ...
                                <div class="items-inner">
                                    <?foreach($arTmpItems as $arItem):?>
                                        ...
                                        <div class="item" data-coordinates="<?=$pos; ?>" data-id="<?=$arItem['ID']; ?>">
                                            <?// print_r($arItem);?>
                                            <div class="title option-font-bold font_sm"><?=TSolution\Utils::sanitizeHtml($arItem['ADDRESS']); ?></div>
                                            <?if($arItem['QUANTITY']):?>
                                                <?=$arItem['QUANTITY']; ?>
                                            <?endif; ?>
                                        </div>
