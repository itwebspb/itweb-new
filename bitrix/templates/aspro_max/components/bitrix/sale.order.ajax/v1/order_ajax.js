      ...
      (BX.Sale.OrderAjaxComponent = {
        ...
        init: function (parameters) {
          this.initializePrimaryFields(),
            ...
            (this.result.IS_AUTHORIZED && void 0 === this.result.LAST_ORDER_DATA.FAIL) || this.initFirstSection(),
            this.initOptions(),
            this.editOrder(),
            this.bindEvents(),
            ...
            "saveOrderAjax" === (action = BX.type.isNotEmptyString(action) ? action : "refreshOrderAjax")
              ...
              : BX.ajax({
                  ...
                  onsuccess: BX.delegate(function (result) {
                    switch (
                      (result.redirect && result.redirect.length && (document.location.href = result.redirect),
                      this.saveFiles(),
                      action)
                    ) {
        ...
        refreshOrder: function (result) {
          if (result.error)
            ...
            var animation =
              (result.order.OK_MESSAGE && result.order.OK_MESSAGE.length) || "OK" === result.order.SMS_AUTH.TYPE
                ? "bx-step-good"
                : "bx-step-bad";
        ...
        saveOrderWithJson: function (result) {
          ...
          result &&
            ...
            ((result = result.order).REDIRECT_URL
              ? ("Y" === this.params.USE_ENHANCED_ECOMMERCE && this.setAnalyticsDataLayer("purchase", result.ID),
                (redirected = !0),
        ...
        checkNotifications: function () {
          ...
          informer &&
            (this.firstLoad && this.result.IS_AUTHORIZED && void 0 === this.result.LAST_ORDER_DATA.FAIL
              ? ((className = (success =
                  (sections = this.orderBlockNode.querySelectorAll(".bx-soa-section.bx-active")).length &&
                  "true" == sections[sections.length - 1].getAttribute("data-visited"))
                  ? "success"
                  : "warning"),
        ...
        checkBlockErrors: function (node) {
          ...
          if (
            ...
            ((showError =
              (errorNode = hiddenNode.querySelector("div.alert.alert-danger")) && "none" != errorNode.style.display),
            (showWarning = hiddenNode.querySelector("div.alert.alert-warning.alert-show")),
            !showError)
          )
            for (errorTooltips = hiddenNode.querySelectorAll("div.tooltip"), i = 0; i < errorTooltips.length; i++)
        ...
        showWarnings: function () {
          ...
          for (k = 0; k < sections.length; k++)
            BX.removeClass(sections[k], "bx-step-warning"),
              "false" == sections[k].getAttribute("data-visited") && BX.removeClass(sections[k], "bx-step-completed");
          if (
                ...
                switch (k.toUpperCase()) {
                  case "DELIVERY":
                    "true" === this.deliveryBlockNode.getAttribute("data-visited") &&
                      ...
                      this.showBlockWarning(this.deliveryHiddenBlockNode, this.result.WARNING[k], !0));
                    break;
                  case "PAY_SYSTEM":
                    "true" === this.paySystemBlockNode.getAttribute("data-visited") &&
                      ...
                      this.showBlockWarning(this.paySystemHiddenBlockNode, this.result.WARNING[k], !0));
                }
        },
        ...
        showBlockWarning: function (node, warnings, hide) {
          ...
          if (errorNode) {
            ...
            else for (i in warnings) warnings.hasOwnProperty(i) && warnings[i] && (warnStr += warnings[i] + "<br>");
            if (!warnStr) return;
        ...
        isPriceChanged: function (result) {
          var priceBefore =
              null === this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY || "" === this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY
        ...
        initPagination: function () {
          ...
          if (this.result.DELIVERY)
            if (
              ...
              this.options.deliveriesPerPage > 0 && this.result.DELIVERY.length > this.options.deliveriesPerPage)
            ) {
              for (
          ...
          if (this.result.PAY_SYSTEM)
            if (this.options.paySystemsPerPage > 0 && this.result.PAY_SYSTEM.length > this.options.paySystemsPerPage) {
              ...
              for (this.paySystemPagination.pages = arPages, i = 0; i < this.result.PAY_SYSTEM.length; i++)
        ...
        clickNextAction: function (event) {
          ...
          return (
            ...
            (this.result.IS_AUTHORIZED && void 0 === this.result.LAST_ORDER_DATA.FAIL) ||
              ...
              section.next.id == allSections[allSections.length - 1].id && this.switchOrderSaveButtons(!0)),
            ...
            if (allSections[i].id === actionSection.id && allSections[i + 1])
              return (
                ...
                this.shouldSkipSection(nextSection)
                  ? (this.markSectionAsCompleted(nextSection), this.getNextSection(nextSection, nextSection))
            ...
            if (
              ...
              ((nextSectionHidden = BX(nextSection.id + "-hidden")).style.left = "-10000"),
                ...
                (offset = window.innerHeight - orderPos.height - diff) > 0
                  ...
                  : (scrollTo =
                      nodePos.top > windowScrollTop ? nodePos.top : nodePos.bottom + 6 - objHeightOrig + objHeight) +
                      window.innerHeight >
                      orderPos.bottom + 25 + diff && (scrollTo = orderPos.bottom + 25 + diff - window.innerHeight),
        ...
        show: function (node) {
          if (node && node.id && this.activeSectionId != node.id) {
            switch (
              ((this.activeSectionId = node.id), BX.removeClass(node, "bx-step-error bx-step-warning"), node.id)
            ) {
              case this.authBlockNode.id:
          ...
          for (
            ...
            if (
              ...
              this.activeSectionId !== allSections[i].id &&
                (BX(this.activeSectionId) && this.fade(BX(this.activeSectionId)), this.show(allSections[i]));
              break;
            }
            BX.addClass(allSections[i], "bx-step-completed"), allSections[i].setAttribute("data-visited", "true"), i++;
          }
        },
        ...
        changeVisibleContent: function () {
          var sections = this.orderBlockNode.querySelectorAll(".bx-soa-section.bx-active"),
            ...
            state,
            orderDataLoaded =
              !!this.result.IS_AUTHORIZED && "Y" === this.params.USE_PRELOAD && !0 !== this.result.LAST_ORDER_DATA.FAIL,
            skipFlag = !0;
          for (i = 0; i < sections.length; i++)
            (state = (state = this.firstLoad && orderDataLoaded) || this.shouldBeSectionVisible(sections, i)),
              ...
              this.firstLoad &&
                ...
                ((orderDataLoaded && this.checkPreload(sections[i])) ||
                  ...
                  ? (this.fade(sections[i]), this.markSectionAsCompleted(sections[i]), this.show(sections[i + 1]))
                  : (skipFlag = !1));
          (this.result.IS_AUTHORIZED && void 0 === this.result.LAST_ORDER_DATA.FAIL) ||
        ...
        editOrder: function () {
          if (this.orderBlockNode && this.result) {
            this.result.DELIVERY.length > 0
              ...
              : (BX.removeClass(this.deliveryBlockNode, "bx-active"), (this.deliveryBlockNode.style.display = "none")),
        ...
        editSection: function (section) {
          if (section && section.id) {
            this.result.SHOW_AUTH && section.id != this.authBlockNode.id && section.id != this.basketBlockNode.id
            ...
            switch (
              ...
              (this.hasErrorSection[section.id] = errorContainer && "none" != errorContainer.style.display),
              section.id)
            ) {
              case this.authBlockNode.id:
                this.editAuthBlock();
        ...
        editAuthorizeForm: function (authContent) {
          ...
          (login = this.createAuthFormInputContainer(
                ...
                BX.create("DIV", {
                  ...
                  children: [
                    BX.create("LABEL", {
                      ...
                      children: [
                        BX.create("INPUT", { props: { type: "checkbox", name: "USER_REMEMBER", value: "Y" } }),
                        BX.create("SPAN", {
            ...
            (button = BX.create("DIV", {
              props: { className: "bx-authform-formgroup-container" },
              children: [
                BX.create("INPUT", { props: { id: "do_authorize", type: "hidden", name: "do_authorize", value: "N" } }),
                BX.create("button", {
          ...
          if (this.result.AUTH) {
            ...
            showSmsConfirm
              ? (authFormNodes.push(
                ...
                authFormNodes.push(
                  BX.create("INPUT", {
                    props: { type: "hidden", name: "SIGNED_DATA", value: this.result.SMS_AUTH.SIGNED_DATA || "" },
                  })
                ),
                authFormNodes.push(
                  this.createAuthFormInputContainer(
                    ...
                    BX.create("INPUT", {
                      attrs: { "data-send": !0 },
                      props: { name: "SMS_CODE", type: "text", size: 40, value: "" },
                      events: { keypress: BX.proxy(this.checkKeyPress, this) },
                    }),
                    !0
                  )
                ...
                authFormNodes.push(
                  BX.create("DIV", {
                    props: { className: "bx-authform-formgroup-container" },
                    children: [
                      BX.create("DIV", { props: { id: "bx_register_error" }, style: { display: "none" } }),
                      BX.create("DIV", { props: { id: "bx_register_resend" } }),
                    ],
                  })
                ))
              : (authFormNodes.push(
                  BX.create("H3", { props: { className: "bx-title" }, text: BX.message("STOF_REG_REQUEST") })
                ),
                authFormNodes.push(
                  this.createAuthFormInputContainer(
                  ...
                  authFormNodes.push(
                    BX.create("LABEL", {
                      ...
                      events: {
                        change: BX.delegate(function () {
                          var generated;
                          (this.authBlockNode.querySelector(".generated").style.display = "none"),
                  ...
                  BX.create("DIV", {
                    ...
                    children: [
                      this.createAuthFormInputContainer(
                        BX.message("STOF_LOGIN"),
                        BX.create("INPUT", {
                          props: { name: "NEW_LOGIN", type: "text", size: 30, value: this.result.AUTH.NEW_LOGIN || "" },
                          events: { keypress: BX.proxy(this.checkKeyPress, this) },
                        }),
                        !0
                      ),
                        ...
                        BX.create("DIV", {
                          ...
                          children: [
                            ...
                            BX.create("DIV", {
                              ...
                              children: [
                                BX.create("INPUT", {
                                  props: { name: "captcha_sid", type: "hidden", value: this.result.AUTH.capCode || "" },
                                }),
                                BX.create("IMG", {
                                  props: {
                ...
                authFormNodes.push(
                  BX.create("DIV", {
                    ...
                    children: [
                      BX.create("INPUT", {
                        props: { id: "do_register", name: "do_register", type: "hidden", value: "N" },
                      }),
                      BX.create("INPUT", {
                        props: {
                          type: "submit",
          ...
          if (BX("bx-soa-soc-auth-services")) {
            ...
            if (!1 === this.socServiceHiddenNode) {
              ...
              BX.type.isDomNode(socServiceHiddenNode) &&
                ((this.socServiceHiddenNode = socServiceHiddenNode.innerHTML), BX.remove(socServiceHiddenNode));
            }
        ...
        alignBasketColumns: function () {
          if (this.basketBlockNode) {
            ...
            if (windowSize.innerWidth > 580 && windowSize.innerWidth < 992) {
              if (
                ((percent = 100), (basketRows = this.basketBlockNode.querySelectorAll(".bx-soa-basket-info")).length)
              ) {
                if (
                ...
                if ((columns = columnNodes.length) > 0)
                  for (
                    ...
                    for (
                      columnNodes = basketRows[i].querySelectorAll(".bx-soa-item-properties"), k = 0;
              ...
              if (
                (columnNodes = this.basketBlockNode.querySelectorAll(".bx-soa-item-properties")).length &&
                "" == columnNodes[0].style.width
              )
                return;
            ...
            : ((basketContent = node.querySelector(".bx-soa-section-content")),
              ...
              basketContent.appendChild(
                BX.create("DIV", {
                  ...
                  children: [
                    BX.create("DIV", { style: { overflowX: "auto", overflowY: "hidden" }, children: [basketTable] }),
                  ],
                })
              ),
        ...
        editBasketItems: function (basketItemsNode, active) {
          if (this.result.GRID.ROWS) {
            var index = 0,
              i;
            for (i in ("Y" === this.params.SHOW_BASKET_HEADERS && this.editBasketItemsHeader(basketItemsNode),
        ...
        editBasketItemsHeader: function (basketItemsNode) {
          if (basketItemsNode) {
            ...
            basketItemsNode.appendChild(
              BX.create("TR", { props: { className: "bx-soa-item-tr hidden-sm hidden-xs" }, children: headers })
            );
          }
        },
        createBasketItem: function (basketItemsNode, item, index, active) {
          ...
          if (active)
            for (i = 0; i < this.result.GRID.HEADERS_HIDDEN.length; i++)
              (tr = this.createBasketItemHiddenColumn(this.result.GRID.HEADERS_HIDDEN[i], item)),
                BX.type.isArray(tr) ? (hiddenColumns = hiddenColumns.concat(tr)) : tr && hiddenColumns.push(tr);
          cols = [
            BX.create("TD", {
              props: { className: "bx-soa-item-td" },
          ...
          (countCols = dopColCount + countCols + 1),
            withServices &&
              ((servicesItems = this.createServicesBasketItem(item, countCols)).forEach(function (elem, i, arr) {
                    ...
                    BX.create("TD", {
                      ...
                      children: [
                        ...
                        BX.create("DIV", {
                          ...
                          children: [
                            BX.create("TABLE", { props: { className: "bx-soa-info-block" }, children: hiddenColumns }),
                          ],
                        }),
                      ],
                    }),
          ...
          if (data)
            return (
              ...
              "Y" !== this.params.HIDE_DETAIL_PAGE_URL &&
                ...
                (logoNode = BX.create("A", { props: { href: data.DETAIL_PAGE_URL }, children: [logoNode] })),
            ...
            if ("PRICE_FORMATED" === column.id)
              textNode.appendChild(
                ...
                parseFloat(data.DISCOUNT_PRICE) > 0 &&
                  ...
                  textNode.appendChild(
                    BX.create("STRONG", { props: { className: "bx-price-old" }, html: data.BASE_PRICE_FORMATED })
                  )),
              ...
              textNode.appendChild(BX.create("STRONG", { props: { className: "bx-price all" }, html: data.SUM })),
                parseFloat(data.DISCOUNT_PRICE) > 0 &&
                  (textNode.appendChild(BX.create("BR")),
                  textNode.appendChild(
                    BX.create("STRONG", { props: { className: "bx-price-old" }, html: data.SUM_BASE_FORMATED })
                  ));
              ...
              if (BX.type.isArray(columnData)) {
                ...
                columnData && textNode.appendChild(BX.create("SPAN", { html: BX.util.htmlspecialchars(columnData) }));
            }
            return BX.create("TD", {
              props: { className: "bx-soa-item-td bx-soa-item-properties" + (toRight ? " bx-text-right" : "") },
              children: [
          ...
          if (column && allData) {
            ...
            if ("PROPS" !== column.id) {
              ...
              return BX.create("TR", {
                ...
                children: [
                  BX.create("TD", { props: { className: "bx-soa-info-title" }, text: column.name + ":" }),
                  textNode,
                ],
              });
            }
                      ...
                      if (content) {
                        ...
                        BX.cleanNode(content),
                          ...
                          contentHeight > windowSize.innerHeight * ratio &&
                            ...
                            (content.style.width =
                              contentWidth * ((windowSize.innerHeight * ratio) / contentHeight) + "px"),
                            (contentHeight = content.offsetHeight),
                            (contentWidth = content.offsetWidth)),
                          contentWidth > windowSize.innerWidth * ratio &&
                            ((content.style.width = windowSize.innerWidth * ratio + "px"),
                            (content.style.height =
                              contentHeight * ((windowSize.innerWidth * ratio) / contentWidth) + "px")),
        ...
        editCoupons: function (basketItemsNode) {
                    ...
                    BX.create("INPUT", {
                      ...
                      events: {
                        change: BX.delegate(function (event) {
                          ...
                          newCoupon &&
                            newCoupon.value &&
                            (this.sendRequest("enterCoupon", newCoupon.value), (newCoupon.value = ""));
                        }, this),
                      },
                    }),
                  ],
          ...
          basketItemsNode.appendChild(
            BX.create("DIV", { props: { className: "bx-soa-coupon" }, children: [couponsLabel, couponsBlock] })
          );
        },
        ...
        addCoupon: function (coupon) {
          for (
            ...
            couponListNodes[i].appendChild(this.getCouponNode({ text: coupon }, !0, "bx-soa-coupon-item-danger"));
        },
        ...
        editFadeRegionBlock: function () {
          var regionContent = this.regionBlockNode.querySelector(".bx-soa-section-content"),
            newContent;
          this.initialized.region || (this.editActiveRegionBlock(!1), BX.remove(BX.lastChild(this.regionBlockNode))),
        ...
        getLocationString: function (node) {
          ...
          if (locationInputNode && locationInputNode.value && locationInputNode.value.length)
            ...
            for (
              i = (locationSteps = node.querySelectorAll(".bx-ui-combobox-fake.bx-combobox-fake-as-input")).length;
              i--;

            )
              locationSteps[i].innerHTML.indexOf("...") >= 0 ||
                (locationSteps[i].innerHTML.indexOf("---") >= 0
                  ? (altLoc = BX("altProperty")) && altLoc.value.length && (locationString += altLoc.value)
        ...
        getZipLocationInput: function (node) {
          ...
          zipProperty &&
            ...
            (labelTextHtml = "Y" == zipProperty.REQUIRED ? '<span class="bx-authform-starrequired">*</span> ' : ""),
        ...
        getPersonTypeControl: function (node) {
          if (this.result.PERSON_TYPE) {
            ...
            if (
              (personTypesCount > 1 &&
                ((input = BX.create("DIV", {
                  props: { className: "form-group" + (personTypesCount > 2 ? " bx-soa-location-input-container" : "") },
            ...
            oldPersonTypeId &&
              node.appendChild(
                BX.create("INPUT", { props: { type: "hidden", name: "PERSON_TYPE_OLD", value: oldPersonTypeId } })
              );
          }
        },
            ...
            if (
              ...
              for (i in this.result.USER_PROFILES)
                this.result.USER_PROFILES.hasOwnProperty(i) &&
                  ...
                  node.appendChild(
                    BX.create("INPUT", {
                      props: { name: "PROFILE_ID", type: "hidden", value: this.result.USER_PROFILES[i].ID },
                    })
                  );
        },
        ...
        editActivePaySystemBlock: function (activeNodeMode) {
          ...
          if (this.initialized.paySystem);
            ...
            if (
              ...
              divs &&
                divs.length &&
                (BX.cleanNode(divs[0]), this.editCoupons(divs[0]), (this.couponsCustomInitialized = !0));
            }
        ...
        editPaySystemInfo: function (paySystemNode) {
          if (this.result.PAY_SYSTEM && (0 != this.result.PAY_SYSTEM.length || "Y" == this.result.PAY_FROM_ACCOUNT)) {
            var paySystemInfoContainer = BX.create("DIV", {
                props: {
                  className:
                    (0 == this.result.PAY_SYSTEM.length ? "col-sm-12" : "col-sm-5") + " bx-soa-pp-desc-container",
                },
              }),
              innerPs,
              extPs,
            ...
            BX.cleanNode(paySystemInfoContainer),
              "Y" == this.result.PAY_FROM_ACCOUNT && (innerPs = this.getInnerPaySystem(paySystemInfoContainer)),
              (currentPaySystem = this.getSelectedPaySystem()),
              paySystemInfoContainer.appendChild(
                BX.create("DIV", { props: { className: "bx-soa-pp-company" }, children: [innerPs, void 0, extPs] })
              );
        ...
        selectPaySystem: function (event) {
          if (this.orderBlockNode && event) {
            var target = event.target || event.srcElement,
              ...
              selectedSection;
            if (innerPsAction)
              "INPUT" == target.nodeName && (innerPaySystemCheckbox.checked = !innerPaySystemCheckbox.checked),
                innerPaySystemCheckbox.checked
                  ? (BX.removeClass(innerPaySystemSection, "bx-selected"), (innerPaySystemCheckbox.checked = !1))
                  : (BX.addClass(innerPaySystemSection, "bx-selected"), (innerPaySystemCheckbox.checked = !0));
        ...
        editFadeDeliveryContent: function (node) {
          var selectedDelivery = this.getSelectedDelivery(),
            ...
            arNodes,
            i;
          errorNode && errorNode.innerHTML ? node.appendChild(errorNode.cloneNode(!0)) : this.getErrorContainer(node),
            warningNode && warningNode.innerHTML && node.appendChild(warningNode.cloneNode(!0)),
          ...
          if (this.orderBlockNode) {
            ...
            if (!actionCompany) {
              if (BX.hasClass(actionSection, "bx-selected"))
                return "INPUT" == target.nodeName || "A" == target.nodeName || "LABEL" == target.nodeName
                  ? void 0
                  : BX.PreventDefault(event);
              actionSection &&
        ...
        activatePickUp: function (deliveryName) {
          this.pickUpBlockNode &&
            ...
            BX.hasClass(this.pickUpBlockNode, "bx-active") ||
              (BX.addClass(this.pickUpBlockNode, "bx-active"), (this.pickUpBlockNode.style.display = "")));
        },
        deactivatePickUp: function () {
          this.pickUpBlockNode &&
            this.pickUpHiddenBlockNode &&
            BX.hasClass(this.pickUpBlockNode, "bx-active") &&
            (BX.removeClass(this.pickUpBlockNode, "bx-active"), (this.pickUpBlockNode.style.display = "none"));
        },
          ...
          this.initialized.pickup
            ? ("Y" === this.params.SHOW_NEAREST_PICKUP &&
              ...
              this.maps &&
                ...
                ((this.pickUpMapFocused = !0), setTimeout(BX.proxy(this.maps.pickUpMapFocusWaiter, this.maps), 200)))
            : ((pickUpContent = node.querySelector(".bx-soa-section-content")) ||
              ...
              pickUpContent.appendChild(
                BX.create("DIV", { props: { className: "bx_soa_pickup row" }, children: [pickUpContentCol] })
              ),
        ...
        editFadePickUpBlock: function () {
          var pickUpContent = this.pickUpBlockNode.querySelector(".bx-soa-section-content"),
            newContent;
          this.initialized.pickup || (this.editActivePickUpBlock(!1), BX.remove(BX.lastChild(this.pickUpBlockNode))),
            (newContent = this.getNewContainer()),
          ...
          selectedPickUp &&
            ("Y" == this.params.SHOW_STORES_IMAGES &&
              (html +=
                '<img src="' +
                (imgSrc =
                  (logotype = this.getImageSources(selectedPickUp, "IMAGE_ID")).src_1x || this.defaultStoreLogo) +
        ...
        checkPickUpShow: function () {
          ...
          currentDelivery &&
            ...
            stores && stores.length
              ? ((name =
                  "N" != this.params.SHOW_DELIVERY_PARENT_NAMES ? currentDelivery.NAME : currentDelivery.OWN_NAME),
        ...
        geoLocationSuccessCallback: function (result) {
          var activeStores,
            currentDelivery = this.getSelectedDelivery();
          currentDelivery && currentDelivery.STORE && (activeStores = this.getPickUpInfoArray(currentDelivery.STORE)),
            activeStores &&
        ...
        getStoreInfoHtml: function (currentStore) {
          ...
          return (
            currentStore.ADDRESS &&
              (html +=
                BX.message("SOA_PICKUP_ADDRESS") + ": " + BX.util.htmlspecialchars(currentStore.ADDRESS) + "<br>"),
            currentStore.PHONE &&
            ...
            currentStore.DESCRIPTION &&
              (html +=
                BX.message("SOA_PICKUP_DESC") + ": " + BX.util.htmlspecialchars(currentStore.DESCRIPTION) + "<br>"),
            html
          );
        },
        ...
        editPropsBlock: function (active) {
          this.propsBlockNode &&
            ...
            (active ? this.editActivePropsBlock(!0) : this.editFadePropsBlock(), (this.initialized.props = !0));
        },
        editActivePropsBlock: function (activeNodeMode) {
          var node = activeNodeMode ? this.propsBlockNode : this.propsHiddenBlockNode,
            ...
            i,
            validationErrors;
          if (this.initialized.props) this.maps && setTimeout(BX.proxy(this.maps.propsMapFocusWaiter, this.maps), 200);
          else {
            if (
          ...
          if (property.getId() in this.locations)
            if (disabled) {
              if (
                ...
                for (propNodes = propRow.querySelectorAll("div.bx-soa-loc"), i = 0; i < propNodes.length; i++)
              ...
              for (
                ...
                property.isMultiple() &&
                  (insertedLoc.style.marginBottom = "search" == this.locationsTemplate ? "5px" : "20px"),
          ...
          if (disabled) {
            if (
              ...
              if (
                ...
                for (i = 0; i < inputs.length; i++) inputs[i].value.length && values.push(inputs[i].value);
              propsItemNode.innerHTML += this.valuesToString(values);
            }
          } else
            (propContainer = BX.create("DIV", { props: { className: "soa-property-container" } })),
          ...
          if (disabled) {
            if (
              ...
              if (((values = []), (inputs = prop.querySelectorAll("input[type=radio]")).length))
                for (i = 0; i < inputs.length; i++) inputs[i].checked && values.push(inputs[i].nextSibling.nodeValue);
              if ((inputs = prop.querySelectorAll("option")).length)
                for (i = 0; i < inputs.length; i++) inputs[i].selected && values.push(inputs[i].innerHTML);
              propsItemNode.innerHTML += this.valuesToString(values);
            }
          } else
            (propContainer = BX.create("DIV", { props: { className: "soa-property-container" } })),
        ...
        insertNumberProperty: function (property, propsItemNode, disabled) {
          ...
          if (disabled) {
            if (
              ...
              for (values = [], inputs = prop.querySelectorAll("input[type=text]"), i = 0; i < inputs.length; i++)
              ...
              BX.delegate(function (e) {
                ...
                if (
                  ...
                  textInputs.length &&
                    ...
                    "DATE" == settings.TYPE && this.alterDateProperty(settings, textInputs[textInputs.length - 1]),
                    settings.PATTERN &&
        ...
        isValidForm: function () {
          ...
          if (
            ...
            if (this.activeSectionId == this.propsBlockNode.id) {
              for (tooltips = this.propsBlockNode.querySelectorAll("div.tooltip"), i = 0; i < tooltips.length; i++)
        ...
        isValidProperty: function (data, fieldName) {
          ...
          for (i = 0; i < data.inputs.length; i++)
            (inputErrors = data.func(data.inputs[i], !!fieldName)).length && (propErrors[i] = inputErrors.join("<br>"));
        ...
        validateCheckbox: function (input, arProperty, fieldName) {
          ...
          return "Y" == arProperty.MULTIPLE
            ? errors
            : ("Y" != arProperty.REQUIRED || input.checked || errors.push(field + " " + BX.message("SOA_REQUIRED")),
              errors);
        },
        ...
        validateEnum: function (inputs, arProperty, fieldName) {
          ...
          return (
            "Y" == arProperty.REQUIRED && 0 == values.length && errors.push(field + " " + BX.message("SOA_REQUIRED")),
            errors
          );
        },
          ...
          if (this.totalInfoBlockNode && this.result.TOTAL) {
            ...
            (params = {}),
              ...
              this.options.showDiscountPrice &&
                ...
                this.totalInfoBlockNode.appendChild(
                  this.createTotalUnit(discText + ":", total.DISCOUNT_PRICE_FORMATED, { highlighted: !0 })
                )),
              this.options.showPayedFromInnerBudget
                ? (this.totalInfoBlockNode.appendChild(
                ...
                : this.totalInfoBlockNode.appendChild(
                    this.createTotalUnit(BX.message("SOA_SUM_IT"), total.ORDER_TOTAL_PRICE_FORMATED, { total: !0 })
                  ),
              ...
              this.result.SHOW_AUTH ||
                this.totalInfoBlockNode.appendChild(
                  BX.create("DIV", {
                    props: {
                      className:
                        "bx-soa-cart-total-button-container lic_condition" + (showOrderButton ? "" : " visible-xs"),
                    },
                    children: [
                      BX.create("A", {
                        props: { href: "javascript:void(0)", className: "btn btn-default btn-lg btn-order-save" },
            ...
            (tableNode = (parentNode = scrollableNodes[i]).querySelector("table.bx-soa-item-table")),
              ...
              (scrollable = scrollable || tableWidth > parentWidth)
                ? ((scrollLeft = (scrollNode = BX.firstChild(parentNode)).scrollLeft),
                  ...
                  0 == scrollLeft && 0 == scrollRight && BX.addClass(parentNode, "bx-soa-table-fade-right"))
        ...
        totalBlockScrollCheck: function () {
          if (this.totalInfoBlockNode && this.totalGhostBlockNode) {
            ...
            headerFixed && BX.hasClass(headerFixed, "fixed") && (offset = headerFixed.offsetHeight + 20),
              ...
              scrollTop + offset > ghostTop && !BX.hasClass(this.totalInfoBlockNode, "bx-soa-cart-total-fixed")
          ...
          if (this.params.DATA_LAYER_NAME) {
            ...
            for (i in this.result.GRID.ROWS)
              if (this.result.GRID.ROWS.hasOwnProperty(i)) {
                for (item = this.result.GRID.ROWS[i], dataVariant = [], i = 0; i < item.data.PROPS.length; i++)
