...
?>
        <div class="item-views reviews<?= $bCompact ? ' compact' : ''; ?><?= $bMoreItem ? ' more-item' : ''; ?> <?= $arParams['LINKED_MODE'] == 'Y' ? ' linked ' : ''; ?> <?= $notSlider ? ' list-mode ' : ''; ?>" itemscope itemtype="https://schema.org/Review">
            ...
            <?if($notSlider):?>
                <div class="appear-block loading_state<?= !$bOneItem ? ' shadow' : ''; ?> items" >
            <?else:?>
                <?
                $arOptions = [
                    'preloadImages' => false,
            ...
            <?if($notSlider && $arParams['DISPLAY_BOTTOM_PAGER']):?>
                <div class="bottom_nav_wrapper">
                    <div class="bottom_nav animate-load-state<?= $arResult['NAV_STRING'] ? ' has-nav' : ''; ?>" <?= $isAjax ? "style='display: none; '" : ''; ?> data-parent=".item-views" data-append=".items" data-target=".item-wrapper">
                        <?=$arResult['NAV_STRING']; ?>
                    </div>
                </div>
            <?endif; ?>
