...
<div class="top_big_one_banner short_block top_margin_banner top_big_banners <?=$arResult['HAS_CHILD_BANNERS'] ? 'with_childs' : '';?>" style="overflow: hidden;">
    <div class="row dd">
        <?if ($arResult['HAS_SLIDE_BANNERS'] && $arResult['HAS_CHILD_BANNERS']):?>
            ...
            <div class="col-md-12 items visible_side_mobile clearfix side-childs <?=$arParams['SIDE_SLIDER_VIEW_MOBILE'];?><?='slider' === $arParams['SIDE_SLIDER_VIEW_MOBILE'] ? ' swipeignore mobile-overflow' : '';?> c_<?=count($arResult['ITEMS'][$arParams['BANNER_TYPE_THEME_CHILD']]['ITEMS']);?>">
                <?foreach ($arResult['ITEMS'][$arParams['BANNER_TYPE_THEME_CHILD']]['ITEMS'] as $key => $arItem):?>
                    <?php
                    if ($key > 3) {
                        continue;
        ...
        <?elseif ($arResult['HAS_CHILD_BANNERS']):?>
            <div class="col-md-12 items clearfix side-childs <?=$arParams['SIDE_SLIDER_VIEW_MOBILE'];?><?='slider' === $arParams['SIDE_SLIDER_VIEW_MOBILE'] ? ' swipeignore mobile-overflow' : '';?> c_<?=count($arResult['ITEMS'][$arParams['BANNER_TYPE_THEME_CHILD']]['ITEMS']);?>">
                <?foreach ($arResult['ITEMS'][$arParams['BANNER_TYPE_THEME_CHILD']]['ITEMS'] as $key => $arItem):?>
                    <?include 'float.php';?>
                <?endforeach;?>
            </div>
        ...
        <?if ($arResult['HAS_CHILD_BANNERS2']):?>
            <div class="col-md-12 items <?=$arParams['SLIDER_VIEW_MOBILE'];?><?='slider' === $arParams['SLIDER_VIEW_MOBILE'] ? ' swipeignore mobile-overflow' : '';?> c_<?=count($arResult['ITEMS'][$arParams['BANNER_TYPE_THEME_CHILD2']]['ITEMS']);?>">
