					...
					<?foreach($arParams['ITEMS'] as $arItem):?>
						<?if ($arParams['PARAMS']['SHOW_BY_GROUPS'] === 'Y'):?>
							...
							<?if(
								...
								<?if(strlen($conditionName)):?>
									<div class="landings-list__section-title font_xs option-font-bold darken <?=$bModile ? 'landings-list__section-title--mobiled visible-xs' : 'hidden-xs'?>">
                                        <?=htmlspecialcharsbx($conditionName);?>
                                    </div>
								<?endif;?>
							<?endif;?>
							<?$lastConditionId = $arParams['INFO'][$arItem['TAG_ID']]['FILTER_CONDITION_ID'];?>
						...
						<div class="landings-list__item font_xs <?=$class?>">
							<div>
								<?if(strlen($url)):?>
									<?if($arItem['ACTIVE_URL'] == 'Y'):?>
										<span class="landings-list__name rounded3 landings-list__item--active <?=($bActiveFilter ? 'landings-list__item--reset' : '')?>"><span><?=htmlspecialcharsbx($arItem['NAME'])?></span>
											<?if($arItem['ACTIVE']):?>
									...
									<?else:?>
										<a class="landings-list__name<?=($bFilled ? ' landings-list__item--filled-bg box-shadow-sm' : ' landings-list__item--hover-bg')?> rounded3" href="<?=htmlspecialcharsbx($url);?>"><span><?=htmlspecialcharsbx($arItem['NAME'])?></span></a>
									<?endif;?>
								<?else:?>
									<span class="landings-list__name<?=($bFilled ? ' landings-list__item--filled-bg box-shadow-sm' : ' landings-list__item--hover-bg')?> rounded3"><span><?=htmlspecialcharsbx($arItem['NAME'])?></span></span>
								<?endif?>
							</div>
						</div>
					<?endforeach;?>
