...
$moduleFilePrefix = array('esol_export_excel', 'esol_import_excel');
...
if($USER->IsAdmin())
{
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['action']=='convert_profiles' && check_bitrix_sessid())
	{
		...
		$timeBegin = time();
		$maxTime = 5;
		$cnt = (isset($_POST['cnt']) ? (int)$_POST['cnt'] : 0);
		$lastId = (isset($_POST['lastId']) ? $_POST['lastId'] : '');
		
		if(class_exists('\CKDAImportProfile'))
		{
			if(strlen($lastId)==0 || preg_match('/^i(\d+)$/', $lastId, $m))
			{
				$entity = \CKDAImportProfile::getInstance()->GetEntity();
				$dbRes = $entity->getList(array('order'=>array('ID'=>'ASC'), 'filter'=>array('>ID'=>(strlen($lastId) > 0 ? (int)$m[1] : 0)), 'select'=>array('ID', 'PARAMS')));
				while($arProfile = $dbRes->Fetch())
				{
					$entity->Update($arProfile['ID'], array('PARAMS'=>$arProfile['PARAMS'], 'EXPRESSIONS'=>'?'));
					$cnt++;
					if(time() - $timeBegin > $maxTime)
					{
						echo \KdaIE\Utils::PhpToJSObject(array('result'=>'progress', 'cnt'=>$cnt, 'lastId'=>'i'.$arProfile['ID']));
						die();
					}
				}
				$lastId = '';
			...
			if(strlen($lastId)==0 || preg_match('/^ih(\d+)$/', $lastId, $m))
			{
				$entity = \CKDAImportProfile::getInstance('highload')->GetEntity();
				$dbRes = $entity->getList(array('order'=>array('ID'=>'ASC'), 'filter'=>array('>ID'=>(strlen($lastId) > 0 ? (int)$m[1] : 0)), 'select'=>array('ID', 'PARAMS')));
				while($arProfile = $dbRes->Fetch())
				{
					$entity->Update($arProfile['ID'], array('PARAMS'=>$arProfile['PARAMS'], 'EXPRESSIONS'=>'?'));
					...
					if(time() - $timeBegin > $maxTime)
					{
						echo \KdaIE\Utils::PhpToJSObject(array('result'=>'progress', 'cnt'=>$cnt, 'lastId'=>'ih'.$arProfile['ID']));
						die();
					}
				}
				$lastId = '';
		...
		if(class_exists('\CKDAExportProfile'))
		{
			if(strlen($lastId)==0 || preg_match('/^e(\d+)$/', $lastId, $m))
			{
				$entity = \CKDAExportProfile::getInstance()->GetEntity();
				$dbRes = $entity->getList(array('order'=>array('ID'=>'ASC'), 'filter'=>array('>ID'=>(strlen($lastId) > 0 ? (int)$m[1] : 0)), 'select'=>array('ID', 'PARAMS')));
				while($arProfile = $dbRes->Fetch())
				{
					$entity->Update($arProfile['ID'], array('PARAMS'=>$arProfile['PARAMS'], 'EXPRESSIONS'=>'?'));
					...
					if(time() - $timeBegin > $maxTime)
					{
						echo \KdaIE\Utils::PhpToJSObject(array('result'=>'progress', 'cnt'=>$cnt, 'lastId'=>'e'.$arProfile['ID']));
						die();
					}
				}
				$lastId = '';
			...
			if(strlen($lastId)==0 || preg_match('/^eh(\d+)$/', $lastId, $m))
			{
				$entity = \CKDAExportProfile::getInstance('highload')->GetEntity();
				$dbRes = $entity->getList(array('order'=>array('ID'=>'ASC'), 'filter'=>array('>ID'=>(strlen($lastId) > 0 ? (int)$m[1] : 0)), 'select'=>array('ID', 'PARAMS')));
				while($arProfile = $dbRes->Fetch())
				{
					$entity->Update($arProfile['ID'], array('PARAMS'=>$arProfile['PARAMS'], 'EXPRESSIONS'=>'?'));
					...
					if(time() - $timeBegin > $maxTime)
					{
						echo \KdaIE\Utils::PhpToJSObject(array('result'=>'progress', 'cnt'=>$cnt, 'lastId'=>'eh'.$arProfile['ID']));
						die();
					}
				}
				$lastId = '';
		...
		$arFilter = array('MODULE_ID'=>$moduleId, 'TAG'=>'CONVERT_PROFILES');
		$dbRes = \CAdminNotify::GetList(array(), $arFilter);
		while($arr = $dbRes->Fetch())
		{
			\CAdminNotify::Delete($arr['ID']);
		}
		
		echo \KdaIE\Utils::PhpToJSObject(array('result'=>'finish', 'cnt'=>$cnt));
		die();
	}
	
	/*Security filter*/
	$optionFilterMask = COption::GetOptionString($moduleId, 'SECURITY_FILTER_MASK', 'N');
	$mask = array();
	if(is_array($moduleFilePrefix))
	{
		foreach($moduleFilePrefix as $prefix)
		{
			$mask[] = '/bitrix/admin/'.$prefix.'*';
		}
	}
	else $mask[] = '/bitrix/admin/'.$moduleFilePrefix.'*';
	$findMask = false;
	$arMasks = array();
	...
	if(!$findMask && $optionFilterMask=='Y') COption::SetOptionString($moduleId, 'SECURITY_FILTER_MASK', 'N');
	elseif($findMask && $optionFilterMask!='Y') COption::SetOptionString($moduleId, 'SECURITY_FILTER_MASK', 'Y');
	/*Security filter*/
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && check_bitrix_sessid())
	{
		if(isset($_POST['Update']) && $_POST['Update'] === 'Y' && is_array($_POST['SETTINGS']))
		{
			foreach($_POST['SETTINGS'] as $k=>$v)
			{
				...
				if($k=='SECURITY_FILTER_MASK' && Loader::includeModule('security') && class_exists('\CSecurityFilterMask'))
				{
					if(!$findMask && $v=='Y')
					{
						foreach($mask as $mkey=>$mval)
						{
							$arMasks['n'.$mkey] = array('MASK'=>$mval, 'SITE_ID'=>'NOT_REF');
						}
	...
	<tr>
		<td width="50%"><? echo Loc::getMessage('KDA_IE_SECURITY_FILTER_MASK'); ?> <span id="hint_SECURITY_FILTER_MASK"></span><script>BX.hint_replace(BX('hint_SECURITY_FILTER_MASK'), '<?echo Loc::getMessage("KDA_IE_SECURITY_FILTER_MASK_HINT"); ?>');</script></td>
		<td width="50%">
			<input type="hidden" name="SETTINGS[SECURITY_FILTER_MASK]" value="N">
			<input type="checkbox" name="SETTINGS[SECURITY_FILTER_MASK]" value="Y" <?if(COption::GetOptionString($moduleId, 'SECURITY_FILTER_MASK', 'N')=='Y'){echo 'checked';}?>>
		</td>
	</tr>
	<tr>
...
if(typeof $=='function')
{
	...
	<h2><?=Loc::getMessage("KDA_IE_SYS_ROU"); ?></h2>
	<?
	$aTabs = array(
		array("DIV" => "edit0", "TAB" => Loc::getMessage("KDA_IE_EXP_CONVERSION_TAB"), "ICON" => "", "TITLE" => Loc::getMessage("KDA_IE_EXP_CONVERSION_TITLE")),
	);
	$tabControl = new CAdminTabControl("esolImportxmlTabControlSys", $aTabs, true, true);
	$tabControl->Begin();
	$tabControl->BeginNextTab();
	...
	<tr>
		<td colspan="2">
			<a name="profilesconv" style="position:absolute; margin-top: -140px;" href="#"></a>
			<form method="POST" action="<?echo htmlspecialcharsbx($APPLICATION->GetCurPage())?>?lang=<?echo LANGUAGE_ID?>&mid_menu=1&mid=<?=$module_id?>">
				<? echo bitrix_sessid_post();?>
				<input type="hidden" name="action" value="convert_profiles">
				<?/*?><h4><?echo Loc::getMessage("KDA_IE_EXP_CONVERSION_TAB");?></h4><?*/?>
				<div>&nbsp;</div>
				<table>
					<tr>
						<td valign="top">
							<input type="submit" class="adm-btn-save" name="convert_profiles" value="<? echo Loc::getMessage('KDA_IE_EXP_CONVERSION_START_BTN') ?>" onclick="return KdaOptions.ConvertProfiles(this)">
							<br><br>
						</td>
						<td>&nbsp; &nbsp; &nbsp;</td>
						<td valign="top">
							<div id="convert_profiles_message"></div>
						</td>
					</tr>
				</table>
				
				<p><?echo Loc::getMessage("KDA_IE_EXP_CONVERSION_NOTE")?></p>
			</form>
		</td>
	</tr>
	<?$tabControl->End();?>
