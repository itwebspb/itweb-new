...
class CKDAExportExcelWriterXlsx {
	...
	function __construct($arParams = array(), $ee = false)
	{
		...
		if(file_exists($funcFile))
		{
			...
			if(!\KdaIE\Utils::IsUtfMode() && CUtil::DetectUTF8($fileContent))
			{
				$fileContent = \Bitrix\Main\Text\Encoding::convertEncoding($fileContent, 'UTF-8', 'CP1251');
			}
			elseif(\KdaIE\Utils::IsUtfMode() && !CUtil::DetectUTF8($fileContent))
			{
			...
			while(!feof($handle)) 
			{
				...
				else
				{
					...
					foreach($arVals as $valIndex=>$arValue)
					{
						...
						foreach($arFields as $k=>$field)
						{
							$val = (!is_array($arValue[$k]) ? trim($arValue[$k]) : '');
    ...
    public function openExcelWriter()
    {
		...
		if($this->byTemplate)
		{
			...
			{
				$sxml = \KdaIE\Utils::GetSimplXmlFromFile($dirPath.'data/[Content_Types].xml');
				if($sxml->Default)
				{
			...
			{
				$sxml = \KdaIE\Utils::GetSimplXmlFromFile($dirPath.'data/xl/sharedStrings.xml');
				if($sxml->si)
				{
			...
			{
				$sxml = \KdaIE\Utils::GetSimplXmlFromFile($dirPath.'data/xl/workbook.xml');
			...
			{
				$sxml = \KdaIE\Utils::GetSimplXmlFromFile($dirPath.'data/xl/_rels/workbook.xml.rels');
			...
			if(file_exists($tmpStyleFile))
			{
				...
				$sxml = \KdaIE\Utils::GetSimplXmlFromFile($tmpStyleFile);
    ...
    public function openWorkSheet($listIndex, $realIndex, $isHidden = false)
    {
		...
		if($this->byTemplate)
		{
			...
			if(file_exists($tmpSheet))
			{
				...
				$arDefaultRows = array();
				$sxml = \KdaIE\Utils::GetSimplXmlFromFile($tmpSheet);
			...
			{
				$sxml = \KdaIE\Utils::GetSimplXmlFromFile($dirPath.'data/xl/worksheets/_rels/sheet'.$sheetId.'.xml.rels.tmp');
				if($sxml->Relationship)
				{
					foreach($sxml->Relationship as $relationship)
					{
			...
			{
				$sxml = \KdaIE\Utils::GetSimplXmlFromFile($dirPath.'data/xl/drawings/_rels/drawing'.$drawSheetId.'.xml.rels');
				if($sxml->Relationship)
				{
					foreach($sxml->Relationship as $relationship)
					{
	...
	public function getValueForXml($value, $quotes=true)
	{
		...
		$cnt = 1;
		while($cnt > 0) $value = preg_replace('/(?<!_x005F)_x(?!005F)[0-9A-Fa-f]{4}_/', '_x005F$0', $value, -1, $cnt);
