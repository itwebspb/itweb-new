...
class CKDAEEFieldList {
	...
	public static function GetFieldTitle($field, $IBLOCK_ID=0, $arParams=array())
	{
		$title = '';
		...
		{
			$fl = new CKDAEEFieldList($arParams);
			ob_start();
			$fl->ShowSelectFields($IBLOCK_ID, '');
			$s = ob_get_clean();
			if(preg_match_all('/<option[^>]*value="([^"]*)"[^>]*>(.*)<\/option>/Uis', $s, $m))
			{
				$arFields = array_combine($m[1], $m[2]);
				$title = html_entity_decode($arFields[$field]);
			}
		}
		return $title;
	}
	...
	public static function GetReviewFields($IBLOCK_ID)
	{
		$arReviewFields = array();
		if(Loader::includeModule('blog') && $IBLOCK_ID > 0 && \Bitrix\Iblock\PropertyTable::getCount(array('IBLOCK_ID'=>$IBLOCK_ID, '=CODE'=>'BLOG_POST_ID')) > 0)
		{
			$arReviewFields = array(
				"REVIEW_AUTHOR_NAME" => Loc::getMessage("KDA_EE_FI_REVIEW_AUTHOR_NAME"),
				"REVIEW_POST_TEXT" => Loc::getMessage("KDA_EE_FI_REVIEW_POST_TEXT"),
				"REVIEW_DATE_CREATE" => Loc::getMessage("KDA_EE_FI_REVIEW_DATE_CREATE")
			);
			if(\CUserTypeEntity::GetList(array(), array('ENTITY_ID'=>'BLOG_COMMENT', 'FIELD_NAME'=>'UF_ASPRO_COM_RATING'))->Fetch())
			{
				//$arReviewFields["REVIEW_UF_ASPRO_COM_RATING"] = Loc::getMessage("KDA_EE_FI_REVIEW_RATING");
			}
		}
	...
	public function GetFieldNames($IBLOCK_ID, $type='')
	{
		...
		if($type=='highload')
		{
			if(!$this->arFieldNames['hl'.$IBLOCK_ID])
			{
				$this->arFieldNames['hl'.$IBLOCK_ID] = array();
				$arFields = $this->GetHigloadBlockFields($IBLOCK_ID);
				foreach($arFields as $k=>$v)
				{
					$this->arFieldNames['hl'.$IBLOCK_ID][$k] = $v['NAME_LANG'];
				}
			}

			return $this->arFieldNames['hl'.$IBLOCK_ID];
		}
	...
	public function ShowSelectFields($IBLOCK_ID, $fname, $value="", $arParams=array())
	{
		...
		if($arReviewFields = self::GetReviewFields($IBLOCK_ID))
		{
			?><optgroup label="<?echo Loc::getMessage("KDA_EE_GROUP_REVIEWS")?>"><?
