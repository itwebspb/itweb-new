...
class CKDAExportExcelHighload {
	...
	function __construct($params=array(), $fparams=array(), $stepparams=false, $pid = false)
	{
		...
		if(!\CKDAExportProfile::getInstance('highload')->CheckExpClassFile($pid, get_class($this)))
		{
	...
	public static function getInstance($params=array(), $fparams=array(), $stepparams=false, $pid = false)
	{
		...
		if (!isset(static::$instance[$hash]))
		{
			...
			if($GLOBALS['USER']->IsAdmin() && strpos($pid, 'tmp')===0)
			{
				...
				\CKDAExportProfile::PrepareProfileFields($arTmpParams, array(), $pid, 'hltmp');
				$tmpFile = true;
			}
			
			$arErrors1 = $arErrors2 = array();
			if($tmpFile && count($arErrors1 = \CKDAExportProfile::getInstance('highload')->GetErrorsOnIncludeExpClass($pid, 'tmp', true))==0)
			{
				$fn = __DIR__ .'/profiles_ext/exporter'.(int)$pid.'_hltmp.php';
				$className = 'CKDAExportExcelHltmp'.(int)$pid;
			}
			elseif(count($arErrors2 = \CKDAExportProfile::getInstance('highload')->GetErrorsOnIncludeExpClass($pid, '', true))==0)
			{
				$fn = __DIR__ .'/profiles_ext/exporter'.(int)$pid.'_hl.php';
				$className = 'CKDAExportExcelHl'.(int)$pid;
			}
			else
			{
				$fn = __DIR__ .'/profiles_ext/exporter_highload.php';
				$className = 'CKDAExportExcelHighload';
			}
				...
				foreach($this->fparams[$listIndex] as $fieldIndex=>$arSettings)
				{
					...
					if(isset($arSettings['CONVERSION']) && is_array($arSettings['CONVERSION']) && $field && isset($arElementData[$field]))
					{
						if(is_array($arElementData[$field]))
						{
							...
							{
								$arData[$k][$field.'_'.$fieldIndex][$k2] = $arElementData[$field.'_'.$fieldIndex][$k2] = $newVal = $this->ApplyConversions($val, $arSettings['CONVERSION'], $arElementData, array('NAME'=>$field, 'FIELDKEY'=>$fieldIndex));
								if($newVal===false) $skipLine = true;
							}
						}
						else
						{
							$arData[$k][$field.'_'.$fieldIndex] = $arElementData[$field.'_'.$fieldIndex] = $newVal = $this->ApplyConversions($arElementData[$field], $arSettings['CONVERSION'], $arElementData, array('NAME'=>$field, 'FIELDKEY'=>$fieldIndex));
							if($newVal===false) $skipLine = true;
						}
					}
