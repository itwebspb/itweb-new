...
class CKDAExportExcel {
	...
	function __construct($params=array(), $fparams=array(), $stepparams=false, $pid = false)
	{
		...
		$this->bBlog = \Bitrix\Main\Loader::IncludeModule('blog');
		...
		if(!\CKDAExportProfile::getInstance()->CheckExpClassFile($pid, get_class($this)))
		{
			$this->expClassFileError = true;
			return;
		}
	}
	...
	public static function getInstance($params=array(), $fparams=array(), $stepparams=false, $pid = false)
	{
		...
		if (!isset(static::$instance[$hash]))
		{
			...
			if($GLOBALS['USER']->IsAdmin() && strpos($pid, 'tmp')===0)
			{
				$pid = substr($pid, 3);
				$arTmpParams = array('PARAMS'=>\CKDAExportProfile::EncodeProfileParams(array('EXTRASETTINGS'=>$fparams)));
				\CKDAExportProfile::PrepareProfileFields($arTmpParams, array(), $pid, 'tmp');
				$tmpFile = true;
			}
	
			$arErrors1 = $arErrors2 = array();
			if($tmpFile && count($arErrors1 = \CKDAExportProfile::getInstance()->GetErrorsOnIncludeExpClass($pid, 'tmp', true))==0)
			{
				$fn = __DIR__ .'/profiles_ext/exporter'.(int)$pid.'_tmp.php';
				$className = 'CKDAExportExcelTmp'.(int)$pid;
			}
			elseif(count($arErrors2 = \CKDAExportProfile::getInstance()->GetErrorsOnIncludeExpClass($pid, '', true))==0)
			{
				$fn = __DIR__ .'/profiles_ext/exporter'.(int)$pid.'.php';
				$className = 'CKDAExportExcel'.(int)$pid;
			}
			else
			{
				$fn = __DIR__ .'/profiles_ext/exporter.php';
				$className = 'CKDAExportExcel';
			}
			if(count($arErrors = $arErrors1) > 0 || count($arErrors = $arErrors2) > 0)
			{
				list($params['PHPEXPERROR_SHEET'], $params['PHPEXPERROR_FIELD']) = explode('|', $arErrors['FIELD_INDEX']);
				unset($arErrors['FIELD_INDEX']);
				$params['PHPEXPERROR_TEXT'] = implode("\r\n", $arErrors);
			}
			
			if(file_exists($fn))
			{
				include_once($fn);
				static::$instance[$hash] = new $className($params, $fparams, $stepparams, $pid);
			}
			else static::$instance[$hash] = new static($params, $fparams, $stepparams, $pid);
		}
	...
	{
		if(isset($this->expClassFileError) && $this->expClassFileError===true)
		{
			$isCron = ($this->stepparams['EXPORT_MODE']=='CRON');
			$this->errors[] = sprintf(Loc::getMessage("KDA_EE_ERROR_EXP_CLASS_FILE"), '/bitrix/admin/settings.php?lang=ru&mid='.static::$moduleId.'&mid_menu=1#profilesconv');
			if($isCron) \KdaIE\Utils::AddNotifyConvertProfiles('error');
			return $this->GetBreakParams($isCron ? 'finish_error' : 'finish');
		}
		...
		foreach($arListIndexes as $listIndex)
		{
			...
			$this->UpdateColumnTitles($arFields, $listIndex);
		}
	...
	public function CheckExtServices($outputFile)
	{
		...
		if($this->params['EXPORT_TO_YADISK']=="Y" && $this->params['YADISK_TOKEN'] && $this->params['YADISK_PATH'])
		{
			...
			if(!\KdaIE\Utils::IsUtfMode()) $path = \Bitrix\Main\Text\Encoding::convertEncoding($path, 'CP1251', 'UTF-8');
			$client = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>15));
			...
			{
				$client = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>60));
				$client->setHeader('Authorization', "OAuth ".$token);
		...
		if($this->params['EXPORT_TO_GOOGLE_SPREADSHEETS']=="Y" && $this->params['GOOGLE_TOKEN'] && $this->params['GOOGLE_SID'] && function_exists('json_encode'))
		{
			...
			$accessToken = '';
			$ob = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>20));
			...
			if($accessToken)
			{
				...
				if($tblSheet!==false)
				{
					...
					foreach($this->params['LIST_NAME'] as $listIndex=>$listName)
					{
						...
						if(file_exists($dataFile) && isset($arSheets[$sheetNumber+$tblSheet]))
						{
							$arSheet = $arSheets[$sheetNumber+$tblSheet];
							...
							while(!feof($handle)) 
							{
								...
								if(isset($arElement['RTYPE']) && ($arElement['RTYPE']=='SECTION_PATH' || preg_match('/^SECTION_(\d+)$/', $arElement['RTYPE'], $m)))
								{
									...
									$sectionValue = $this->GetGoogleCellValue($arElement['NAME']);
									$arData[] = array($sectionValue);
									if(!preg_match('/^=[A-Z][A-Z0-9\.]+\(/', $sectionValue))
									{
										$arDataFormula[] = array();
									}
									else
									{
										$bFormula = true;
										$arDataFormula[] = array($sectionValue);
									}
								}
								else
								{
	...
	public function GetGoogleCellValue($val)
	{
		...
		if(preg_match('/<a[^>]+class="kda\-ee\-(conversion|section)\-link"[^>]+href="([^"]*)"[^>]*>(.*)<\/a>/Uis', $val, $m)) return '=HYPERLINK("'.addcslashes($m[2], '"').'"; "'.addcslashes($m[3], '"').'")';
		return (string)$val;
	}
	...
	public function ExecuteFilterExpression($val, $expression, $altReturn = true, $field = false, $convKey = '')
	{
		$sheetKey = $this->listIndex;
		$fieldKey = (is_array($field) && array_key_exists('FIELDKEY', $field) ? $field['FIELDKEY'] : '');
		$expression = trim($expression);
		if(strlen($expression)==0) return $val;
		
		$ret = '';
		try{
			$method = 'ExpSheet'.$sheetKey.'Field'.$fieldKey.'Conv'.$convKey;
			if(is_callable(array($this, $method)))
			{
				$ret = call_user_func(array($this, $method), $val);
			}
			else $ret = $val;
		}catch(Exception $ex){
			//return $altReturn;
	...
	public function GetFileValue($val, $key=false, $relField='', $index='')
	{
		if($val)
		{
			...
			if($this->params['EXPORT_FILES_IN_ARCHIVE']=='Y' && strlen($this->params['FILES_ARCHIVE_PATH']) > 0 && strlen($val) > 0 && file_exists($this->docRoot.$val))
			{
				...
				{
					$this->filesForMove[] = array('path'=>$val, 'conv'=>$this->fparamsByName[$this->listIndex][$key]['CONVERSION'], 'index'=>$index, 'field'=>array('FIELDKEY'=>$key));
				}
				else
				{
	...
	public function ProcessMoveFiles($arElementData)
	{
		...
		{
			$newPath = $this->ApplyConversions($arFile['path'], $arFile['conv'], $arElementData, $arFile['field'], $arFile['index']);
	...
	{
		if(file_exists($zipFile)) unlink($zipFile);
		if(strlen(trim($this->params['EXPORT_ARCHIVE_PASSWORD'])) > 0) $usePass = true;
		...
		{
			if($usePass) $zipObj->setPassword($this->params['EXPORT_ARCHIVE_PASSWORD']);
			if(is_dir($parentDir))
			{
				...
				foreach($files as $file){
					...
					if (is_dir($file) === true){
						...
						$fn = str_replace($parentDir, '', $file);
						$zipObj->addFile($file, $fn);
						/*
						if($usePass)
						{
							if(defined('\ZipArchive::EM_AES_256') && $zipObj->setEncryptionName($fn, \ZipArchive::EM_AES_256)){}
							elseif(defined('\ZipArchive::EM_TRAD_PKWARE') && $zipObj->setEncryptionName($fn, \ZipArchive::EM_TRAD_PKWARE)){}
						}
						*/
					}
				}
			...
			{
				$fn = bx_basename($parentDir);
				$zipObj->addFile($parentDir, $fn);
				if($usePass)
				{
					if(defined('\ZipArchive::EM_AES_256') && $zipObj->setEncryptionName($fn, \ZipArchive::EM_AES_256)){}
					elseif(defined('\ZipArchive::EM_TRAD_PKWARE') && $zipObj->setEncryptionName($fn, \ZipArchive::EM_TRAD_PKWARE)){}
		...
		{
			if($usePass && function_exists('exec') && !is_dir($parentDir))
			{
				$command = 'zip -j -P '.escapeshellarg(str_replace("'", '', $this->params['EXPORT_ARCHIVE_PASSWORD'])).' '.escapeshellarg($zipFile).' '.escapeshellarg($parentDir);
				exec($command);
			}
			if(!file_exists($zipFile))
			{
	...
	public function ConversionReplaceValues($m)
	{
		...
		if($isVar)
		{
			$this->convParams[$paramName] = $value;
			return '$this->convParams['.$quot.$paramName.$quot.']';
		}
		elseif($this->convIsExpression) $this->convParams[$paramName] = $value;
		return $value;
	}
	...
	public function ApplyConversions($val, $arConv, $arItem, $field=false, $valueIndex='', $iblockFields=array())
	{
		...
		if(is_array($arConv))
		{
			...
			$this->convIsExpression = (bool)($v['THEN']=='EXPRESSION');
			...
			foreach($arConv as $k=>$v)
			{
				...
				if(($v['CELL']=='ELSE' && !$execConv)
					...
					|| ($v['WHEN']=='REGEXP' && $this->CheckRegexpCond($v['FROM'], $condVal))
					|| ($v['WHEN']=='NOT_REGEXP' && !$this->CheckRegexpCond($v['FROM'], $condVal))
					...
					|| ($v['WHEN']=='ANY'))
				{
					...
					elseif($v['THEN']=='EXPRESSION') $val = $this->ExecuteFilterExpression($val, $v['TO'], '', $field, $k);
		...
		if(isset($this->params['PHPEXPERROR_SHEET']) && $this->params['PHPEXPERROR_SHEET']==$this->listIndex && is_array($field) && array_key_exists('FIELDKEY', $field) && isset($this->params['PHPEXPERROR_FIELD']) && $this->params['PHPEXPERROR_FIELD']==$field['FIELDKEY'] && isset($this->params['PHPEXPERROR_TEXT'])) $val = $this->params['PHPEXPERROR_TEXT'];
		
		return $val;
	}
	
	public function BacktrackLimitPrepare()
	{
		$backtrack_limit = ini_get('pcre.backtrack_limit') ? : 100000;
		ini_set('pcre.backtrack_limit', 10000);
		return $backtrack_limit;
	}
	
	public function BacktrackLimitRestore($backtrack_limit)
	{
		ini_set('pcre.backtrack_limit', $backtrack_limit);
	}
	
	public function GetRegexpProtectParam($val)
	{
		return preg_replace('#\\\*/#', '\/', $val);
	}
	
	public function CheckRegexpCond($from, $cond)
	{
		$backtrackLimit = $this->BacktrackLimitPrepare();
		$res = preg_match('/'.ToLower($this->GetRegexpProtectParam($from)).'/is'.CKDAExportUtils::getUtfModifier(), ToLower($cond));
		$this->BacktrackLimitRestore($backtrackLimit);
		return $res;
	}
	...
	public function GetConvReplaceRegexp($val, $from, $to, $substr = true)
	{
		...
		$safeFrom = $this->GetRegexpProtectParam($from);
		$bFinVal = false;
		if($substr===false)
		{
			if(!preg_match('/[\\\$]\d/', $to))
			...
			elseif(preg_match('/'.$safeFrom.'/i'.$mod, $val, $m)) $val = $m[0];
			elseif(preg_match('/'.ToLower($safeFrom).'/i'.$mod, $val, $m)) $val = $m[0];
			else 
			{
				$val = $to; 
				$bFinVal = true;
		...
		if(!$bFinVal)
		{
			if(preg_match('/'.$safeFrom.'/i'.$mod, $val)) $val = preg_replace('/'.$safeFrom.'/i'.$mod, $to, $val);
			else $val = preg_replace('/'.ToLower($safeFrom).'/i'.$mod, $to, $val);
		}
		$this->BacktrackLimitRestore($backtrackLimit);
		return $val;
	}
			...
			if(is_array($this->fparams[$listIndex]))
			{
				foreach($this->fparams[$listIndex] as $fieldIndex=>$arSettings)
				{
					...
					if(isset($arSettings['CONVERSION']) && is_array($arSettings['CONVERSION']) && $field && isset($arElementData[$field]))
					{
							...
							foreach($fieldVal as $k2=>$val)
							{
								if($isMulty)
								{
									...
									else
									{
										...
										$arData[$k][$field.'_'.$fieldIndex][$k2] = $arElementData[$field.'_'.$fieldIndex][$k2] = $newVal = $this->ApplyConversions($val, $arSettings['CONVERSION'], $arElementData2, array('NAME'=>$field, 'FIELDKEY'=>$fieldIndex), $k2);
								...
								$arData[$k][$field.'_'.$fieldIndex][$k2] = $arElementData[$field.'_'.$fieldIndex][$k2] = $newVal = $this->ApplyConversions($val, $arSettings['CONVERSION'], $arElementData, array('NAME'=>$field, 'FIELDKEY'=>$fieldIndex), $k2);
								if($newVal===false)
								{
						...
						{
							$arData[$k][$field.'_'.$fieldIndex] = $arElementData[$field.'_'.$fieldIndex] = $newVal = $this->ApplyConversions($fieldVal, $arSettings['CONVERSION'], $arElementData, array('NAME'=>$field, 'FIELDKEY'=>$fieldIndex));
							if($newVal===false) $skipLine = true;
					...
					if($isMultiple)
					{
						...
						{
							$val = $arVals;
							if($arVals['TYPE']=='MULTIROW') $arMultiRows[$field.'_'.$fieldIndex] = $field.'_'.$fieldIndex;
						}
						else
						{
	...
	public function UpdateColumnTitles($arFields, $listIndex)
	{
		if(!isset($this->params['FIELDS_LIST_NAMES'][$listIndex]) || !is_array($this->params['FIELDS_LIST_NAMES'][$listIndex])) return;
		foreach($this->params['FIELDS_LIST_NAMES'][$listIndex] as $k=>$v)
		{
			if($v=='{FIELD_NAME}')
			{
				$this->params['FIELDS_LIST_NAMES'][$listIndex][$k] = \CKDAEEFieldList::GetFieldTitle($arFields[$k], $this->GetIblockIdByListIndex($listIndex), $this->params);
			}
		}
	}
	...
	public function GetElementsData(&$arData, $arParams)
	{
		...
		foreach($arSections as $skey=>$arSection)
		{
			...
			foreach($arElementList as $arElement)
			{
				...
				if(count($arFieldsReview) > 0 && $this->bBlog && ($arPostProp = \CIBlockElement::GetProperty((int)$arElement['IBLOCK_ID'], (int)$arElement['ID'], array(), array('CODE'=>'BLOG_POST_ID'))->Fetch()) && (int)$arPostProp['VALUE'] > 0)
				{
					foreach($arFieldsReview as $reviewField)
					{
						$arElement2['REVIEW_'.$reviewField] = array('TYPE' => 'MULTIROW');
					}
					$dbRes = \CBlogComment::GetList(array('ID'=>'ASC'), array("POST_ID" => (int)$arPostProp['VALUE']), false, false, array('ID', 'AUTHOR_NAME', 'POST_TEXT', 'DATE_CREATE'));
					while($arr = $dbRes->Fetch())
					{
						foreach($arFieldsReview as $reviewField)
						{
							$arElement2['REVIEW_'.$reviewField][] = $arr[$reviewField];
						}
					}
				}
	...
	public static function CorrectCalculatedValue($val)
	{
		...
		if(!\KdaIE\Utils::IsUtfMode() && CUtil::DetectUTF8($val)/*function_exists('mb_detect_encoding') && (mb_detect_encoding($val) == 'UTF-8')*/)
		{
