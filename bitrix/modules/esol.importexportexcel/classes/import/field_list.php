...
class CKDAFieldList {
	...
	{
		$this->uid = (isset($params['ELEMENT_UID']) ? $params['ELEMENT_UID'] : '');
		$this->uidSku = (isset($params['ELEMENT_UID_SKU']) ? $params['ELEMENT_UID_SKU'] : '');
		$this->isSku = !empty($params['ELEMENT_UID_SKU']);
		$this->showStyles = (bool)(!isset($params['ELEMENT_NOT_LOAD_STYLES']) || $params['ELEMENT_NOT_LOAD_STYLES']!='Y');
		$this->iblockId = (isset($params['IBLOCK_ID']) ? (int)$params['IBLOCK_ID'] : 0);
		
		$this->sectionLevels = ((isset($params['MAX_SECTION_LEVEL']) && is_numeric($params['MAX_SECTION_LEVEL']) > 0) ? $params['MAX_SECTION_LEVEL'] : 5);
	...
	public function GetHigloadFiltrableFields($HIGHLOADBLOCK_ID)
	{
		...
		if(!$this->aHlFiltrableFields[$HIGHLOADBLOCK_ID])
		{
			$this->aHlFiltrableFields[$HIGHLOADBLOCK_ID]['hlbl'] = array(
				'title' => Loc::getMessage("KDA_IE_GROUP_HIGHLOAD"),
				'items' => array()
			);
	...
	public function GetHigloadBlockFields($HIGHLOADBLOCK_ID)
	{
		if(!isset($this->hlblFields[$HIGHLOADBLOCK_ID]))
		{
			...
			while($arHLField = $dbRes->Fetch())
			{
				...
				if(in_array($arHLField['USER_TYPE_ID'], array('string', 'hlblock', 'integer', 'double', 'datetime', 'date', 'boolean', 'enumeration', 'iblock_section', 'iblock_element')))
				{
					$arHLField['UID'] = 'Y';
					if($arHLField['MULTIPLE']!='Y') $arHLField['sortable'] = 'Y';
				}
				$arHLField['type'] = $type;
				$arHLField['filtrable'] = 'Y';
				
				$arHLField['NAME_LANG'] = ($arHLField['EDIT_FORM_LABEL'] ? $arHLField['EDIT_FORM_LABEL'] : $arHLField['FIELD_NAME']);
				$arHLField['name'] = $arHLField['NAME_LANG'];
