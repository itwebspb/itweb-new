...
class CKDAImportProfileDB extends CKDAImportProfileAll {
	...
	public function CheckStorage()
	{
		...
		$moduleVersion = false;
		$oldModuleVersion = \Bitrix\Main\Config\Option::get(static::$moduleId, $optionName);
		if(is_callable(array('\Bitrix\Main\ModuleManager', 'getVersion')))
		{
			...
			if($moduleVersion==$oldModuleVersion) return;
		}
		
		/*Security filter*/
		/*
		if(!$oldModuleVersion && Loader::includeModule('security') && class_exists('\CSecurityFilterMask'))
		{
		...
		*/
		...
		else
		{
			...
			foreach($fields as $columnName => $field)
			{
				...
				if(!in_array(ToUpper($realColumnName), $arDbFields))
				{
	...
	{
		$this->pid = $pid;
		$this->importMode = ((isset($arParams['IMPORT_MODE']) && $arParams['IMPORT_MODE']=='CRON') ? 'CRON' : 'USER');
