...
class CKDAImportUtils {
	...
	protected static $profileIdForExp = null;
	...
	public static function GetFileName($fn)
	{
		...
		if(\KdaIE\Utils::IsUtfMode()) $tmpfile = $APPLICATION->ConvertCharsetArray($fn, LANG_CHARSET, 'CP1251');
	...
	public static function SaveStat()
	{
		...
		if(0 && class_exists('\Bitrix\Main\Web\HttpClient') && is_callable(array('CUpdateClientPartner', 'GetUpdatesList')))
		{
			...
			$arPostData = array(
				...
				'ENCODING_UTF8' => (\KdaIE\Utils::IsUtfMode() ? 'Y' : 'N')
			);
	...
	public static function MakeFileArray($path, $maxTime = 0, $arCookies = array())
	{
		...
		$arArchiveExt = array('zip', 'rar', 'gz', 'tar.gz');
		if(is_array($path))
		{
			$arTmpFile = $path;
		...
		else
		{
			...
			if(strpos($path, '/')===0)
			{
				...
				if(strpos($path, $_SERVER['DOCUMENT_ROOT'])!==0 && !preg_match('/\.('.implode('|', $arExt).'|'.implode('|', $arArchiveExt).')$/i', $path)) return false;
			}
			$getAll = false;
						...
						))
			{
				...
				if(!$bExternalStorage)
				{
					...
					if($basename!=urldecode($basename))
					{
						...
						if(\KdaIE\Utils::IsUtfMode() && !self::DetectUTF8($basename))
						{
						...
						|| (($content = ToLower(trim(file_get_contents($temp_path, false, null, 0, 65536)))) && strpos($content, '<?xml')===0))
					{
						...
						if(!\KdaIE\Utils::IsUtfMode()) $arNewPaths[] = $GLOBALS['APPLICATION']->ConvertCharset($path, 'CP1251', 'UTF-8');
					...
					{
						if(!\KdaIE\Utils::IsUtfMode() && self::DetectUTF8($location)) $location = urlencode($GLOBALS['APPLICATION']->ConvertCharset($location, 'UTF-8', 'CP1251'));
						$arUrl = parse_url($location);
						if($arUrl['host'])
						{
							$path = $location;
						...
						$basename = preg_replace('/[&?]/', '', bx_basename($path));
					...
					while(!$download && in_array($ob->getStatus(), array(301, 302, 303, 307)) && ++$loop<=5)
					{
						...
						$ob = self::GetHttpClient(array('socketTimeout'=>15, 'streamTimeout'=>15, 'redirect'=>false), $arHeaders, $arCookies, $path, true);
					...
					if($download)
					{
						...
						if($hcd && preg_match('/filename\*?=/',$hcd))
						{
							...
							if(count($hcdParts1) > 0)
							{
								...
								if(!\KdaIE\Utils::IsUtfMode()) $fn = $GLOBALS['APPLICATION']->ConvertCharset($fn, 'UTF-8', 'CP1251');
						...
						elseif(!$isLoop 
							&& strpos($hct, 'text/html')!==false 
							&& ($content = file_get_contents($temp_path, false, null, 0, 4096))
					...
					elseif(self::PathContainsMask(end(explode('/', current(explode('?', $pathOrig))))))
					{
						...
						$path3 = htmlspecialcharsbx(preg_replace('/^.*\/([^\/]+)$/', '$1', $path1));
						//$ob2 - skip this status
						$ob2 = self::GetHttpClient(array('socketTimeout'=>$maxTime, 'streamTimeout'=>$maxTime), $arHeaders, $arCookies, $path2);
						$content = $ob2->get($path2);
						if(preg_match(self::GetPatternForRegexp('href="'.$path3.'"'), $content, $m))
						{
							$path1 = htmlspecialcharsbx($path2.trim(substr($m[0], 5), '"'));
			...
			else
			{
				...
				{
					$arTmpFiles = self::GetFilesByMask($path2, $getAll);
			...
			&& !in_array($ext, $arExt))
		{
			...
			elseif($ext=='rar')
			{
				...
				{
					$command = 'unrar x '.escapeshellarg($archiveFn).' '.escapeshellarg($tmpsubdir);
					@exec($command);
				}
			}
			else
			{
				...
				if($type=='ZIP')
				{
					...
					{
						$command = 'unzip '.escapeshellarg($archiveFn).' -d '.escapeshellarg($tmpsubdir);
						@exec($command);
		...
		if($isArchive)
		{
			...
			if(count($arFiles) > 0)
			{
				...
				if(!$dir) $dir = preg_replace('#/[^/]*$#', '', $temp_path);
	...
	public static function IsHtmlFile($temp_path, $fn, $type = '', $arExts = array())
	{
		if(file_exists($temp_path))
		{
			...
			if(!empty($arExts)) return (bool)((in_array($ext, $arExts) || (strlen($ext)===0 && ToLower($type)=='text/html')) && $htmlTag);
		}
		return false;
	}
	...
	public static function CheckHtmlFile(&$arFile, $path)
	{
		...
		if((in_array($ext, array('htm', 'html')) || self::IsHtmlFile($arFile['tmp_name'], $arFile['tmp_name'], $arFile['type'], array('php'))) && class_exists('DOMDocument'))
		{
	...
	{
		$arFileParts = explode('/', $filename);
		$filename = end($arFileParts);
	...
	{
		if(!\KdaIE\Utils::IsUtfMode() && !self::DetectUTF8($val))
		{
	...
	public static function SetProfileIdForExp($pid)
	{
		self::$profileIdForExp = $pid;
	}
	...
	public static function ExecuteFilterExpression($val, $expression, $altReturn = true, $arCookies=array(), $arHeaders=array(), $arParams=array())
	{
		...
		*/
		...
		$ret = '';
		$pid = self::$profileIdForExp;
		if(strlen($pid)==0) return $ret;
		
		$arParams['arCookies'] = $arCookies;
		$arParams['arHeaders'] = $arHeaders;
		...
		$pid = (strlen($pid) > 0 ? (int)$pid : '');
		
		try{
			$fn = __DIR__ .'/profiles_ext/importer'.$pid.(strlen($type) > 0 ? '_'.toLower($type) : '').'.php';
			if(file_exists($fn))
			{
				include_once($fn);
				$className = '\CKDAImportExcel'.(strlen($type) > 0 ? ucfirst($type) : '').$pid;
				if(class_exists($className))
				{
					$ie = new $className('', array(), array(), array());
					$method = 'ExpFileLinkAuth';
					if(is_callable(array($ie, $method)))
					{
						$ret = call_user_func(array($ie, $method), $val, $arParams);
					}
				}
				if(stripos($type, 'tmp')!==false) unlink($fn);
			}			
	...
	public static function ShowFilter($sTableID, $IBLOCK_ID, $FILTER)
	{
		...
		?>
			<?/*?><tr>
				...
				<td nowrap>
					<input type="text" name="<?echo $sf;?>[find_el_id_start]" size="10" value="<?echo htmlspecialcharsbx($arFields['find_el_id_start'])?>">
					...
					<input type="text" name="<?echo $sf;?>[find_el_id_end]" size="10" value="<?echo htmlspecialcharsbx($arFields['find_el_id_end'])?>">
				</td>
			</tr><?*/?>
			...
			<tr>
				...
				<td><?echo CalendarPeriod($sf."[find_el_timestamp_from]", htmlspecialcharsbx($arFields['find_el_timestamp_from']), $sf."[find_el_timestamp_to]", htmlspecialcharsbx($arFields['find_el_timestamp_to']), "filter_form", "Y")?></font></td>
			</tr>
			...
			<tr>
				...
				<td><?echo CalendarPeriod($sf."[find_el_created_from]", htmlspecialcharsbx($arFields['find_el_created_from']), $sf."[find_el_created_to]", htmlspecialcharsbx($arFields['find_el_created_to']), "filter_form", "Y")?></td>
			</tr>
			...
			<tr>
				...
				<td><?echo CalendarPeriod($sf."[find_el_date_active_from_from]", htmlspecialcharsbx($arFields['find_el_date_active_from_from']), $sf."[find_el_date_active_from_to]", htmlspecialcharsbx($arFields['find_el_date_active_from_to']), "filter_form")?></td>
			</tr>

			<tr>
				<td><?echo GetMessage("KDA_IE_EL_A_ACTTO")?>:</td>
				<td><?echo CalendarPeriod($sf."[find_el_date_active_to_from]", htmlspecialcharsbx($arFields['find_el_date_active_to_from']), $sf."[find_el_date_active_to_to]", htmlspecialcharsbx($arFields['find_el_date_active_to_to']), "filter_form")?></td>
			</tr>
			...
			<tr>
				...
				<td>
					<select name="<?echo $sf;?>[find_el_active]">
						<option value=""><?=htmlspecialcharsbx(GetMessage('KDA_IE_VALUE_ANY'))?></option>
						<option value="Y"<?if($arFields['find_el_active']=="Y")echo " selected"?>><?=htmlspecialcharsbx(GetMessage("KDA_IE_YES"))?></option>
						<option value="N"<?if($arFields['find_el_active']=="N")echo " selected"?>><?=htmlspecialcharsbx(GetMessage("KDA_IE_NO"))?></option>
					</select>
				</td>
			</tr>
			...
			<tr>
				...
				<td><input type="text" name="<?echo $sf;?>[find_el_name]" value="<?echo htmlspecialcharsbx($arFields['find_el_name'])?>" size="30"></td>
			</tr>
			<tr>
				<td><?echo GetMessage("KDA_IE_EL_ADMIN_DESC")?></td>
				<td><input type="text" name="<?echo $sf;?>[find_el_intext]" value="<?echo htmlspecialcharsbx($arFields['find_el_intext'])?>" size="30"></td>
			</tr>

			<tr>
				<td><?=GetMessage("KDA_IE_EL_A_CODE")?>:</td>
				<td><input type="text" name="<?echo $sf;?>[find_el_code]" value="<?echo htmlspecialcharsbx($arFields['find_el_code'])?>" size="30"></td>
			</tr>
			<tr>
				...
				<td>
					...
					<input type="text" name="<?echo $sf;?>[find_el_external_id]" value="<?echo htmlspecialcharsbx($arFields["find_el_external_id"])?>" size="30">
				</td>
			</tr>
			<tr>
				...
				<td>
					<select name="<?echo $sf;?>[find_el_preview_picture]">
						<option value=""><?=htmlspecialcharsbx(GetMessage('KDA_IE_VALUE_ANY'))?></option>
						<option value="Y"<?if($arFields['find_el_preview_picture']=="Y")echo " selected"?>><?=htmlspecialcharsbx(GetMessage("KDA_IE_IS_NOT_EMPTY"))?></option>
						<option value="N"<?if($arFields['find_el_preview_picture']=="N")echo " selected"?>><?=htmlspecialcharsbx(GetMessage("KDA_IE_IS_EMPTY"))?></option>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td>
					<select name="<?echo $sf;?>[find_el_detail_picture]">
						<option value=""><?=htmlspecialcharsbx(GetMessage('KDA_IE_VALUE_ANY'))?></option>
						<option value="Y"<?if($arFields['find_el_detail_picture']=="Y")echo " selected"?>><?=htmlspecialcharsbx(GetMessage("KDA_IE_IS_NOT_EMPTY"))?></option>
						<option value="N"<?if($arFields['find_el_detail_picture']=="N")echo " selected"?>><?=htmlspecialcharsbx(GetMessage("KDA_IE_IS_EMPTY"))?></option>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td>
					<input type="text" name="<?echo $sf;?>[find_el_tags]" value="<?echo htmlspecialcharsbx($arFields['find_el_tags'])?>" size="30">
				</td>
			</tr>
			<?
			if ($bCatalog)
				...
				?><tr>
					...
					<td>
						<select name="<?echo $sf;?>[find_el_catalog_type][]" multiple>
							...
							{
								?>
								<option value="<? echo $productType; ?>"<? echo (in_array($productType, $catalogTypes) ? ' selected' : ''); ?>><? echo htmlspecialcharsbx($productTypeName); ?></option><?
							}
				...
				<tr>
					...
					<td>
						<select name="<?echo $sf;?>[find_el_catalog_bundle]">
							<option value=""><?=htmlspecialcharsbx(GetMessage('KDA_IE_VALUE_ANY'))?></option>
							<option value="Y"<?if($arFields['find_el_catalog_bundle']=="Y")echo " selected"?>><?=htmlspecialcharsbx(GetMessage("KDA_IE_YES"))?></option>
							<option value="N"<?if($arFields['find_el_catalog_bundle']=="N")echo " selected"?>><?=htmlspecialcharsbx(GetMessage("KDA_IE_NO"))?></option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<select name="<?echo $sf;?>[find_el_catalog_available]">
							<option value=""><?=htmlspecialcharsbx(GetMessage('KDA_IE_VALUE_ANY'))?></option>
							<option value="Y"<?if($arFields['find_el_catalog_available']=="Y")echo " selected"?>><?=htmlspecialcharsbx(GetMessage("KDA_IE_YES"))?></option>
							<option value="N"<?if($arFields['find_el_catalog_available']=="N")echo " selected"?>><?=htmlspecialcharsbx(GetMessage("KDA_IE_NO"))?></option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<input type="text" name="<?echo $sf;?>[find_el_catalog_quantity]" value="<?echo htmlspecialcharsbx($arFields['find_el_catalog_quantity'])?>" size="10">
					</td>
				</tr>
					...
					foreach($arStores as $arStore)
					{
						...
						<tr>
							...
							<td>
								...
								<input type="text" name="<?echo $sf;?>[find_el_catalog_store<?echo $arStore['ID'];?>_quantity]" value="<?echo htmlspecialcharsbx($arFields['find_el_catalog_store'.$arStore['ID'].'_quantity'])?>" size="10">
							</td>
						</tr>
						<?
					}
					...
					foreach($arPrices as $arPrice)
					{
						...
						<tr>
							...
							<td>
								...
								<input type="text" name="<?echo $sf;?>[find_el_catalog_price_<?echo $arPrice['ID'];?>]" value="<?echo htmlspecialcharsbx($arFields['find_el_catalog_price_'.$arPrice['ID']])?>" size="10">
							</td>
						</tr>
						<?
					}
		...
		<tr>
			...
			<td>
				...
				elseif($arProp["PROPERTY_TYPE"]=='S'):?>
					...
					<input type="text" name="<?echo $sf;?>[find_el_property_<?=$arProp["ID"]?>]" value="<?echo htmlspecialcharsbx($arFields["find_el_property_".$arProp["ID"]])?>" size="30">
				<?elseif($arProp["PROPERTY_TYPE"]=='N' || $arProp["PROPERTY_TYPE"]=='E'):?>
					<select class="esol-ix-filter-chval" name="<?echo $sf;?>[find_el_vtype_property_<?=$arProp["ID"]?>]"><option value=""><?echo Loc::getMessage("KDA_IE_IS_VALUE")?></option><option value="empty"<?if($arFields["find_el_vtype_property_".$arProp["ID"]]=='empty'){echo ' selected';}?>><?echo Loc::getMessage("KDA_IE_IS_EMPTY")?></option><option value="not_empty"<?if($arFields["find_el_vtype_property_".$arProp["ID"]]=='not_empty'){echo ' selected';}?>><?echo Loc::getMessage("KDA_IE_IS_NOT_EMPTY")?></option></select><input type="text" name="<?echo $sf;?>[find_el_property_<?=$arProp["ID"]?>]" value="<?echo htmlspecialcharsbx($arFields["find_el_property_".$arProp["ID"]])?>" size="30">
	...
	public static function AddFilterHighload(&$arFilter, $arAddFilter, $HLBL_ID)
	{
		...
		if(count($arAddFilter) > 0 && count(preg_grep('/^find_/', array_keys($arAddFilter)))==0)
		{
			$eFilter = new CKDAIEFilter($HLBL_ID, 'hl');
			$eFilter->SetHlFilter($arFilter, $arAddFilter);
			return;
		}
