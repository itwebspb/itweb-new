...
class CKDAImportExcelHighload extends CKDAImportExcelHighloadData {	
	function __construct($filename='', $params=array(), $fparams=array(), $stepparams=array(), $pid = false)
	{
		...
		if(!\CKDAImportProfile::getInstance('highload')->CheckExpClassFile($pid, get_class($this)))
		{
	...
	public function SaveRecord($arItem, $isPacket=false)
	{
		...
		foreach($filedList as $key=>$field)
		{
			...
			{
				$value = $this->ApplyConversions($value, $conversions, $arItem, array('KEY'=>$k, 'FIELDKEY'=>$key, 'NAME'=>$field));
				$origValue = $this->ApplyConversions($origValue, $conversions, $arItem, array('KEY'=>$k, 'FIELDKEY'=>$key, 'NAME'=>$field));
		...
		if(is_array($arConv))
		{
			...
			foreach($arConv as $k=>$v)
			{
					...
					|| ($v['WHEN']=='ANY'))
				{
					...
					elseif($v['THEN']=='EXPRESSION') $val = $this->ExecuteFilterExpression($val, $v['TO'], '', array(), $field, $k);
	...
	public function CheckRegexpCond($from, $cond)
	{
		...
		$res = preg_match('/'.$this->GetRegexpProtectParam(ToLOwer($from)).'/i'.CKDAImportUtils::getUtfModifier(), ToLower($cond));		
	...
	public function GetConvReplaceRegexp($val, $from, $to, $substr = true)
	{
		...
		if($substr===false)
		{
			...
			if(preg_match('/'.$safeFrom.'/i'.$mod, $val, $m)) $val = $m[0];
