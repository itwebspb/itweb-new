...
class CKDAImportLogger {
	...
	public function AddElementChanges($type, $arFields, $arOldFields=array())
	{
		...
		if(is_array($arFields))
		{
			foreach($arFields as $k=>$v)
			{
				$key = $type.$k;
				$this->elemFields[$key] = array('VALUE' => $v);
				if(isset($arOldFields[$k])) $this->elemFields[$key]['OLDVALUE'] = (is_object($arOldFields[$k]) ? (is_callable(array($arOldFields[$k], 'toString')) ? $arOldFields[$k]->toString() : ''): $arOldFields[$k]);
			}
		}
	}
		...
		if(is_array($arFields))
		{
			foreach($arFields as $k=>$v)
			{
				...
				$this->sectionFields[$key] = array(
					'OLDVALUE' => (isset($arOldFields[$k]) ? (is_object($arOldFields[$k]) ? (is_callable(array($arOldFields[$k], 'toString')) ? $arOldFields[$k]->toString() : ''): $arOldFields[$k]) : ''),
					'VALUE' => $v
				);
			}
		}
	...
	public function GetElementDescription($description, $excel=false)
	{
		...
		$arFields = (!is_array($description) ? \KdaIE\Utils::Unserialize(htmlspecialcharsback($description), array('Bitrix\Main\Type\DateTime', 'Bitrix\Main\Type\Date', 'Date', 'DateTime')) : $description);
