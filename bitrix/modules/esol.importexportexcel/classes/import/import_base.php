...
class CKDAImportExcelBase {
	...
	var $baseElemFields = array('ID', 'NAME', 'IBLOCK_SECTION_ID', 'MODIFIED_BY', 'PREVIEW_PICTURE', 'DETAIL_PICTURE', 'WF_STATUS_ID', 'ACTIVE');
	...
	public function GetFileArrayDirect($file, $arParams=array(), $fieldName='', $oldId=0)
	{
		...
		elseif(strpos($file, '/')===0 || (strpos($file, '://')===false && strpos($file, '/')!==false))
		{
			...
			else
			{
				...
				if(strlen($arFile['tmp_name'])==0 && strpos($file, '#')!==false) $arFile = self::MakeFileArray($file);
		...
		elseif(in_array($arFile['type'], array('application/zip', 'application/x-zip-compressed')) && $extractZip)
		{
			...
			if(!$archiveParams['exists'])
			{
				...
				{
					$command = 'unzip '.escapeshellarg($arFile['tmp_name']).' -d '.escapeshellarg($archiveParams['path']);
					@exec($command);
		...
		if(array_key_exists('name', $arFile))
		{
			...
			{
				if(\KdaIE\Utils::IsUtfMode() && $io->ValidateFilenameString(\CKDAImportUtils::Win1251Utf8($arFile['name']))) $arFile['name'] = \CKDAImportUtils::Win1251Utf8($arFile['name']);
			}
		}
	...
	public static function ResizePicture($arFile, $arResize)
	{
		...
		if(($width > 0 && $orig[0] > $width) || ($height > 0 && $orig[1] > $height)
			|| ($arResize['FILLING']==='Y' && $width > 0 && $height > 0 && ($orig[0] < $width || $orig[1] < $height)))
		{
		...
		if(is_array($arConv))
		{
			...
			foreach($arConv as $k=>$v)
			{
					...
					|| ($v['WHEN']=='ANY'))
				{
					...
					elseif($v['THEN']=='EXPRESSION') $val = $this->ExecuteFilterExpression($val, $v['TO'], false, $arExpParams, $field, $k);
	...
	public function CalcFloatValuePhp($val)
	{
		...
		while(preg_match_all('/(^|\()([^\(\)]+)\?([^\(\)]+):([^\(\)]+)(\)|$)/', $val, $m))
		{
			foreach($m[0] as $k=>$v)
			{
				$cond = $m[2][$k];
				$vKey = 3;
				if(preg_match('/^(.+)(!=+|==+|<=*|>=*)(.+)$/', $cond, $m2))
				{
					$comp = substr($m2[2], 0, 2);
					if($comp=='>') $vKey = ($this->CalcFloatValue($m2[1], true) > $this->CalcFloatValue($m2[3], true) ? 3 : 4);
					elseif($comp=='>=') $vKey = ($this->CalcFloatValue($m2[1], true) >= $this->CalcFloatValue($m2[3], true) ? 3 : 4);
					elseif($comp=='<') $vKey = ($this->CalcFloatValue($m2[1], true) < $this->CalcFloatValue($m2[3], true) ? 3 : 4);
					elseif($comp=='<=') $vKey = ($this->CalcFloatValue($m2[1], true) <= $this->CalcFloatValue($m2[3], true) ? 3 : 4);
					elseif($comp=='==' || $comp=='!=')
					{
						$v1 = $m2[1];
						$v2 = $m2[3];
						if(preg_match('/[^\d\.,+\-\/*%\(\)]/', $v1)) $v1 = $this->CalcFloatValue($v1);
						if(preg_match('/[^\d\.,+\-\/*%\(\)]/', $v2)) $v2 = $this->CalcFloatValue($v2);
						$vKey = ((($comp=='==' && $v1 == $v2) || ($comp=='!=' && $v1 != $v2)) ? 3 : 4);
					}
				}
				$val = str_replace($v, trim($m[$vKey][$k]), $val);
			}
		}
		if(preg_match('/[+\-\/*]/', $val))
		{
