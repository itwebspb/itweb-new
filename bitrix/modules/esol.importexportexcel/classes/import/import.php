...
class CKDAImportExcel extends CKDAImportExcelData {	
	function __construct($filename='', $params=array(), $fparams=array(), $stepparams=array(), $pid = false)
	{
		...
		if((!isset($this->params['ELEMENT_IMAGES_FORCE_UPDATE']) || $this->params['ELEMENT_IMAGES_FORCE_UPDATE']!='Y') && (isset($this->params['CHECK_CHANGES']) && $this->params['CHECK_CHANGES']=='N'))
		{
			$this->params['ELEMENT_IMAGES_FORCE_UPDATE'] = 'Y';
		}
		
		if(!\CKDAImportProfile::getInstance()->CheckExpClassFile($pid, get_class($this)))
		{
			$this->expClassFileError = true;
			return;
		}
		
		if(isset($this->params['PACKET_IMPORT']) && $this->params['PACKET_IMPORT']=='Y')
		{
			$this->isPacket = true;
		...
		if(!isset($stepparams['NOT_CHANGE_PROFILE']) || $stepparams['NOT_CHANGE_PROFILE']!='Y')
		{
			...
			if($pid!==false) $this->logger->SetExecId($this->stepparams['loggerExecId']);
		}
		...
		if(isset($this->stepparams['arTitles']) && is_array($this->stepparams['arTitles'])) $this->arTitles = $this->stepparams['arTitles'];
		...
		if(isset($this->params['ONLY_UPDATE_MODE']) && $this->params['ONLY_UPDATE_MODE']=='Y')
		{
			$this->params['ONLY_UPDATE_MODE_ELEMENT'] = $this->params['ONLY_UPDATE_MODE_SECTION'] = 'Y';
		}
		if(isset($this->params['ONLY_UPDATE_MODE_SEP']) && $this->params['ONLY_UPDATE_MODE_SEP']!='Y')
		{
			$this->params['ONLY_UPDATE_MODE_PRODUCT'] = $this->params['ONLY_UPDATE_MODE_OFFER'] = $this->params['ONLY_UPDATE_MODE_ELEMENT'];
		}
		if(isset($this->params['ONLY_CREATE_MODE']) && $this->params['ONLY_CREATE_MODE']=='Y')
		{
			$this->params['ONLY_CREATE_MODE_ELEMENT'] = $this->params['ONLY_CREATE_MODE_SECTION'] = 'Y';
		}
		if(isset($this->params['ONLY_CREATE_MODE_SEP']) && $this->params['ONLY_CREATE_MODE_SEP']!='Y')
		{
	...
	public function Import()
	{
		...
		{
			$isCron = ($this->stepparams['IMPORT_MODE']=='CRON');
			$this->errors[] = sprintf(Loc::getMessage("KDA_IE_ERROR_EXP_CLASS_FILE"), '/bitrix/admin/settings.php?lang=ru&mid='.static::$moduleId.'&mid_menu=1#profilesconv');
			if($isCron)
			{
				\KdaIE\Utils::AddNotifyConvertProfiles('error');
				return $this->GetBreakParams('finish_error');
			}
			elseif(isset($this->stepparams['finishstatus']) && $this->stepparams['finishstatus']=='Y')
			{
				$this->errors = array();
	...
	public function EndOfLoading($time)
	{
		...
		if(($bElemDeactivate || $bSetDefaultProps || $bSetDefaultProps2) && !isset($this->stepparams['break_import']))
		{
	...
	public function GetBreakParams($action = 'continue')
	{
		...
		unset($arStepParams['params']['arTitles']);
		return $arStepParams;
	}
	...
	public function SetWorksheet($worksheetNum, $worksheetCurrentRow)
	{
		...
		$oldWorksheet = (bool)($this->worksheet!==false);
		$this->worksheet = $this->worksheetIterator->current();
		if($this->worksheet===false)
		{
			if($oldWorksheet===false) $this->stepparams['break_import'] = true;
			return false;
		}
		if($getCount)
		{
	...
	public function CheckSkipLine($currentRow, $worksheetNum, $checkValue = true)
	{
		...
		if($this->worksheet && !$this->worksheet->getRowDimension(intval($currentRow))->getVisible() && (int)$this->worksheet->getRowDimension(intval($currentRow))->getOutlineLevel()<=0) $load = false;
		...
		{
			if($this->titlesRow!==false && $this->titlesRow==($currentRow - 1))
			{
				$arItem = array();
				foreach($this->GetNeedFileColumns() as $column)
				{
					$val = $this->worksheet->getCellByColumnAndRow($column, $currentRow);	
					$arItem[$column] = $this->Trim($this->GetCalculatedValue($val));
				}
				$this->arTitles = $arItem;
			}
			$load = false;
		}
		...
		if($load && $checkValue && is_array($this->fparams[$worksheetNum]))
		{
			foreach($this->fparams[$worksheetNum] as $k=>$v)
			{
				...
				if(is_array($v['UPLOAD_VALUES']) || is_array($v['NOT_UPLOAD_VALUES']) || $v['FILTER_EXPRESSION'])
				{
					...
					$val = $this->ApplyConversions($valOrig, $v['CONVERSION'], array(), array('FIELDKEY'=>$k, 'SHEETKEY'=>$worksheetNum));
				...
				{
					$load = ($load && $this->ExecuteFilterExpression($valOrig, $v['FILTER_EXPRESSION'], false, array(), array('FIELDKEY'=>$k), '', 'filter'));
				}
			}
		}
	...
	public function ExecuteFilterExpression($val, $expression, $altReturn = true, $arParams = array(), $field = false, $convKey = '', $expType='')
	{
		$sheetKey = (is_array($field) && array_key_exists('SHEETKEY', $field) ? $field['SHEETKEY'] : $this->worksheetNumForSave);
		...
		$ret = '';
		try{
			if($expType=='filter') $method = 'ExpSheet'.$sheetKey.'Field'.$fieldKey.'FilterExpression';
			elseif($expType=='extra') $method = 'ExpSheet'.$sheetKey.'Field'.$fieldKey.'ExtraConv'.$convKey;
			else $method = 'ExpSheet'.$sheetKey.'Field'.$fieldKey.'Conv'.$convKey;
	...
	{
		try{
			$method = 'ExpOnAfterSave';
			if(is_callable(array($this, $method)))
			{
				call_user_func(array($this, $method), $ID);
			}
		}catch(Exception $ex){}
	}
	...
	public function SaveRecord($arItem, $isPacket=false)
	{
		...
		foreach($filedList as $key=>$field)
		{
			...
			if(!empty($conversions))
			{
				...
				$value = $this->ApplyConversions($value, $conversions, $arItem, array('KEY'=>$k, 'FIELDKEY'=>$key, 'NAME'=>$field), $iblockFields);
				if($eqValues) $origValue = $value;
				else $origValue = $this->ApplyConversions($origValue, $conversions, $arItem, array('KEY'=>$k, 'FIELDKEY'=>$key, 'NAME'=>$field), $iblockFields);
		...
		foreach($arUid as $v)
		{
			if(!$v['substring'])
			{
				...
				else 
				{
					...
					if(!\KdaIE\Utils::IsUtfMode() && strpos($v['valUid'], "\xA0")!==false) $arSubfilter[] = str_replace("\xA0", ' ', $v['valUid']);
				}
	...
	public function SaveSKUWithGenerate($ID, $NAME, $IBLOCK_ID, $arItem)
	{
		...
		if(!empty($this->fieldsForSkuGen))
		{
			...
			foreach($this->fieldsForSkuGen as $key)
			{
				...
				$arItem[$key] = $this->ApplyConversions($arItem[$key], $conversions, $arItem, array('PARENT_ID'=>$ID, 'FIELDKEY'=>$key));
			...
			foreach($filedList as $key=>$field)
			{
				...
				$val = $this->ApplyConversions($valOrig, $conversions, $arItem, array('PARENT_ID'=>$ID, 'FIELDKEY'=>$key));
	...
	public function SaveSKU($ID, $NAME, $IBLOCK_ID, $arItem, $convertedFields=array())
	{
		...
		foreach($filedList as $key=>$field)
		{
			...
			{
				$value = $this->ApplyConversions($value, $conversions, $arItem, array('KEY'=>$k, 'FIELDKEY'=>$key, 'NAME'=>$field, 'PARENT_ID'=>$ID), $iblockFields);
				$origValue = $this->ApplyConversions($origValue, $conversions, $arItem, array('KEY'=>$k, 'FIELDKEY'=>$key, 'NAME'=>$field, 'PARENT_ID'=>$ID), $iblockFields);
				if($value===false) continue;
			}
	...
	public function GetStyleCellValue($arItem, $level)
	{
		...
		if(!empty($conversions))
		{
			$sectName = $this->ApplyConversions($sectName, $conversions, $arItem, array('FIELDKEY'=>'__'.$level));
		}
		return $sectName;
	}
	...
	public function SetSectionSeparate($arItem, $IBLOCK_ID, $SECTION_ID, $level)
	{
		...
		foreach($filedList as $key=>$field)
		{
			...
			{
				$value = $this->ApplyConversions($value, $conversions, $arItem, array('KEY'=>$k, 'FIELDKEY'=>$key, 'NAME'=>$field), $iblockFields);
				$origValue = $this->ApplyConversions($origValue, $conversions, $arItem, array('KEY'=>$k, 'FIELDKEY'=>$key, 'NAME'=>$field), $iblockFields);
				if($value===false) continue;
			}
	...
	public function SaveProperties($ID, $IBLOCK_ID, $arProps, $arOldVals=array(), $needUpdate = false, $arFieldsElement=array())
	{
		...
		{
			if(!is_array($arProps[$k])) $this->PropReplaceId($arProps[$k]);
			else array_walk_recursive($arProps[$k], array($this, 'PropReplaceId'));
			if(!is_numeric($k)) continue;
			if(($propsDef[$k]['USER_TYPE']=='directory' || $propsDef[$k]['PROPERTY_TYPE']=='L') && $propsDef[$k]['MULTIPLE']=='Y' && is_array($arProps[$k]))
			{
				$newProp = array();
		...
		if(!empty($arProps))
		{
			...
			foreach($arOldVals as $arrKey=>$arr)
			{
				...
				{
					$propVal = $this->GetPropOldValue($propsDef[$arr['ID']], $arr['VALUE']);
	...
	public function GetPropOldValue($arProp, $val)
	{
		if(is_array($val))
		{
			if($arProp['PROPERTY_TYPE']=='S' && $arProp['USER_TYPE']=='C')
			{
				if(!empty($val) && !array_key_exists('VALUE', $val)) $val = array('VALUE' => $val);
			}
			else $val = serialize($val);
		}
		return $val;
	}
	...
	public function GetCurrencyRates()
	{
		if(!isset($this->currencyRates))
		{
			...
			if(!isset($arRates['TIME']) || $arRates['TIME'] < time() - 6*60*60)
			{
				...
				if($res)
				{
					$xml = \KdaIE\Utils::GetSimplXmlFromString($res);
					if($xml->Valute)
					{
	...
	public static function CorrectCalculatedValue($val, $encoding='UTF-8')
	{
		...
		if(!\KdaIE\Utils::IsUtfMode() && ($encoding=='UTF-8' || \CUtil::DetectUTF8($val)))
		{
