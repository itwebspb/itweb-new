...
class ZipArchive
{
	...
	public static function RemoveFileDir($dir)
	{
		...
		elseif(is_numeric($dir)) $dir = $_SERVER["DOCUMENT_ROOT"].'/upload/tmp/'.\KdaIE\Utils::GetModuleId().'/'.\KdaIE\Utils::GetModuleSubDir().'_archives/'.$dir.'/';
		if($dir && is_dir($dir))
		{
	...
	{
		if(($pos = mb_strpos($pFilename, '/'.\KdaIE\Utils::GetModuleId().'/'))!==false)
		{
			...
			if(strlen($fileName) > 0 && strlen($subDir) > 0)
			{
				...
				{
					return $_SERVER["DOCUMENT_ROOT"].'/upload/tmp/'.\KdaIE\Utils::GetModuleId().'/'.\KdaIE\Utils::GetModuleSubDir().'_archives/'.self::$arFileIds[$fileKey].'/';
				}
			}
		}
		return false;
	...
	public function open($pFilename)
	{
		...
		else
		{
			...
			elseif(($arFile = \CFile::MakeFileArray($pFilename)) && in_array($arFile['type'], array('application/zip', 'application/x-zip-compressed', 'application/octet-stream', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')))
			{
				...
				{
					$command = 'unzip '.escapeshellarg($pFilename).' -d '.escapeshellarg($this->tmpDir);
					@exec($command);
				}
	...
	public function getSharedStringsFromIndexes($reader)
	{
		...
		while(++$ind==0 || $xml->next('si'))
		{
			...
			$val = \KdaIE\Utils::GetSimplXmlFromString($xml->readOuterXml());
