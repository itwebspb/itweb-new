...
class IblockElementTable extends Entity\DataManager
{
	...
	{
		if(isset($arParams['ELEM_API_OPTIMIZE']) && $arParams['ELEM_API_OPTIMIZE']=='Y') $this->optimizeApi = true;
	}
				...
				if($arIblock['WORKFLOW']!='Y')
				{
					...
					elseif(\Bitrix\KdaImportexcel\DataManager\ElementPropertyTable::issetValueIndex() && count(preg_grep('/^(IBLOCK_ID|CHECK_PERMISSIONS|[=%]PROPERTY_\d+|[=%]PROPERTY_\d+_VALUE)$/', $arFilterKeys))==count($arFilterKeys) && is_array($arIblock) && $limit===false)
					{
						if($arIblock['VERSION']==1)
						{
							...
							if(!isset(self::$arIblockClasses[$iblockId]) || !class_exists('\Bitrix\KdaImportexcel\DataManager\ElementProperty'.$arFilter['IBLOCK_ID'].'Table'))
							{
								$command = 'namespace Bitrix\KdaImportexcel\DataManager;'."\r\n".
									'class ElementProperty'.$iblockId.'Table extends ElementPropertyTable{'."\r\n".
										'public static function getMap(){return parent::getMapForIblock('.$iblockId.');}'.
									'}';
								eval($command);
								self::$arIblockClasses[$iblockId] = $iblockId;
							}
