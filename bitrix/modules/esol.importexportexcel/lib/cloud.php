...
class Cloud
{
	...
	{
		$token = \Bitrix\Main\Config\Option::get(\KdaIE\Utils::GetModuleId(), 'YANDEX_APIKEY', '');
		...
		if(strlen($fragment) > 0)
		{
			...
			$client = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>15, 'useCurl'=>self::GetUseCurl()));
	...
	{
		$token = \Bitrix\Main\Config\Option::get(\KdaIE\Utils::GetModuleId(), 'YANDEX_APIKEY', '');
		if(!$token) return array('ERROR_MESSAGE'=>sprintf(Loc::getMessage("KDA_IE_YANDEX_APIKEY_NOT_DEFINED"), '/bitrix/admin/settings.php?lang=ru&mid_menu=1&mid='.\KdaIE\Utils::GetModuleId().'#yandex_token'));
		$origPath = $path;
		...
		{
			$client = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>10, 'useCurl'=>self::GetUseCurl()));
			$client->setHeader('Authorization', "OAuth ".$token);
	...
	public function YadiskGetFileByYaLink(&$tmpPath, $fileLink)
	{
		...
		if(strlen($fileLink) > 0)
		{
			...
			if(count($filename)==1)
			{
				...
				if(!\KdaIE\Utils::IsUtfMode()) $filename = $GLOBALS['APPLICATION']->ConvertCharset($filename, 'UTF-8', 'CP1251');
	...
	{
		$client = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>$this->maxTime, 'streamTimeout'=>$this->maxTime));
	...
	{
		$refreshToken = \Bitrix\Main\Config\Option::get(\KdaIE\Utils::GetModuleId(), 'GOOGLE_APIKEY', '');
		$accessToken = \Bitrix\Main\Config\Option::get(\KdaIE\Utils::GetModuleId(), 'GOOGLE_ACCESS_TOKEN', '');
		...
		if($accessToken)
		{
			...
			{
				$accessToken = '';
				\Bitrix\Main\Config\Option::set(\KdaIE\Utils::GetModuleId(), 'GOOGLE_ACCESS_TOKEN', $accessToken);
			}
		}
	...
	public function DropboxGetFile(&$tmpPath, $path, $fromFile=false)
	{
		...
		$client = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>30, 'redirect'=>false));
	...
	public function Bitrix24GetFile(&$tmpPath, $path, $fromFile=false)
	{
		...
		{
			$arUrl = parse_url($loc);
			$client = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>10, 'redirect'=>false));
