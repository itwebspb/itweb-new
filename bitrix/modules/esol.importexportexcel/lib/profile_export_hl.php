...
class ProfileHlTable extends Entity\DataManager
{
	...
	public static function Add(array $arFields)
	{
		...
		if($result->isSuccess())
		{
			self::Update($result->getId(), array('PARAMS'=>$params));
		}
	...
	{
		$arOldFields = self::getList(array('filter'=>array('ID'=>$ID)))->fetch();
		\CKDAExportProfile::PrepareProfileFields($arFields, is_array($arOldFields) ? $arOldFields : array(), $ID - 1, 'hl');
