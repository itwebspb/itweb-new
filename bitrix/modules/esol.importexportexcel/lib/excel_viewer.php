...
class ExcelViewer 
{
	...
	public function __construct($DATA_FILE_NAME='', $SETTINGS_DEFAULT=array(), $PROFILE_ID=false)
	{
	...
	public function GetColumnVals($listNumber, $colNumber, $useConv, $arFieldParams=array(), $arProfileParams=array())
	{
		if($useConv)
		{
			$pid = $this->profileId;
			$arExtraOrig = array();
			\CKDAImportExtrasettings::HandleParams($arExtraOrig, $arFieldParams);
			$arExtra = $arExtraOrig;
			$arKeys = array();
			for($i=0; $i<2; $i++)
			{
			...
			if($isExpressions)
			{
				...
				elseif($GLOBALS['USER']->IsAdmin() && strlen($pid) > 0)
				{
					$arTmpParams = array('PARAMS'=>\CKDAImportProfile::EncodeProfileParams(array('EXTRASETTINGS'=>$arExtraOrig)));
					\CKDAImportProfile::PrepareProfileFields($arTmpParams, array(), $pid, 'tmp');
				}
			}

			$importFile = substr($this->filename, strlen($_SERVER['DOCUMENT_ROOT']));
			$fn = __DIR__ .'/../classes/'.(strpos(\KdaIE\Utils::GetModuleId(), 'kda.')===0 ? 'general' : 'import').'/profiles_ext/importer'.(int)$pid.'_tmp.php';
			if($isExpressions && strlen($pid) > 0  && file_exists($fn))
			{
				$arTmpErrors = \CKDAImportProfile::getInstance()->GetErrorsOnIncludeExpClass($pid, 'tmp');
				if(count($arTmpErrors) > 0)
				{
					unlink($fn);
					return $arTmpErrors;
				}
				...
				include_once($fn);
				$ie = new ('\CKDAImportExcelTmp'.(int)$pid)($importFile, $arProfileParams, array(), array());
			}
			else $ie = new \CKDAImportExcel($importFile, $arProfileParams, array(), array());
			...
			if(is_array($rows))
			{
				foreach($rows as $k=>$row)
				{
					...
					$val = $ie->ApplyConversions($val, $arConv, array(), (count($arKeys)==2 ? array_combine(array('SHEETKEY', 'FIELDKEY'), $arKeys) : array()));
