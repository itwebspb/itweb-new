...
class Conversion
{
		...
		if(is_array($fs[$fKey]['EXTRA_CONVERSION']))
		{
			...
			foreach($fs[$fKey]['EXTRA_CONVERSION'] as $k=>$v)
			{
				...
				if(($v['CELL']=='ELSE' && !$execConv)
					...
					|| (!in_array($v['CELL'], array('ELSE', 'LOADED', 'DUPLICATE')) 
						&& (($v['WHEN']=='EQ' && $condVal==$v['FROM'])
							...
							|| ($v['WHEN']=='REGEXP' && $this->ie->CheckRegexpCond($v['FROM'], $condVal))
							|| ($v['WHEN']=='NOT_REGEXP' && !$this->ie->CheckRegexpCond($v['FROM'], $condVal))
					...
					))
				{
					...
					elseif($v['THEN']=='EXPRESSION') $val = $this->ExecuteFilterExpression($ID, $val, $v['TO'], false, array('FIELDKEY'=>(isset($fs[$fKey]['FIELD_INDEX']) ? $fs[$fKey]['FIELD_INDEX'] : ''), 'NAME'=>$fKey), $k);
	...
	public function ConversionExtraReplaceValues($m)
	{
		...
		if($isVar)
		{
			$this->ie->convParams[$paramName] = $value;
			return '$this->convParams['.$quot.$paramName.$quot.']';
		}
		elseif($this->convIsExpression) $this->ie->convParams[$paramName] = $value;
		return $value;
	}
	...
	public function GetElementFieldsEx($arFilter, $loadElemFields)
	{
		...
		foreach($loadElemFields as $field)
		{
			if(strpos($field, 'IE_')===0)
			{
				...
				if(strpos($key, 'PREVIEW_PICTURE')===0 || strpos($key, 'DETAIL_PICTURE')===0)
				{
					if(!in_array($key, $arElementRelFields)) $arElementRelFields[] = $key;
					$key = substr($key, 0, strpos($key, '_PICTURE') + 8);
				}
		...
		if($arElement = $dbResElements->Fetch())
		{
			foreach($arElement as $k=>$v)
			{
				if(in_array($k, $arElementFields))
				{
					...
					{
						foreach($arElementRelFields as $v2)
						{
							if(strpos($v2, $k.'_')===0)
							{
								$relField = substr($v2, strlen($k.'_'));
								$arElement2['IE_'.$k.'_'.$relField] = $this->GetFileValue($v, $relField);
							}
						}
						$v = $this->GetFileValue($v);
					}
				...
				while($arProp = $dbRes2->Fetch())
				{
					if(in_array($arProp['ID'], $arPropsFields))
					{
						...
						elseif($arProp['PROPERTY_TYPE']=='F')
						{
							...
							foreach($arRelFieldKeys as $k=>$v)
							{
								$arRelFields[$v] = $this->GetFileValue($val, $v);
							}
	...
	public function GetFileValue($val, $relField='')
	{
		if($val)
		{
			...
			if($arFile)
			{
				if(strlen($relField) > 0) $val = $arFile[$relField];
				else $val = $arFile['SRC'];
			}
			else
			{
	...
	public function ExecuteFilterExpression($ID, $val, $expression, $altReturn = true, $field = false, $convKey = '')
	{
		return $this->ie->ExecuteFilterExpression($val, $expression, $altReturn, array('ID'=>$ID), $field, $convKey, 'extra');
	}
