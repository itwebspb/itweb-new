...
class IUtils
{
	...
	public static function Translate($string, $langFrom, $langTo=false)
	{
		...
		if($apiKey = \Bitrix\Main\Config\Option::get(\KdaIE\Utils::GetModuleId(), 'TRANSLATE_GOOGLE_KEY', ''))
		{
		...
		elseif(($apiKey = \Bitrix\Main\Config\Option::get('main', 'translate_key_yandex', '')) ||
			($apiKey = \Bitrix\Main\Config\Option::get(\KdaIE\Utils::GetModuleId(), 'TRANSLATE_YANDEX_KEY', '')))
		{
			if($langTo===false) $langTo = LANGUAGE_ID;
			$client = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>10));
	...
	public static function Str2Url($string, $arParams=array(), $allowEmpty=true)
	{
		...
		if(count($arParams)==0)
		{
			$arTransParams = \Bitrix\Main\Config\Option::get(\KdaIE\Utils::GetModuleId(), 'TRANS_PARAMS', '');
	...
	{
		$client = new \Bitrix\KdaImportexcel\HttpClient(array('socketTimeout'=>10));
				...
				foreach($m[1] as $k=>$img)
				{
					...
					*/
					...
					if(!file_exists($realFile) || filesize($realFile)==0)
					{
						...
						$ob = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>15, 'streamTimeout'=>15));
