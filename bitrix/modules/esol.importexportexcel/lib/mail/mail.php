...
class SMail
{
	...
	public function CheckParams($maxTime=10)
	{
		if(!$this->paramsChecked)
		{
			...
			$charset = (\KdaIE\Utils::IsUtfMode() ? 'UTF-8' : 'CP1251');
	...
	public function GetFileId(&$arParams, $maxTime = 0, $extId = '')
	{
		if($this->CheckParams($maxTime))
		{
			...
			$fragment = '';
			if($arCriterias['FILENAME_REGEXP']!='Y' && strpos($arCriterias['FILENAME'], '#')!==false)
			{
				list($arCriterias['FILENAME'], $fragment) = explode('#', $arCriterias['FILENAME'], 2);
			}
			...
			while(!empty($arFolders) && !$fid)
			{
				...
				if(!empty($mailsIds))
				{
					...
					{
						$mailId = $mailsIds[$i];
						if(($arMailFile = $this->imap->getMessageFile($folder, $mailId, $arCriterias, $allowExt, $error))!==false)
						{
							...
							if((!$this->params['LAST_DATE'] && strtotime($arMailFile['DATE']) > $time) || ($arMailFile['DATE']!=$this->params['LAST_DATE'] && strtotime($arMailFile['DATE']) > strtotime($this->params['LAST_DATE'])))
							{
								...
								$dir = $_SERVER["DOCUMENT_ROOT"].'/upload/tmp/'. \KdaIE\Utils::GetModuleId().'/'.\KdaIE\Utils::GetModuleSubDir();
								CheckDirPath($dir);
								$i = 0;
								...
								$arFile = \CKDAImportUtils::MakeFileArray($tmpdir.$fn.(strlen($fragment) > 0 ? '#'.$fragment : ''), $maxTime);
