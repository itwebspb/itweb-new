...
class HttpClient extends \Bitrix\Main\Web\HttpClient
{
	...
	public function mInitProxyParams()
	{
		...
		{
			$moduleId = \KdaIE\Utils::GetModuleId();
	...
	public function mDownloadCurl($url, $filePath, $p=array(), $loop=0)
	{
		...
		{
			if(!\KdaIE\Utils::IsSafePublicUrl($url)) return false;
			...
			if(is_callable(array($this, 'getRequestHeaders'))) $cookies = (string)$this->getRequestHeaders()->get('Cookie');
			if(strlen($cookies)==0 && isset($this->requestCookies)) $cookies = $this->requestCookies->toString();
	...
	public function mCurlGetHeaders($ch, $header)
	{
		...
		{
			if(isset($this->responseHeaders)) $this->responseHeaders->set($headerName, $headerValue);
			else $this->getHeaders()->set($headerName, $headerValue);
		}
		return $len;
	}
	...
	public function getFileFromAddon($url, $filePath)
	{
		...
		$ob = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>1, 'streamTimeout'=>22));
		if($res = $ob->post('https://downloads.esolutions.su/getfile.php', array('domain'=>$domain, 'url'=>$url, 'hash'=>$hash)))
		{
			CheckDirPath($filePath);
