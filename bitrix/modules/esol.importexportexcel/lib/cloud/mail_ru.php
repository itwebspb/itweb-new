...
class MailRu
{
	...
	public static function GetInstance()
	{
		...
		{
			$user = \Bitrix\Main\Config\Option::get(\KdaIE\Utils::GetModuleId(), 'CLOUD_MAILRU_LOGIN', '');
			$pass = \Bitrix\Main\Config\Option::get(\KdaIE\Utils::GetModuleId(), 'CLOUD_MAILRU_PASSWORD', '');
    ...
    {
		$ob = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>10));
	...
	public function DownloadFile(&$tmpPath, $link)
	{
		...
		if($client && $client->getStatus()==200)
		{
			...
			if($hcd = $headers->get('content-disposition'))
			{
				...
				{
					$fn = urldecode($fn);
					if(!\KdaIE\Utils::IsUtfMode() && \CUtil::DetectUTF8($fn))
					{
	...
	{
		$client = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>10, 'redirect'=>$redirect));
