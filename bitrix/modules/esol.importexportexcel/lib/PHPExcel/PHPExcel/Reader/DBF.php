	...
	public function loadIntoExisting($pFilename, KDAPHPExcel $objKDAPHPExcel)
	{
		...
		while($currentRow<=$endRow)
		{
			if($currentRow - 1 == 0)
			{
				...
				{
					$rowData[] = array('v'=>$column->getRawname(), 't'=>KDAPHPExcel_Cell_DataType::TYPE_STRING);
				}
			}
			else
			{
				...
				$rowData = array();
				foreach ($columns as $column) {
					$rowData[] = array('v'=>$record->forceGetString($column->name), 't'=>($column->getType()==\XBase\Record::DBFFIELD_TYPE_NUMERIC ? KDAPHPExcel_Cell_DataType::TYPE_NUMERIC : KDAPHPExcel_Cell_DataType::TYPE_STRING));
				}
			}
			$columnLetter = 'A';
			foreach($rowData as $rowDatum) {
				if ($rowDatum['v'] != '' && $this->_readFilter->readCell($columnLetter, $currentRow)) {
					...
					// Set cell value
					$sheet->getCell($columnLetter . $currentRow)->setValueExplicit($rowDatum['v'], $rowDatum['t']);
				}
				++$columnLetter;
			}
			++$currentRow;
