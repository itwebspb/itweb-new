...
{	
	...
	if(1)
	{
		...
		echo '<script>EList.OnAfterAddNewProperties("'.htmlspecialcharsbx($listIndex).'", "'.(int)$IBLOCK_ID.'", '.\KdaIE\Utils::PhpToJSObject($arResult).');</script>';
		die();
	}
}
...
<form action="" method="post" enctype="multipart/form-data" name="field_settings">
	...
	if(is_array($arPost['items']))
	{
		...
		{
			echo '<input type="hidden" name="items['.htmlspecialcharsbx($k).'][index]" value="'.htmlspecialcharsbx($arItem['index']).'">';
			echo '<input type="hidden" name="items['.htmlspecialcharsbx($k).'][text]" value="'.htmlspecialcharsbx($arItem['text']).'">';
		}
	}
	?>
	...
	<table width="100%" class="kda-ie-newprops-tbl">
			...
			<td class="adm-detail-content-cell-r">
				...
				<div class="kda-ie-newprops-mchosen">
					<select multiple name="FIELDS_FOR_UPDATE[]">
						...
						foreach($arFeatures as $arFeature)
						{
							echo '<option value="'.htmlspecialcharsbx($arFeature['MODULE_ID'].':'.$arFeature['FEATURE_ID']).'">'.$arFeature['FEATURE_NAME'].'</option>';
						}
						?>
					</select>
				</div>
			...
			<td class="adm-detail-content-cell-r">
				<select name="NAMES[]" multiple>
					...
					if(is_array($arPost['items']))
					{
						foreach($arPost['items'] as $arItem)
						{
							echo '<option value="'.htmlspecialcharsbx($arItem['index']).'">'.htmlspecialcharsbx($arItem['text']).'</option>';
						}
					}
					?>
				</select>
		...
		<tr>
			...
			<td class="adm-detail-content-cell-r">
				<input type="text" name="FIELD[SORT]" value="<?echo ($arPost['FIELD']['SORT'] ? htmlspecialcharsbx($arPost['FIELD']['SORT']) : '500')?>">
			</td>
		</tr>
		...
		<tr>
			...
			<td class="adm-detail-content-cell-r">
				<input type="text" name="CODE_PREFIX" value="<?echo ($arPost['CODE_PREFIX'] ? htmlspecialcharsbx($arPost['CODE_PREFIX']) : '')?>">
			</td>
		</tr>
		...
		?>
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<input type="hidden" name="FIELD[<?echo htmlspecialcharsbx($arFeature['MODULE_ID'].':'.$arFeature['FEATURE_ID']);?>]" value="N">
					<input type="checkbox" name="FIELD[<?echo htmlspecialcharsbx($arFeature['MODULE_ID'].':'.$arFeature['FEATURE_ID']);?>]" value="Y"<?echo ($arPost['FIELD'][$arFeature['MODULE_ID'].':'.$arFeature['FEATURE_ID']]=='Y' ? ' checked' : '')?>>
				</td>
			</tr>
		<?
		}
