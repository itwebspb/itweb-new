...
$listIndex = htmlspecialcharsbx($_REQUEST['list_index']);
$fieldKey = htmlspecialcharsbx($_REQUEST['key']);
$fieldType = htmlspecialcharsbx($_REQUEST['type']);
...
if($_POST['action']=='save' && is_array($_POST['SETTINGS']))
{
	...
	echo '<script>EList.SetDisplayParams("'.htmlspecialcharsbx($listIndex).'", '.$returnJson.')</script>';
	die();
}
	...
	if(is_array($SETTINGS['DISPLAY_PARAMS'][$listIndex]))
	{
		...
		foreach($arParams as $k=>$v)
		{
			if(is_array($v))
			{
				foreach($v as $k2=>$v2)
				{
					?><input type="hidden" name="<?echo htmlspecialcharsbx($sharePrefix);?>[<?echo htmlspecialcharsbx($k);?>][<?echo htmlspecialcharsbx($k2);?>]" value="<?echo htmlspecialcharsbx($v2);?>"><?
				}
			}
		}
	}
		...
		<?if($fieldKey=='SECTION_PATH'){?>
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					...
					?>
					<input type="text" name="<?=$fName?>" value="<?=htmlspecialcharsbx($val)?>" placeholder="/">
				</td>
			</tr>
		<?}?>
		...
		<?if($fieldType!='image'){?>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					...
					?>
					<input type="text" name="<?=$fName?>" value="<?=htmlspecialcharsbx($val)?>" placeholder="#000000">
				</td>
			</tr>
