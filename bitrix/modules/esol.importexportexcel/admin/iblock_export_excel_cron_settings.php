...
if(!$GLOBALS['USER']->IsAdmin() /*$MODULE_RIGHT <= "R"*/) $APPLICATION->AuthForm(GetMessage("ACCESS_DENIED"));
...
function GetPhpPathForCron($checkPath = false)
{
	...
	if(is_array($arLines))
	{
		foreach($arLines as $line)
		{
			...
			$path = (isset($arLineParts[5]) ? str_replace("'", '', $arLineParts[5]) : '');
			if(strlen($path) > 0 && !in_array($path, $arPaths) && stripos($path, 'php')!==false && stripos($line, '.php')!==false)
			{
				$arPaths[] = $path;
			}
		}
...
if($_REQUEST["action"]=="deleterecord" && check_bitrix_sessid() && $_REQUEST["key"])
{
...
if ($_REQUEST["action"]=="save")
{
	...
	if(!check_bitrix_sessid()) $strErrorMessage = 'Invalid CSRF token';
	
	if(is_array($PROFILE_ID)) $PROFILE_ID = implode(',', $PROFILE_ID);
	if (strlen($strErrorMessage)<=0 && strlen($PROFILE_ID) < 1)
	{
	...
	if (strlen($strErrorMessage)<=0 && $_REQUEST["subaction"]=='add')
	{
		...
		{
			$expert = (string)$_REQUEST['agent_period_expert'];
			if(!preg_match('#^[\d*/,\-]+(\s+[\d*/,\-]+){4}$#', trim($expert))) $expert = '0 0 * * *';
			$strTime = $expert." ";
		}

		$agent_php_path = Trim($_REQUEST["agent_php_path"]);
		if(!preg_match('#^/[\w\./\-]+$#', $agent_php_path) || stripos($agent_php_path, 'php')===false) $agent_php_path = '/usr/bin/php';
		...
		if (strlen($PROFILE_ID) > 0)
		{
			...
			$cfg_subdata = $strTime.escapeshellarg($agent_php_path)." ".$phpParams." -f ".$execFile." ".$PROFILE_ID." > ".$logFile." 2>&1\n";
			...
			else
			{
				...
				if($cronWritable && $_REQUEST["auto_cron_tasks"]=="Y")
				{
					...
					*/
					...
					$strSuccessMessage .= GetMessage("KDA_EE_CRON_SUCCESS_SAVE")."<br><br><i>".str_replace("'", '', $cfg_subdata).'</i><br><br>';
					$strSuccessMessage .= GetMessage("KDA_EE_CRON_WHERE_IS")."<br>";
					$strSuccessMessage .= '<i>'.$strTime.'</i> - '.GetMessage("KDA_EE_CRON_TIME_EXECUTE_COMMENT")."<br>";
					$strSuccessMessage .= '<i>'.$agent_php_path.'</i> - '.GetMessage("KDA_EE_CRON_PHP_PATH_COMMENT")."<br>";
					$strSuccessMessage .= '<i>'.$execFile.'</i> - '.GetMessage("KDA_EE_CRON_EXEC_FILE_COMMENT")."<br>";
				...
				else
				{
					...
					{
						$strSuccessMessage .= GetMessage("KDA_EE_CRON_SAVE_IN_PANEL_TIMEWEB")."<br><br><i>".str_replace("'", '', str_replace(' '.$phpParams, '', mb_substr($cfg_subdata, mb_strlen($strTime)))).'</i><br><br>';
					}
					else
					{
						$strSuccessMessage .= GetMessage("KDA_EE_CRON_SAVE_IN_PANEL")."<br><br><i>".str_replace("'", '', mb_substr($cfg_subdata, mb_strlen($strTime))).'</i><br><br>';
					}
					$strSuccessMessage .= GetMessage("KDA_EE_CRON_WHERE_IS")."<br>";
					$strSuccessMessage .= '<i>'.$agent_php_path.'</i> - '.GetMessage("KDA_EE_CRON_PHP_PATH_COMMENT")."<br>";
					$strSuccessMessage .= '<i>'.$execFile.'</i> - '.GetMessage("KDA_EE_CRON_EXEC_FILE_COMMENT")."<br>";
...
?>
<form action="<?=htmlspecialcharsbx($_SERVER['REQUEST_URI'])?>" method="post" enctype="multipart/form-data" name="field_settings" class="kda-ee-cron-form">
		...
		<table width="100%" class="kda-ee-cron-records" cellspacing="8">
			...
			foreach($arRecords as $key=>$rec)
			{
				...
				echo '<tr>'.
					...
					'<td><a class="kda-ee-cron-record-edit" href="javascript:void(0)" onclick="return EProfile.EditCronRecord(this, \''.addcslashes($key, "'").'\');" title="'.htmlspecialcharsbx(GetMessage("KDA_EE_CRON_EDIT")).'" data-phppath="'.htmlspecialcharsbx($rec['phppath']).'" data-time="'.htmlspecialcharsbx($rec['time']).'" data-profiles="'.htmlspecialcharsbx(implode(',', $rec['pid'])).'"></a></td>'.
					'<td><a class="kda-ee-cron-record-delete" href="javascript:void(0)" onclick="return EProfile.DeleteFromCron(this, \''.addcslashes($key, "'").'\');" title="'.htmlspecialcharsbx(GetMessage("KDA_EE_CRON_UNSET")).'"></a></td>'.
				'</tr>';
			}
			?>
		</table>
