...
if(is_array($_POST['AUTH_SETTINGS']) && !\KdaIE\Utils::IsUtfMode()) 
{
	$_POST['AUTH_SETTINGS'] = $AUTH_SETTINGS = $APPLICATION->ConvertCharsetArray($_POST['AUTH_SETTINGS'], 'UTF-8', 'CP1251');
}

if($_POST['action']=='checkconnect' && check_bitrix_sessid())
{
	...
	$fileParams = \KdaIE\Utils::PhpToJSObject($_POST['AUTH_SETTINGS']);
	$oProfile = \CKDAImportProfile::getInstance($_REQUEST['TYPE']=='HL' ? 'highload' : 'iblock');
	$oProfile->CheckFileSettings($fileParams, $_REQUEST['PROFILE_ID'], 'tmp');
	$arFile = CKDAImportUtils::MakeFileArray($fileParams);
...
elseif($_POST['action']=='loadparams' && check_bitrix_sessid())
{
	define('PUBLIC_AJAX_MODE', 'Y');
	$sess = $_SESSION;
	session_write_close();
	...
	if(strlen($authLink) > 0 && \KdaIE\Utils::IsSafePublicUrl($authLink))
	{
		$arCookies = array();
		...
		while(strlen($location)>0 && $redirectCount<=5)
		{
			...
			$client = new \Bitrix\Main\Web\HttpClient(array('redirect'=>false));
...
if($_POST['action']=='save' && $_POST['AUTH_SETTINGS'] && check_bitrix_sessid())
{
	$APPLICATION->RestartBuffer();
	ob_end_clean();
	...
	<table width="100%" class="kda-ie-list-settings">
		...
		<tr>
			<td colspan="2" class="kda-ie-linkauth-handler">
				...
				<textarea name="AUTH_SETTINGS[HANDLER_FOR_LINK]"<?if(!$USER->IsAdmin()){echo ' readonly';}?>><?echo htmlspecialcharsbx($AUTH_SETTINGS['HANDLER_FOR_LINK'])?></textarea>
			</td>
		</tr>
	</table>
</form>
