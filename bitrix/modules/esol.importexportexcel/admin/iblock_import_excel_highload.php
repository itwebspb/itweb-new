...
if ($REQUEST_METHOD == "POST" && $MODE=='AJAX')
{
	...
	{
		$oFilter = new CKDAIEFilter($_POST['IBLOCK_ID'], 'hl');
...
if ($REQUEST_METHOD == "POST" && $STEP > 1 && check_bitrix_sessid())
{
	...
	if($ACTION == 'SHOW_REVIEW_LIST')
	{
		...
		foreach($arWorksheets as $k=>$worksheet)
		{
			...
			$iblockId = (int)($SETTINGS['IBLOCK_ID'][$k] ? $SETTINGS['IBLOCK_ID'][$k] : $SETTINGS_DEFAULT['HIGHLOADBLOCK_ID']);
		?>
			<table class="kda-ie-tbl <?if($bEmptyList){echo 'empty';}?> <?if($MODULE_RIGHT <= 'T'){echo 'disabled';}?>" data-list-index="<?echo htmlspecialcharsbx($k);?>" data-iblock-id=<?echo $iblockId;?>>
				<tr class="heading">
...
$arTmpErrors = \CKDAImportProfile::getInstance('highload')->GetErrorsOnIncludeExpClass($PROFILE_ID);
...
?>
	...
	?>
		...
		<tr>
			...
			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[ONLY_DELETE_MODE]" value="Y" <?if($SETTINGS_DEFAULT['ONLY_DELETE_MODE']=='Y'){echo 'checked';}?> onchange="EProfile.RadioChb(this, ['SETTINGS_DEFAULT[ONLY_UPDATE_MODE]', 'SETTINGS_DEFAULT[ONLY_CREATE_MODE]'], '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_ONLY_DELETE_MODE_CONFIRM")); ?>')">
			</td>
		</tr>
...
if($STEP > 1)
{
	...
	else
	{
		...
		{
			?><input type="hidden" name="SETTINGS_DEFAULT[<?echo htmlspecialcharsbx($k)?>]" value="<?echo htmlspecialcharsbx($v) ?>"><?
		}
	}
}
?>
...
<?if ($STEP < 2): 
	...
	if($arFile['path'])
	{
		...
		function KdaSetFilePath(l)
		{
			...
			$('#bx_file_data_file_cont .adm-input-file-name').text("<?echo strip_tags(addslashes($arFile['path']))?>");<?
			if($arFile['link'])
			{
				?>
				$('#bx_file_data_file_cont .adm-input-file-name').attr("target", "_blank").attr("href", "<?echo strip_tags(addslashes($arFile['link']))?>");<?
			}
			?>
		}
		KdaSetFilePath(0);
