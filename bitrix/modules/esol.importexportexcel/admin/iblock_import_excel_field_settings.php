...
if($_POST['action']=='export_conv_csv' && check_bitrix_sessid())
{
	$arExtra = array();
...
elseif($_POST['action']=='import_conv_csv' && check_bitrix_sessid())
{
	$arImportConv = array();
...
elseif($_POST['action']=='save' && is_array($_POST['EXTRASETTINGS']) && check_bitrix_sessid())
{
	$APPLICATION->RestartBuffer();
	ob_end_clean();
...
?>
<form action="" method="post" enctype="multipart/form-data" name="field_settings" class="kda_ie_settings_form"<?if(!$isAdmin){echo ' data-notadmin="1"';}?>>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					...
					<select name="<?echo $fName;?>" class="chosen">
						...
						{
							echo '<option value="'.htmlspecialcharsbx($k).'"'.(($val==$k || (!$val && $k=='UF_NAME')) ? ' selected' : '').'>'.htmlspecialcharsbx($name).'</option>';
						}
						?>
					</select>
				</td>
					...
					*/
					echo '<div class="kda-ie-settings-conversion" '.(!$showCondition ? 'style="display: none;"' : '').'>'.
							...
							' <span class="kda-ie-conv-field field_to">'.
								'<textarea name="'.$fName.'[TO][]" rows="1"'.($isDisabled ? ' readonly' : '').'>'.(strpos($v['TO'], "\n")===0 ? "\n" : '').htmlspecialcharsbx($v['TO']).'</textarea>'.
				...
				<tr>
					<td class="kda-ie-settings-margin-container" colspan="2">
						...
						<div class="kda-ie-settings-phpexpression" style="display: none;">
							...
							<textarea name="<?echo $fName?>"<?if(!$isAdmin){echo ' readonly';}?>><?echo htmlspecialcharsbx($val)?></textarea>
						</div>
					</td>
				</tr>
			<?}?>
		...
		<?if($bPicture){?>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r" style="min-width: 30%;">
					...
					?>Accept: text/html;q=0.9,image/webp,*/*;q=0.8"><?echo htmlspecialcharsbx($val)?></textarea>
				</td>
			</tr>
		<?}?>
...
var admKDASettingMessages = {
	'VAL': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_CURRENT_VALUE"));?>',
	'CELL_VALUE': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_CELL_VALUE"));?>',
	'CLINK': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_CELL_LINK"));?>',
	'CNOTE': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_CELL_COMMENT"));?>',
	'HASH': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_HASH_FILEDS"));?>',
	'FILENAME': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_IFILENAME"));?>',
	'FILEDATE': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_IFILEDATE"));?>',
	'SHEETNAME': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_ISHEETNAME"));?>',
	'ROWNUMBER': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_IROWNUMBER"));?>',
	'DATETIME': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_DATETIME"));?>',
	'SEP_SECTION': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_SEP_SECTION"));?>',
	'RATE_USD': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_RATE_USD"));?>',
	'RATE_EUR': '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_SETTINGS_LANG_RATE_EUR"));?>',
