...
function ShowSheetData($list, $SETTINGS, $SETTINGS_DEFAULT, $EXTRASETTINGS, $PROFILE_ID, $MODULE_RIGHT)
{
	...
	$iblockId = (int)$SETTINGS_DEFAULT['HIGHLOADBLOCK_ID'];
	$list = (int)$list;
	...
	{
		$iblockId = (int)$SETTINGS['LIST_HLBL_ID'][$list];
	}
	...
	$ee = \CKDAExportExcelHighload::getInstance($params, $EXTRASETTINGS, false, 'tmp'.$PROFILE_ID);
	...
	<div class="kda-ee-hidden-settings">
		...
		<?if(isset($SETTINGS['DISPLAY_PARAMS'][$list]) && !empty($SETTINGS['DISPLAY_PARAMS'][$list])){?>
			<input type="hidden" name="SETTINGS[DISPLAY_PARAMS][<?echo htmlspecialcharsbx($list);?>]" value="">
			<script>EList.SetDisplayParams("<?echo htmlspecialcharsbx($list)?>", <?echo \KdaIE\Utils::PhpToJSObject($SETTINGS['DISPLAY_PARAMS'][$list])?>)</script>
		<?}?>
		<input type="hidden" name="SETTINGS[SORT][<?echo htmlspecialcharsbx($list);?>]" value="<?echo htmlspecialcharsbx($sortVal);?>">
		<input type="hidden" name="SETTINGS[SORT_OFFER][<?echo htmlspecialcharsbx($list);?>]" value="<?echo htmlspecialcharsbx($sortOfferVal);?>">
	</div>
	<?
	if($MODULE_RIGHT > 'T')
	{?>
		...
		<div class="addsettings_inner">
			<table class="additional">
				...
				<tr>
					...
					<td>
						<input type="text" name="SETTINGS[LIST_LABEL_COLOR][<?echo htmlspecialcharsbx($list);?>]" value="<?=htmlspecialcharsbx($SETTINGS['LIST_LABEL_COLOR'][$list])?>" placeholder="#ffffff" size="10">
					</td>
				</tr>
				<tr>
	...
	foreach($arData as $arElement)
	{
		...
		foreach($arFields as $key=>$field)
		{
			...
			echo '<td><div class="cell"><div class="cell_inner">'.nl2br(htmlspecialcharsbx($val)).'</div></div></td>';
		}
		echo '</tr>';
	}
	echo '</table>';
...
$arTmpErrors = \CKDAExportProfile::getInstance('highload')->GetErrorsOnIncludeExpClass($PROFILE_ID);
		 ...
		 <table width="100%"><tr><td width="50%">
			...
			<div class="detail_status">
				...
				echo CAdminMessage::ShowMessage(array(
					...
					' <span id="kda_ee_ready_file" style="visibility: hidden;"><br>'.GetMessage("KDA_EE_READY_FILE_LINK").' <br><a href="'.htmlspecialcharsbx($outputFile).'?hash='.md5(mt_rand()).'">'.$outputFile.'</a> '.($GLOBALS['USER']->CanDoFileOperation('fm_download_file', array('', htmlspecialcharsbx($outputFile))) ? '<br><br><a href="/bitrix/admin/fileman_file_download.php?path='.htmlspecialcharsbx($outputFile).'&lang='.LANGUAGE_ID.'&hash='.md5(mt_rand()).'">'.GetMessage("KDA_EE_DOWNLOAD_FILE").'</a>' : '').'</span><br>',
					"HTML" => true,
				));?>
			</div>
		 </td></tr></table>
