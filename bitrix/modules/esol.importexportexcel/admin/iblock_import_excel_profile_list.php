...
{
	$error = '';
	if(!check_bitrix_sessid()) $error = 'Invalid CSRF token';
	$newGroupName = trim(strip_tags($_POST['new_group_name']));
	if(strlen($newGroupName)==0) $error = GetMessage("KDA_IE_NEW_GROUP_NAME_EMPTY");	
	if(strlen($error)==0)
	{
...
elseif($_GET['action']=='showoldparams' || $_GET['action']=='saveoldparams')
{
	...
	if($_GET['action']=='saveoldparams')
	{
		...
		{
			echo \KdaIE\Utils::PhpToJSObject(array('TYPE'=>'ERROR', 'MESSAGE'=>'Invalid CSRF token'));
		}
		else
		{
		...
		<form action="" method="post" enctype="multipart/form-data" id="restore_profile_params">
			...
			<table width="100%">
				...
				<tr>
					...
					<td class="adm-detail-content-cell-r"><b><?echo htmlspecialcharsbx($arProfile['NAME'])?></b></td>
				</tr>
				<tr>
...
if($lAdmin->EditAction())
{
	foreach ($_POST['FIELDS'] as $ID => $arFields)
	{
		...
		if(array_key_exists('NAME', $arFields)) $arFields['NAME'] = trim(strip_tags($arFields['NAME']));
...
while($arGroup = $rsDataGroup->NavNext(true, "f_"))
{
	...
	$row->AddViewField("NAME", '<a href="'.$moduleFilePrefix.'_profile_list.php?set_filter=Y&filter_group_id='.$f_ID.'&lang='.LANG.'"><span class="adm-submenu-item-link-icon adm-list-table-icon iblock-section-icon"></span> '.htmlspecialcharsbx($f_NAME).'</a>');
...
foreach($arProfiles as $arProfile)
{
	...
	$row->AddViewField("NAME", '<a href="'.$moduleFilePrefix.'.php?PROFILE_ID='.$f_ID.'&lang='.LANG.'">'.htmlspecialcharsbx($arProfile['NAME']).'</a>');
	$row->AddInputField("NAME", array('SIZE'=>40));
	$row->AddInputField("SORT", array('SIZE'=>10));
...
if($USER->IsAdmin() && $oProfile instanceof CKDAImportProfileDB)
{	
	$aMenu[] = array(
