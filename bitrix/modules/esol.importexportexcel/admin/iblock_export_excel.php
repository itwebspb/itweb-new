...
if(($ACTION=='SHOW_PREVIEW' || $ACTION=='DO_EXPORT') && !\KdaIE\Utils::IsUtfMode())
{
...
function ShowSheetData($list, $SETTINGS, $SETTINGS_DEFAULT, $EXTRASETTINGS, $PROFILE_ID, $MODULE_RIGHT)
{
	ob_start();
	if(!is_array($SETTINGS)) $SETTINGS = array();
	$iblockId = (int)$SETTINGS_DEFAULT['IBLOCK_ID'];
	$list = (int)$list;
	$changeIblockId = (bool)($SETTINGS['CHANGE_IBLOCK_ID'][$list]=='Y');
	if($changeIblockId && $SETTINGS['LIST_IBLOCK_ID'][$list])
	{
		$iblockId = (int)$SETTINGS['LIST_IBLOCK_ID'][$list];
	}
	$arFieldParams = array();
	...
	$ee = \CKDAExportExcel::getInstance($params, $EXTRASETTINGS, false, 'tmp'.$PROFILE_ID);
	...
	foreach($textKeys as $textKey)
	{
		...
		if(!empty($SETTINGS[$textKey][$list]))
		{
			foreach($SETTINGS[$textKey][$list] as $k=>$v)
			{
				...
				$rowContent .= '<td>'.($MODULE_RIGHT > 'T' ? '<span class="sandwich" data-key="'.htmlspecialcharsbx($dataKey).'" data-type="'.htmlspecialcharsbx($dataType).'" title="'.GetMessage("KDA_EE_ACTIONS_BTN").'"></span>' : '').'</td>';
				if($dataType == 'image')
				{
					...
					$rowContent .= '<input type="hidden" name="SETTINGS['.htmlspecialcharsbx($textKey).']['.htmlspecialcharsbx($list).']['.htmlspecialcharsbx($k).']" value="'.htmlspecialcharsbx($v).'">';
					$rowContent .= '</div></div></td>';
				}
				else
				{
					$rowContent .= '<td colspan="'.count($arFields).'"><div class="cell cell_wide80"><div class="cell_inner">';
					$rowContent .= '<textarea class="kda-ee-text-block" name="SETTINGS['.htmlspecialcharsbx($textKey).']['.htmlspecialcharsbx($list).']['.htmlspecialcharsbx($k).']" '.CKDAExportUtils::GetCellStyleFormatted($dSettings, $SETTINGS_DEFAULT).($MODULE_RIGHT <= 'T' ? ' disabled' : '').'>'.htmlspecialcharsbx($v).'</textarea>';
	...
	foreach($arFields as $k=>$field)
	{
		...
		echo '<th class="kda-ee-field-select">'.
				'<b>'.($k+1).' ('.CKDAExportUtils::GetColLetterByIndex($k).')</b>'.
				'<div>'.
	...
	if($ACTION=='SHOW_PREVIEW')
	{
		...
		if(array_key_exists('SHEET_INDEXES', $_POST))
		{
			...
			foreach($arIndexes as $k=>$index)
			{
				...
				echo '<div id="kda-ee-sheet-'.htmlspecialcharsbx($index).'">';
				ShowSheetData($index, $SETTINGS, $SETTINGS_DEFAULT, $EXTRASETTINGS, $PROFILE_ID, $MODULE_RIGHT);
				echo '</div>';
			}
			echo '</div>';
		}
		...
		{
			ShowSheetData($_POST['SHEET_INDEX'], $SETTINGS, $SETTINGS_DEFAULT, $EXTRASETTINGS, $PROFILE_ID, $MODULE_RIGHT);
		}
...
$arTmpErrors = \CKDAExportProfile::getInstance()->GetErrorsOnIncludeExpClass($PROFILE_ID);
	...
	?>
		...
		<tr>
			...
			<td>
				...
				<td id="export_add_domain_value"<?if($SETTINGS_DEFAULT['EXPORT_ADD_DOMAIN']!='Y'){echo ' style="display: none;"';}?>>
					<input type="text" name="SETTINGS_DEFAULT[EXPORT_ADD_DOMAIN_VALUE]" value="<?=htmlspecialcharsbx($SETTINGS_DEFAULT['EXPORT_ADD_DOMAIN_VALUE'])?>" size="40" placeholder="<?echo htmlspecialcharsbx($placeholder);?>">
				</td>
				</tr></table>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<table cellspacing="0"><tr>
				<td style="padding-left: 0px;">
					<input type="checkbox" name="SETTINGS_DEFAULT[EXPORT_ARCHIVE]" value="Y" <?=htmlspecialcharsbx($SETTINGS_DEFAULT['EXPORT_ARCHIVE']=='Y' ? 'checked' : '')?> onchange="if(this.checked){$('#EXPORT_ARCHIVE_wrap').show();}else{$('#EXPORT_ARCHIVE_wrap').hide();}">
				</td>
				<td>&nbsp; &nbsp;</td>
				<td>
					<div id="EXPORT_ARCHIVE_wrap" <?if($SETTINGS_DEFAULT['EXPORT_ARCHIVE']!='Y'){echo 'style="display: none;"';}?>>
						<?echo GetMessage("KDA_EE_OTHER_EXPORT_ARCHIVE_PASSWORD");?><br>
						<input type="text" name="SETTINGS_DEFAULT[EXPORT_ARCHIVE_PASSWORD]" placeholder="<?echo htmlspecialcharsbx(GetMessage("KDA_EE_OTHER_EXPORT_ARCHIVE_EMPTY"))?>" value="<?echo htmlspecialcharsbx($SETTINGS_DEFAULT['EXPORT_ARCHIVE_PASSWORD']); ?>" size="55">
					</div>
				</td>
				</tr></table>
			</td>
			...
			<td>
				...
				<td>
					<div id="EXPORT_FILES_IN_ARCHIVE_wrap" <?if($SETTINGS_DEFAULT['EXPORT_FILES_IN_ARCHIVE']!='Y'){echo 'style="display: none;"';}?>>
						...
						?>
						<input type="text" name="SETTINGS_DEFAULT[FILES_ARCHIVE_PATH]" value="<?echo htmlspecialcharsbx($archivePath); ?>" size="55">
			...
			foreach($arKeys as $list)
			{
				...
				<div class="kda-ee-sheet-wrap<?if($ind > 0){echo ' withmargin';} if($MODULE_RIGHT <= 'T'){echo ' minrigths';}?>">
					...
					<div class="kda-ee-sheet" 
						id="kda-ee-sheet-<?echo htmlspecialcharsbx($list);?>" 
						data-sheet-index="<?echo htmlspecialcharsbx($list);?>" 
						data-show-field-codes="<?echo htmlspecialcharsbx($showCodes);?>"
						data-init="false">
						<?
						if($SETTINGS_DEFAULT['USE_NEW_FILTER']=='Y')
						{
							//ShowSheetData($list, $SETTINGS, $SETTINGS_DEFAULT, $EXTRASETTINGS, $PROFILE_ID, $MODULE_RIGHT);
						}
						?>
					</div>
					<?
