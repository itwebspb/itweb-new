...
if(strlen($PROFILE_ID) > 0 && $PROFILE_ID!=='new')
{
	...
	if($SETTINGS_DEFAULT['URL_DATA_FILE'] && !$SETTINGS_DEFAULT["DATA_FILE"])
	{
		...
		{
			if(\KdaIE\Utils::IsUtfMode()) $filepath = $APPLICATION->ConvertCharsetArray($filepath, LANG_CHARSET, 'CP1251');
	...
	?><tr>
		...
		foreach($data as $row)
		{
			...
			if($row['STYLE'])
			{
				...
				$dataStyle = 'data-style="'.htmlspecialcharsbx(\KdaIE\Utils::PhpToJSObject($row['STYLE'])).'"';
			}
			$style = ($style ? 'style="'.$style.'"' : '');
			$parentStyle = ($parentStyle ? 'style="'.$parentStyle.'"' : '');
		?><td <?echo $parentStyle;?>><div class="cell" <?echo $parentStyle;?>><div class="cell_inner" <?echo $style;?> <?echo $dataStyle;?>><?echo nl2br(htmlspecialcharsbx($row['VALUE']));?></div></div></td><?
		}
	?></tr><?
}
/////////////////////////////////////////////////////////////////////
if ($REQUEST_METHOD == "POST" && isset($MODE) && $MODE=='AJAX')
{
	...
	if($ACTION=='RENAME_PROFILE')
	{
		...
		if(!\KdaIE\Utils::IsUtfMode()) $newName = $APPLICATION->ConvertCharset($newName, 'UTF-8', 'CP1251');
		$fl = new CKDAImportProfile();
	...
	if($ACTION=='GET_COLUMN_VALUES')
	{
		...
		$viewer = new \Bitrix\KdaImportexcel\ExcelViewer($arFile['SRC'], $SETTINGS_DEFAULT, $_POST['PROFILE_ID']);
...
if ($REQUEST_METHOD == "POST" && $STEP > 1 && check_bitrix_sessid())
{
	...
	if ($STEP > 1)
	{
		...
		$bNewFile = false;
		if (strlen($strError) <= 0  && (!$notRewriteFile || (isset($_POST['FORCE_UPDATE_FILE']) && $_POST['FORCE_UPDATE_FILE']=='Y')))
		{
			if($STEP==2 || ($STEP==3 && (isset($_POST['FORCE_UPDATE_FILE']) && $_POST['FORCE_UPDATE_FILE']=='Y')))
			{
		...
		if (strlen($strError) <= 0)
		{
			...
			elseif($bNewFile && isset($_POST['FORCE_UPDATE_FILE']) && $_POST['FORCE_UPDATE_FILE']=='Y')
			{
	...
	if($ACTION == 'SHOW_REVIEW_LIST')
	{
		...
		foreach($arWorksheets as $k=>$worksheet)
		{
			...
			$iblockId = (int)($SETTINGS['IBLOCK_ID'][$k] ? $SETTINGS['IBLOCK_ID'][$k] : $SETTINGS_DEFAULT['IBLOCK_ID']);
		?>
			<table class="kda-ie-tbl <?if($bEmptyList){echo 'empty';}?> <?if($MODULE_RIGHT <= 'T'){echo 'disabled';}?>" data-list-index="<?echo htmlspecialcharsbx($k);?>" data-iblock-id="<?echo $iblockId;?>" data-field-mapping="<?echo htmlspecialcharsbx($SETTINGS_DEFAULT['FIELD_MAPPING']);?>">
				<tr class="heading">
					...
					<td class="right list-settings">
						<?if(count($worksheet['lines']) > 0){?>
							...
							<?}?>
							<input type="hidden" name="SETTINGS[ADDITIONAL_SETTINGS][<?echo $k;?>]" value="<?if($SETTINGS['ADDITIONAL_SETTINGS'][$k])echo htmlspecialcharsbx(\KdaIE\Utils::PhpToJSObject($SETTINGS['ADDITIONAL_SETTINGS'][$k]));?>">
							...
							if(is_array($SETTINGS['LIST_SETTINGS'][$k]))
							{
								...
								{
									?><input type="hidden" name="SETTINGS[LIST_SETTINGS][<?echo htmlspecialcharsbx($k);?>][<?echo htmlspecialcharsbx($k2);?>]" value="<?echo htmlspecialcharsbx($v2);?>"><?
								}
							}
							if(is_array($EXTRASETTINGS[$k]))
							{
								foreach($EXTRASETTINGS[$k] as $k2=>$v2)
								{
									...
									{
										?><div><a href="javascript:void(0)" id="field_settings_<?echo htmlspecialcharsbx($k);?>_<?echo htmlspecialcharsbx($k2);?>" onclick="EList.ShowFieldSettings(this);"><input type="hidden" name="EXTRASETTINGS[<?echo htmlspecialcharsbx($k);?>][<?echo htmlspecialcharsbx($k2);?>]" value=""><script>EList.SetExtraParams("field_settings_<?echo htmlspecialcharsbx($k);?>_<?echo htmlspecialcharsbx($k2);?>", <?echo \KdaIE\Utils::PhpToJSObject($v2);?>)</script></a></div><?
									}
								}
							}
						}?>
									...
									?>
										...
										<tr class="subfield" <?if(!$changeCsvParams){echo 'style="display: none;"';}?>>
											...
											<td>
												...
												?>
												<input type="text" name="SETTINGS[CSV_PARAMS][SEPARATOR]" value="<?echo htmlspecialcharsbx($val)?>" size="3" maxlength="3">
											</td>
										</tr>
										<tr class="subfield" <?if(!$changeCsvParams){echo 'style="display: none;"';}?>>
											...
											<td>
												...
												?>
												<input type="text" name="SETTINGS[CSV_PARAMS][ENCLOSURE]" value="<?echo htmlspecialcharsbx($val)?>" size="3" maxlength="3">
											</td>
										</tr>
										<tr class="subfield" <?if(!$changeCsvParams){echo 'style="display: none;"';}?>>
											...
											<td>
												...
												?>
												<input type="text" name="SETTINGS[CSV_PARAMS][ENCODING]" value="<?echo htmlspecialcharsbx($val)?>" size="10" maxlength="50">
											</td>
										</tr>
										<?if($fileExt=='txt'){?>
										<tr class="subfield" <?if(!$changeCsvParams){echo 'style="display: none;"';}?>>
											...
											<td>
												...
												?>
												<input type="text" name="SETTINGS[CSV_PARAMS][ROW_SEPARATOR]" value="<?echo htmlspecialcharsbx($val)?>" size="10" maxlength="50">
											</td>
										</tr>
										<?}?>
									<?}?>
										...
										<td class="kda-ie-field-select" title="#CELL<?echo ($i+1);?>#">
											...
											<?foreach($arKeys as $j){?>
												<div>
													...
													<input type="hidden" name="SETTINGS[FIELDS_LIST][<?echo htmlspecialcharsbx($k)?>][<?echo htmlspecialcharsbx($j)?>]" value="<?echo htmlspecialcharsbx($SETTINGS['FIELDS_LIST'][$k][$j])?>" >
													<?/*?><input type="text" name="FIELDS_LIST_SHOW[<?echo $k?>][<?echo $j?>]" value="" class="fieldval"><?*/?>
													<span class="fieldval_wrap"><span class="fieldval" id="field-list-show-<?echo htmlspecialcharsbx($k)?>-<?echo htmlspecialcharsbx($j)?>"></span></span>
													<?if($MODULE_RIGHT > 'T'){?>
													<?if(!$USER->IsAdmin()){?><input type="hidden" name="COL_POSITIONS[<?echo htmlspecialcharsbx($k)?>][<?echo htmlspecialcharsbx($j)?>]" value="<?echo htmlspecialcharsbx($k.'_'.$j)?>"><?}?>
													<a href="javascript:void(0)" class="field_settings <?=(empty($EXTRASETTINGS[$k][$j]) ? 'inactive' : '')?>" id="field_settings_<?=htmlspecialcharsbx($k)?>_<?=htmlspecialcharsbx($j)?>" title="<?echo GetMessage("KDA_IE_SETTINGS_FIELD"); ?>" onclick="EList.ShowFieldSettings(this);">
														<input type="hidden" name="EXTRASETTINGS[<?echo htmlspecialcharsbx($k)?>][<?echo htmlspecialcharsbx($j)?>]" value="">
														<?if(!empty($EXTRASETTINGS[$k][$j])){?>
															<script>EList.SetExtraParams("field_settings_<?=htmlspecialcharsbx($k)?>_<?=htmlspecialcharsbx($j)?>", <?echo \KdaIE\Utils::PhpToJSObject($EXTRASETTINGS[$k][$j]);?>)</script>
														<?}?>
													</a>
...
$arTmpErrors = \CKDAImportProfile::getInstance()->GetErrorsOnIncludeExpClass($PROFILE_ID);
if(count($arTmpErrors) > 0) $strErrorProfile .= implode('<br>', $arTmpErrors);
...
?>
	...
	?>
		...
		<tr>
			...
			<td width="60%" class="kda-ie-file-choose">
				...
				if($arFile['SRC'])
				{
					...
					{
						if(\KdaIE\Utils::IsUtfMode()) $arFile['SRC'] = $APPLICATION->ConvertCharsetArray($arFile['SRC'], LANG_CHARSET, 'CP1251');
		...
		<tr>
			...
			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[ONLY_DELETE_MODE]" value="Y" <?if($SETTINGS_DEFAULT['ONLY_DELETE_MODE']=='Y'){echo 'checked';}?> onchange="EProfile.RadioChb(this, ['SETTINGS_DEFAULT[ONLY_UPDATE_MODE_ELEMENT]', 'SETTINGS_DEFAULT[ONLY_CREATE_MODE_ELEMENT]'], '<?echo htmlspecialcharsbx(GetMessage("KDA_IE_ONLY_DELETE_MODE_CONFIRM")); ?>')"<?if($arRights['delete_products']=='N'){echo ' disabled';}?>>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				<td id="copy_file_path"<?if($SETTINGS_DEFAULT['COPY_FILE_TO_PATH']!='Y'){echo ' style="display: none;"';}?>>
					<input type="text" name="SETTINGS_DEFAULT[COPY_FILE_PATH]" value="<?echo htmlspecialcharsbx($SETTINGS_DEFAULT['COPY_FILE_PATH'])?>" placeholder="/upload/file.xlsx" size="50">
				</td>
				</tr></table>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<input type="text" name="SETTINGS_DEFAULT[COUNT_LINES_FOR_PREVIEW]" value="<?echo htmlspecialcharsbx($SETTINGS_DEFAULT['COUNT_LINES_FOR_PREVIEW'])?>" placeholder="10">
			</td>
		</tr>
		...
		<tr>
			<td class="kda-ie-settings-margin-container" colspan="2" align="center">
				...
				<div class="kda-ie-settings-phpexpression" style="display: none;">
					...
					<textarea name="SETTINGS_DEFAULT[ONAFTERSAVE_HANDLER]"<?if(!$USER->IsAdmin()){echo ' readonly';}?>><?echo htmlspecialcharsbx($SETTINGS_DEFAULT['ONAFTERSAVE_HANDLER'])?></textarea>
				</div>
			</td>
		</tr>
