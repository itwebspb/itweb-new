...
if($_POST['action']=='save')
{
	...
	if($ID)
	{
		...
		echo '<script>EList.OnAfterAddNewProperty("'.htmlspecialcharsbx($_REQUEST['PARENT_FIELD_NAME']).'", "'.htmlspecialcharsbx($arFields['FIELD_NAME']).'", "'.htmlspecialcharsbx($nameLang).'", "'.$HLBL_ID.'");</script>';
		die();
	}
	else
	{
...
$ENTITY_ID = htmlspecialcharsbx('HLBLOCK_'.$HLBL_ID);
...
<form action="" method="post" enctype="multipart/form-data" name="new_property" id="newPropertyForm">
	...
	<table width="100%">
		...
		<tr class="adm-detail-required-field">
			...
			<td class="adm-detail-content-cell-r">
				<input type="text" name="FIELD_NAME" value="<?=htmlspecialcharsbx($FIELD_NAME)?>" maxlength="20">
			</td>
		</tr>
		<tr>
			<td class="adm-detail-content-cell-l"><?=GetMessage("KDA_IE_USERTYPE_XML_ID")?>:</td>
			<td class="adm-detail-content-cell-r"><input type="text" name="XML_ID" value="<?=htmlspecialcharsbx($XML_ID)?>" maxlength="255"></td>
		</tr>
		<tr>
			<td class="adm-detail-content-cell-l"><?=GetMessage("KDA_IE_USERTYPE_SORT")?>:</td>
			<td class="adm-detail-content-cell-r"><input type="text" name="SORT" value="<?=htmlspecialcharsbx($SORT)?>"></td>
		</tr>
		<tr>
