...
if($_POST['action']=='save' && $_POST['ADDITIONAL_SETTINGS'])
{
	...
	echo '<input type="hidden" name="from_list_settings" id="from_list_settings" value="'.htmlspecialcharsbx(\KdaIE\Utils::PhpToJSObject($_POST['ADDITIONAL_SETTINGS'])).'">';
	echo '<script>';
	echo '$(\'table.kda-ie-tbl[data-list-index='.htmlspecialcharsbx($arGet['list_index']).'] input[name^="SETTINGS[ADDITIONAL_SETTINGS]"]\').val($("#from_list_settings").val());';
...
function ShowPostData($post, $key='')
{
	foreach($post as $k=>$v)
	{
		...
		{
			echo '<input type="hidden" name="'.htmlspecialcharsbx($k2).'" value="'.htmlspecialcharsbx($v).'">';
		}
	}
}
			...
			<td class="kda-ie-ls-range" colspan="2">
				...
				foreach($ADDITIONAL_SETTINGS['LOADING_RANGE'] as $k=>$v)
				{
					...
					<div class="kda-ie-ls-range-item"<?if($k==$cnt-1){echo ' style="display: none;"';}?>>
						...
						<input type="text" name="ADDITIONAL_SETTINGS[LOADING_RANGE_FROM][]" value="<?echo htmlspecialcharsbx($v['FROM'])?>">
						<?echo GetMessage("KDA_IE_LIST_SETTING_LOADING_RANGE_TO")?>
						<input type="text" name="ADDITIONAL_SETTINGS[LOADING_RANGE_TO][]" value="<?echo htmlspecialcharsbx($v['TO'])?>">
		...
		if(is_array($ELEMENT_PROPERTIES_DEFAULT))
		{
			foreach($ELEMENT_PROPERTIES_DEFAULT as $k=>$v)
			{
				...
				<tr class="kda-ie-list-settings-defaults">
					...
					<td class="adm-detail-content-cell-r">
						<input type="text" name="ADDITIONAL_SETTINGS[ELEMENT_PROPERTIES_DEFAULT][<?echo htmlspecialcharsbx($k);?>]" value="<?echo htmlspecialcharsbx($v);?>">
			...
			if(is_array($OFFER_PROPERTIES_DEFAULT))
			{
				foreach($OFFER_PROPERTIES_DEFAULT as $k=>$v)
				{
					...
					<tr class="kda-ie-list-settings-defaults">
						<td class="adm-detail-content-cell-l"><?echo $fieldName;?>:</td>
						<td class="adm-detail-content-cell-r">
							<input type="text" name="ADDITIONAL_SETTINGS[OFFER_PROPERTIES_DEFAULT][<?echo htmlspecialcharsbx($k);?>]" value="<?echo htmlspecialcharsbx($v);?>">
							<a class="delete" href="javascript:void(0)" onclick="ESettings.RemoveDefaultProp(this);" title="<?echo GetMessage("KDA_IE_LIST_SETTING_DELETE"); ?>"></a>
						</td>
					</tr>
					<?
