...
var EList = {
	...
	SetFieldValues: function(gParent, rewriteAutoinsert, newIblock)
	{
		...
		$('select[name^="FIELDS_LIST["]', gParent).each(function(){
			...
			$('input[name^="SETTINGS[FIELDS_LIST]"]', parent).each(function(index){
				...
				if(newIblock && input.value)
				{
					var prefixes = ['IP_PROP', 'OFFER_IP_PROP'];
					for(var j=0; j<prefixes.length; j++)
					{
						if(input.value.indexOf(prefixes[j])==0 && !arVals[input.value])
						{
							for(var i in arVals)
							{
								if(i.indexOf(prefixes[j])==0 && 
									(arVals[i]==inputShow.innerText
									|| arVals[i].replace(/^\s*.*\[([^\]]+)\]\s*$/, '$1')==inputShow.innerText.replace(/^\s*.*\[([^\]]+)\]\s*$/, '$1')
									|| arVals[i].replace(/^\s*(.*)\[[^\]]*\]\s*$/, '$1')==inputShow.innerText.replace(/^\s*(.*)\[[^\]]*\]\s*$/, '$1'))
									)
								{
									input.value = i;
	...
	ApplyToAllLists: function(link)
	{
		...
		var stInput = $('input[name="SETTINGS[LIST_SETTINGS]['+tbl.attr('data-list-index')+'][SET_TITLES]"]', tbl);
		var bfInput = $('input[name="SETTINGS[LIST_SETTINGS]['+tbl.attr('data-list-index')+'][BIND_FIELDS_TO_HEADERS]"]', tbl);
		var iblockId = tbl.find('select[name^="SETTINGS[IBLOCK_ID]"]').val();
		for(var i=0; i<tbls.length; i++)
		{
			var iblockId2 = $(tbls[i]).find('select[name^="SETTINGS[IBLOCK_ID]"]').val();
			...
			for(var j=0; j<ts.length; j++)
			{
				...
				for(var k=0; k<ftss.length; k++)
				{
					if(fts[k])
					{
						var value = fts[k].value;
						var innerHTML = fts2[k].innerHTML;
						if(iblockId!=iblockId2)
						{
							var prefix = '';
							if(value.indexOf('IP_PROP')==0) prefix = 'IP_PROP';
							else if(value.indexOf('OFFER_IP_PROP')==0) prefix = 'OFFER_IP_PROP';
							if(prefix.length > 0)
							{
								var fl = $(tbls[i]).find('select[name^="FIELDS_LIST[');
								var optText = fts2[k].innerHTML.replace('"', '');
								var opt = fl.find('option[value^="'+prefix+'"]:contains("'+optText+'")');
								if(opt.length==0)
								{
									var code = optText.replace(/^.*(\[.*\]).*$/, '$1');
									if(code.length > 2)
									{
										opt = fl.find('option[value^="'+prefix+'"]:contains("'+code+'")');
									}
								}
								if(opt.length==0)
								{
									var name = optText.replace(/^(.*\[).*$/, '$1');
									var opt = fl.find('option[value^="'+prefix+'"]:contains("'+name+'")');
								}
								value = (opt.length > 0 ? opt.val() : '');
								innerHTML = (opt.length > 0 ? opt.text() : '');
							}
						}
						//ftss[k].value = fts[k].value;
						//ftss2[k].value = fts2[k].value;
						EList.SetHiddenFieldVal(ftss[k], value);
						EList.SetShowFieldVal(ftss2[k], innerHTML);
	...
	ChooseIblock: function(select)
	{
		...
		$.post(window.location.href, post, function(data){
			...
			ptable.find('select[name^="FIELDS_LIST["]').each(function(){
				...
				fields.attr('name', this.name);
				$(this).replaceWith(fields);
				EList.SetFieldValues(ptable, true, true);
			});
	...
	ShowListSettings: function(btn)
	{
		...
		BX.addCustomEvent(dialog, 'onWindowRegister', function(){
			...
			{
				$('select.kda-chosen-multi').chosen({search_contains: true});
			}
		});
...
var EProfile = {
		...
		if(select.length > 0)
		{
			...
			$('select.adm-detail-iblock-list').bind('change', function(){
				$.post(window.location.href, {'MODE': 'AJAX', 'IBLOCK_ID': this.value, 'ACTION': 'GET_UID'}, function(data){
					...
					if(typeof $('select.kda-chosen-multi').chosen == 'function')
					{
						$('select.kda-chosen-multi').chosen({width: '300px', search_contains: true});
					}
				});
			});
	...
	EditCronRecord: function(btn, key)
	{
		...
		$('input[name="agent_php_path"]', form).val(btn.attr('data-phppath').replace(/'/g, ''));
	...
	DeleteFromCron: function(btn, key)
	{
		...
		var form = $(btn).closest('form');
		var action = form[0].getAttribute('action');
		$.post(action, 'action=deleterecord&key='+encodeURIComponent(key)+'&sessid='+$('input[name="sessid"]', form).val(), function(data){
			$('#kda-ie-cron-result').html('');
			obj.UpdateCronRecords(action);
		});
	},
	...
	OpenMissignElementFields: function(link)
	{
		...
		BX.addCustomEvent(dialog, 'onWindowRegister', function(){
			...
			$('select.kda-ie-chosen-multi').chosen({search_contains: true});
		});
	...
	{
		var form = $(link).closest('form');
		var post = form.serialize()+'&action=checkconnect&sessid='+$('input[name="sessid"]', form).val();
		$.ajax({
			type: "POST",
			url: form.attr('action'),
			data: post,
	...
	{
		var form = $(link).closest('form');
		var post = form.serialize()+'&action=loadparams&sessid='+$('input[name="sessid"]', form).val();
		$.ajax({
			type: "POST",
			url: form.attr('action'),
			data: post,
	...
	ShowRestoreWindow: function()
	{
		...
		BX.addCustomEvent(dialog, 'onWindowRegister', function(){
			...
			$('input[type=file]', newForm).bind('change', function(){
				...
				$.ajax({
					url: windowUrl+'&action=getprofilesfromfile&sessid='+$('input[name="sessid"]', newForm).val(),
					type: 'POST',
...
var ESettings = {
		...
		$('.kda-ie-settings-conversion:not([data-events-init]), .kda-ie-conv-condition-group-field:not([data-events-init])').each(function(){
			...
			{			
				$('select.field_then, span.field_then input[type="hidden"]', parent).bind('change', function(){
					...
					$('.field_to textarea', parent).prop('readonly', readony);
	...
	ExportConvCSV: function(link)
	{
		...
		newForm.append('<input type="hidden" name="action" value="export_conv_csv"><input type="hidden" name="sessid" value="'+$('input[name="sessid"]', form).val()+'">');
		$('#'+formId).remove();
		form.after(newForm);
	...
	ImportConvCSV: function(link)
	{
		...
		var form = $(link).closest('form');
		var newForm = $('<form method="post" id="'+formId+'" style="display: none;"><input type="file" name="import_file"><input type="hidden" name="action" value="import_conv_csv"><input type="hidden" name="sessid" value="'+$('input[name="sessid"]', form).val()+'"></form>');
		newForm.attr('action', url);
		$('#'+formId).remove();
		form.after(newForm);
		$('input[type=file]', newForm).bind('change', function(){
...
var KdaOptions = {
	...
	ConvertProfiles: function(btn)
	{
		this.wait = BX.showWait();
		var form = $(btn).closest('form');
		this.ConvertProfilesStep(form);
		return false;
	},
	
	ConvertProfilesStep: function(form, lastId, cnt)
	{
		var obj = this;
		$.post(window.location.href, form.serialize()+(lastId ? '&lastId='+lastId : '')+(cnt ? '&cnt='+cnt : ''), function(data){
			eval('var res = '+data+';');
			var resWrap = $('#convert_profiles_message');
			if(typeof res=='object')
			{
				if(res.cnt) resWrap.html(BX.message("KDA_IE_CONVERT_PROFILE_CNT")+': '+res.cnt);
				if(res.result=='progress')
				{
					obj.ConvertProfilesStep(form, res.lastId, res.cnt);
					return;
				}
			}
			
			resWrap.html(resWrap.html()+'<br><b>'+BX.message("KDA_IE_CONVERT_PROFILE_FINISH")+'</b>');
