...
if(!class_exists('CEsolImpExpExcelRunner'))
{
	class CEsolImpExpExcelRunner
	{
		...
		static function ImportIblock($filename, $params, $fparams, $stepparams, $pid = false)
		{
			...
			$fn = __DIR__ .'/classes/'.(strpos(self::GetModuleId(), 'kda.')===0 ? 'general' : 'import').'/profiles_ext/importer'.(int)$pid.'.php';
			if(file_exists($fn))
			{
				include_once($fn);
				$ie = new ('CKDAImportExcel'.(int)$pid)($filename, $params, $fparams, $stepparams, $pid);
			}
			else $ie = new CKDAImportExcel($filename, $params, $fparams, $stepparams, $pid);
			return $ie->Import();
		}
		...
		{
			if(self::DemoExpired()) return array();
			$fn = __DIR__ .'/classes/'.(strpos(self::GetModuleId(), 'kda.')===0 ? 'general' : 'import').'/profiles_ext/importer'.(int)$pid.'_hl.php';
			if(file_exists($fn))
			{
				include_once($fn);
				$ie = new ('CKDAImportExcelHl'.(int)$pid)($filename, $params, $fparams, $stepparams, $pid);
			}
			else $ie = new CKDAImportExcelHighload($filename, $params, $fparams, $stepparams, $pid);
			return $ie->Import();
		}
		...
		{
			if(self::DemoExpired()) return array();
			$fn = __DIR__ .'/classes/'.(strpos(self::GetModuleId(), 'kda.')===0 ? 'general' : 'export').'/profiles_ext/exporter'.(int)$pid.'.php';
			if(file_exists($fn))
			{
				include_once($fn);
				$ee = new ('CKDAExportExcel'.(int)$pid)($params, $fparams, $stepparams, $pid);
			}
			else $ee = new CKDAExportExcel($params, $fparams, $stepparams, $pid);
			return $ee->Export();
		}
		...
		{
			if(self::DemoExpired()) return array();
			$fn = __DIR__ .'/classes/'.(strpos(self::GetModuleId(), 'kda.')===0 ? 'general' : 'export').'/profiles_ext/exporter'.(int)$pid.'_hl.php';
			if(file_exists($fn))
			{
				include_once($fn);
				$ee = new ('CKDAExportExcelHl'.(int)$pid)($params, $fparams, $stepparams, $pid);
			}
			else $ee = new CKDAExportExcelHighload($params, $fparams, $stepparams, $pid);
			return $ee->Export();
		}
	}
}
