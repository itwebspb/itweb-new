...
class Thematics
{
    public const ASPRO_MODULE_NAME = 'aspro.sku';
    public const ASPRO_REP_URL = 'https://theme.aspro.ru/';
    public const BETA_OPTION_CODE = 'SKU_BETA_UPDATES';
    ...
    public static function clear(array $arParams = []): bool
    {
        ...
        $defaultZipPath = $_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/'.static::ASPRO_MODULE_NAME.'/tmp_zip_files';
    ...
    public static function copy(array $arParams = []): bool
    {
        ...
        $defaultZipPath = $_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/'.static::ASPRO_MODULE_NAME.'/tmp_zip_files/files/'.$moduleID;
        $zipDir = is_string($zipDir) && strlen($zipDir) ? $zipDir : $defaultZipPath;
    ...
    public static function check(array $arParams = []): array
    {
        ...
        $moduleID = $arParams['moduleID'] ?? static::ASPRO_MODULE_NAME;
        $thematic = $arParams['thematic'] ?? 'universal';
        $bSeparateModule = $arParams['bSeparateModule'] ?? false;
        $bUpdate = $arParams['bUpdate'] ?? false;

        if ($bUpdate && Option::get(static::ASPRO_MODULE_NAME, static::BETA_OPTION_CODE, 'N') === 'Y') {
            $thematic = 'beta';
        }

        unset($_SESSION[$templateID]);

        if (!Loader::includeModule(static::ASPRO_MODULE_NAME)) {
            throw new \Exception('Failed to include the module '.static::ASPRO_MODULE_NAME);
        }

        $obModule = \CModule::CreateModuleObject(static::ASPRO_MODULE_NAME);
        if (!$obModule) {
            throw new \Exception('Failed to instantiate module '.static::ASPRO_MODULE_NAME);
        }
        ...
        $arData = [
            'd' => Tools::___1596018847($arData, $key),
            'k' => $key,
        ];
        ...
        if ($httpClient->query(HttpClient::HTTP_POST, static::ASPRO_REP_URL, $arData)) {
            // no request error
    ...
    public static function download(array $arParams = []): array
    {
        ...
        if ($arDownloadFile) {
            ...
            $url = static::ASPRO_REP_URL.'?d='.$arData['d'].'&k='.urlencode($arData['k']);
            $arUrl = parse_url($url);
