...
class CMax
{
                                ...
                                <p class="buttons_block">
                                    <?if($bLanding):?>
                                       <?if($arResult['PROPERTIES']['BUTTON_TEXT']['VALUE']):?>
                                            <span>
                                                <?if($arResult['PROPERTIES']['BTN_URL_STRING']['VALUE']):?>
                                                    <a href="<?=$arResult['PROPERTIES']['BTN_URL_STRING']['VALUE']?>" target="<?=$arResult['PROPERTIES']['BTN_TARGETS']['VALUE_XML_ID'] ?? "_blank"?>" <?=$arResult['PROPERTIES']['BTN_NOFOLLOW']['VALUE'] == "Y" ? 'rel="nofollow"' : '';?>>
                                                <?endif;?>
    ...
    public static function GetFrontParametrsValues($SITE_ID = SITE_ID, $SITE_DIR = SITE_DIR){
        ...
        if (
            ...
            if (isset($arBackParametrs['THEME_SWITCHER'])) {
    ...
    public static function showContactPhones($txt = '', $wrapTable = true, $class = '', $icon = 'Phone_black2.svg', $subclass = ''){
        ...
        $bRegionContact = static::getFrontParametrValue('SHOW_REGION_CONTACT') == 'Y';
        ...
        if ($arRegion && $bRegionContact) {
            for ($i = 0; $i < $iCountPhones; ++$i) {
                $phone = $arRegion['PHONES'][$i]['PHONE'];
                $href = $arRegion['PHONES'][$i]['HREF'];
                if (!strlen($href)) {
                    $href = 'javascript:;';
                }
                $description = $arRegion['PROPERTY_PHONES_DESCRIPTION'][$i] ?? '';
                $descriptionAttr = $description !== '' ? ' title="'.SolutionUtils::sanitizeValue($description).'"' : '';

                $arPhonesList[] = '<a'.$descriptionAttr.' href="'.SolutionUtils::sanitizeValue($href).'">'.SolutionUtils::sanitizeValue($phone).'</a>';
            }
        }

        if (!$arPhonesList && self::checkContentFile(SITE_DIR.'include/contacts-site-phone-one.php')) {
            ob_start();
                $APPLICATION->IncludeFile(SITE_DIR."include/contacts-site-phone-one.php", Array(), Array("MODE" => "html", "NAME" => "Phone"));
            $phone = trim(ob_get_clean());
            if ($phone) {
                $arPhonesList = [SolutionUtils::sanitizeHtml($phone)];
            }
        }
        ...
        <?if($arPhonesList):?>
            <div class="property phone">
                ...
                <?=$bRegionContact ? '<div class="'.($class ? ' '.$class : '').'">' : '';?>
                    <?foreach($arPhonesList as $value):?>
                        <div class="value darken" itemprop="telephone"><?=$value?></div>
                    <?endforeach;?>
                <?=$bRegionContact ? '</div>' : '';?>
            </div>
        <?endif;?>
    ...
    public static function showContactEmail($txt = '', $wrapTable = true, $class = '', $icon = 'Email.svg', $subclass = ''){
        ...
        if ($arRegion && $bRegionContact) {
            if (!empty($arRegion['PROPERTY_EMAIL_VALUE'])) {
                $arEmailsList = (array)$arRegion['PROPERTY_EMAIL_VALUE'];
            }
        }

        if (!$arEmailsList && self::checkContentFile(SITE_DIR.'include/contacts-site-email.php')) {
            ob_start();
                $APPLICATION->IncludeFile(SITE_DIR.'include/contacts-site-email.php', [], ['MODE' => 'html', 'NAME' => 'email']);
            $email = trim(ob_get_clean());
            if ($email) {
                $arEmailsList = [$email];
            }
        }
        ...
        <?if($arEmailsList):?>
            <div class="property email">
                <div class="title font_upper muted"><?=($txt ? $txt : Loc::getMessage('SPRAVKA'));?></div>
                <?=$bRegionContact ? '<div class="'.($class ? ' '.$class : '').'">' : '';?>
                    <?foreach($arEmailsList as $value):?>
                        <div class="value darken" itemprop="email">
                            <a href='mailto:<?=SolutionUtils::sanitizeHtml($value);?>'><?=SolutionUtils::sanitizeHtml($value);?></a>
                        </div>
                    <?endforeach;?>
                <?=$bRegionContact ? '</div>' : '';?>
            </div>
    ...
    public static function showContactAddr($txt = '', $wrapTable = true, $class = '', $icon = 'Addres_black.svg', $subclass = '')
    {
        ...
        if ($arRegion) {
            $bRegionContact = static::getFrontParametrValue('SHOW_REGION_CONTACT') == 'Y';
            if ($bRegionContact) {
                $address = trim($arRegion['PROPERTY_ADDRESS_VALUE']['TEXT'] ?? '');
            }
        }

        if (!$address && self::checkContentFile(SITE_DIR.'include/contacts-site-address.php')) {
            ob_start();
                $APPLICATION->IncludeFile(SITE_DIR.'include/contacts-site-address.php', [], ['MODE' => 'html', 'NAME' => 'address']);
            $address = trim(ob_get_clean());
        }
        ...
        <?if ($address):?>
            <div class="property address">
                ...
                <div itemprop="address" class="value darken">
                    <?=SolutionUtils::sanitizeHtml($address);?>
                </div>
            </div>
        <?endif;?>
        <?if($arRegion):?>
    ...
    public static function showContactDesc(){
        ...
        if ($arRegion && $bRegionContact) {
            $desc = trim($arRegion['PROPERTY_REGION_TAG_CONTACT_TEXT_VALUE']['TEXT'] ?? '');
        }

        if (!$desc && self::checkContentFile(SITE_DIR.'include/contacts-about.php')) {
            ob_start();
                $APPLICATION->IncludeFile(SITE_DIR.'include/contacts-about.php', [], ['MODE' => 'html', 'NAME' => 'Contacts about']);
        ...
        <?if($desc):?>
            <div itemprop="description" class="previewtext muted777">
                <?=SolutionUtils::sanitizeHtml($desc);?>
            </div>
        <?endif;?>
        <?if($arRegion):?>
            <?$frame->end();?>
    ...
    public static function showContactSchedule($txt = '', $wrapTable = true, $class = '', $icon = 'WorkingHours_lg.svg', $subclass = '')
    {
        ...
        if ($arRegion) {
            $bRegionContact = static::getFrontParametrValue('SHOW_REGION_CONTACT') == 'Y';
            if ($bRegionContact) {
                $schedule = trim($arRegion['PROPERTY_REGION_TAG_SHEDULLE_VALUE']['TEXT'] ?? '');
            }
        }

        if (!$schedule && self::checkContentFile(SITE_DIR.'include/contacts-site-schedule.php')) {
            ob_start();
                $APPLICATION->IncludeFile(SITE_DIR."include/contacts-site-schedule.php", Array(), Array("MODE" => "html", "NAME" => "schedule"));
            $schedule = trim(ob_get_clean());
        }
        ?>
        <?if($arRegion):?>
            <?$frame = new \Bitrix\Main\Page\FrameHelper('header-allcaddr-block'.$iCalledID);?>
        ...
        <?if($schedule):?>
                    <div class="property schedule">
                        ...
                        <div class="value darken">
                            <?=SolutionUtils::sanitizeHtml($schedule);?>
                        </div>
                    </div>
        <?endif;?>
        <?if($arRegion):?>
    ...
    public static function ShowMobileMenuContacts(){
        ...
        <?if($iCountPhones):?>
            ...
            $description = (!empty($description)) ? '<span class="phones__phone-descript"><span class="descr">' . $description . '</span></span>' : '';
            ...
            <div class="menu middle mobile-menu-contacts">
                <ul>
                    <li>
                        <a rel="nofollow" href="<?=($iCountPhones > 1 ? '' : SolutionUtils::sanitizeHtml($href))?>" class="dark-color<?=($iCountPhones > 1 ? ' parent' : '')?> <?=(empty($description)?'no-decript':'decript')?>">
                            ...
                            <span><?=SolutionUtils::sanitizeHtml($phone)?><?=SolutionUtils::sanitizeHtml($description)?></span>
                            <?if($iCountPhones > 1):?>
                                <span class="arrow">
                        ...
                        <?if($iCountPhones > 1): // if more than one?>
                            <ul class="phone dropdown <?=($bHaveIcons ? 'with_icons' : '')?>">
                                ...
                                <?for($i = 0; $i < $iCountPhones; ++$i):?>
                                    ...
                                    $messengerNames = ['TELEGRAM', 'WHATSAPP'];
                                    $messengerIcons = '';

                                    foreach ($messengerNames as $name) {
                                        $iconPath = "/bitrix/templates/aspro_max/images/svg/social/phones_" . strtolower($name) . ".svg";
                                        $link  = match($name) {
                                            'TELEGRAM' => 'https://t.me/',
                                            'WHATSAPP' => 'https://wa.me/',
                                        };

                                        $bShowMessenger = $arRegion
                                            ? ($arRegion['PHONES'][$i][$name] ?? '') === 'Y'
                                            : ($arBackParametrs['HEADER_PHONES_array_PHONE_MESSENGER_'.$name.'_'.$i] ?? '');

                                        if ($bShowMessenger) {
                                            $messengerIcons .= '<a href="'.$link.substr($href, 4).'" target="_blank" class="phone-messenger">'
                                                .'<span class="icon">'.\Aspro\Max\Iconset::showIcon($iconPath, false, false).'</span>'
                                                .'</a>';
                                        }
                                    }?>
                                    ...
                                    <li>
                                        <div class="more_phone flexbox flexbox--row flexbox--gap flexbox--gap-16">
                                            <a class="more_phone_a dark-color <?= strlen($description) ? '' : 'no-decript' ?> flexbox flexbox--row flexbox--gap flexbox--gap-8" rel="nofollow" href="<?=SolutionUtils::sanitizeHtml($href)?>">
                                                <span class="phones__phone-icon <?= strlen($icon) ? 'has_icon' : 'no-icon' ?>"><?=$icon?></span>
                                                <span class="phones__phone-link-text flexbox dropdown--top-reverse flexbox--gap flexbox--gap-6">
                                                    <span class="phones__phone-title"><?=SolutionUtils::sanitizeHtml($phone)?></span>
                                                    <?if (!empty($description)):?>
                                                        <span class="phones__phone-descript"><?=SolutionUtils::sanitizeHtml($description)?></span>
                                                    <?endif;?>
                                                </span>
                                            </a>
                                            <?if (!empty($messengerIcons)):?>
            ...
            <?if($arRegion):?>
                <?if($arRegion['PROPERTY_ADDRESS_VALUE']):?>
                    <div class="address">
                        ...
                        <?=SolutionUtils::sanitizeHtml($arRegion['PROPERTY_ADDRESS_VALUE']['TEXT'] ?? '');?>
                    </div>
                <?endif;?>
            <?else:?>
                <div class="address">
                    ...
                    echo SolutionUtils::sanitizeHtml(ob_get_clean());
                    ?>
                </div>
            <?endif;?>
            <?if($arRegion):?>
                <?if($arRegion['PROPERTY_EMAIL_VALUE']):?>
                    <div class="email">
                        ...
                        <?foreach($arRegion['PROPERTY_EMAIL_VALUE'] as $value):?>
                            <a href="mailto:<?=SolutionUtils::sanitizeValue($value);?>"><?=SolutionUtils::sanitizeValue($value);?></a>
                        <?endforeach;?>
                    </div>
                <?endif;?>
            <?else:?>
    ...
    public static function showThemeSelector(array $options = [])
    {
        $file = Application::getDocumentRoot().SITE_DIR.'include/header_include/theme.selector.php';
        ...
        if($arBackParametrs['THEME_SWITCHER'] === 'Y'){
            if($_REQUEST && isset($_REQUEST['BASE_COLOR']))
            {
                if($_REQUEST['THEME'] === 'default')
                {
                    ...
                    if ($backurl !== '' && $backurl[0] === '/' && substr($backurl, 0, 2) !== '//') {
                        LocalRedirect($backurl);
                    }
                }

                $baseColor = $_REQUEST['BASE_COLOR'] ?? '';
                if ($baseColor) {
                    LocalRedirect($GLOBALS['APPLICATION']->GetCurPageParam('', ['color_theme', 'BASE_COLOR']));
                }
    ...
    public static function Start($siteID)
    {
        ...
        if (isset($_REQUEST['color_theme']) && $_REQUEST['color_theme']) {
            LocalRedirect($GLOBALS['APPLICATION']->GetCurPageParam('', ['color_theme']));
        }
    ...
    protected static function addExtensionsByCondition(array $arTheme)
    {
        ...
        if ($arTheme['USE_EXTENSION_VALIDATION'] === 'Y') {
            ...
            $APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/conditional/validation.js');
    ...
    public static function ShowLogo($arParams = array()){
        ...
        $arImg = self::unserialize(
            Option::get(self::moduleID, $logoOptionCode, serialize(array()))
        ) ?: (
            $isWhite
                ? self::unserialize(Option::get(self::moduleID, 'LOGO_IMAGE', serialize(array())))
                : array()
        );
        ...
        if($arImg) {
            $logo .= '<img src="'.CFile::GetPath($arImg[0]).'" alt="'.$arSite["SITE_NAME"].'" title="'.$arSite["SITE_NAME"].'" />';
        }
        elseif(self::checkContentFile(SITE_DIR.'/include/logo_svg.php')) {
            $logo .= File::getFileContents($_SERVER['DOCUMENT_ROOT'].SITE_DIR.'/include/logo_svg.php');
        }
        else {
            $logo .= '<img src="'.$arTheme[$logoOptionCode].'" alt="'.$arSite["SITE_NAME"].'" title="'.$arSite["SITE_NAME"].'" />';
        }

        $isMainPage = ($APPLICATION->GetCurPage(false) === SITE_DIR);

        if(!$isMainPage){
            return '<a href="'.SITE_DIR.'">'.$logo.'</a>';
        }
    ...
    public static function ShowLogoFixed($arParams = array()){
        ...
        if($arImg)
            $logo .= '<img src="'.CFile::GetPath($arImg[0]).'" alt="'.$arSite["SITE_NAME"].'" title="'.$arSite["SITE_NAME"].'" data-src="" />';
        elseif(self::checkContentFile(SITE_DIR.'/include/logo_svg.php'))
            $logo .= File::getFileContents($_SERVER['DOCUMENT_ROOT'].SITE_DIR.'/include/logo_svg.php');
        else
            $logo .= '<img src="'.$arTheme[$logoOptionCode].'" alt="'.$arSite["SITE_NAME"].'" title="'.$arSite["SITE_NAME"].'" data-src="" />';
    ...
    public static function showIconSvg($class = 'phone', $path, $title = '', $class_icon = '', $show_wrapper = true, $binline = true){
        ...
        if(self::checkContentFile($path))
        {
            ...
            $content = SvgSprite::getSanitizedContent(SvgSprite::getFullPath($path));
    ...
    public static function showSpriteIconSvg($fullPath, $class = '', $arOptions = []) {
        ...
        $path = $fullPath;
        if(strpos($fullPath, '#') && $hash = substr($fullPath, strpos($fullPath, '#'))) {
            $path = str_replace($hash, '', $fullPath);
        }

        $fileChangeTime = filemtime($_SERVER['DOCUMENT_ROOT'].$path);

        return '<i class="svg '.$class.'" aria-hidden="true"><svg '.$width.' '.$height.' '.$viewbox.'><use xlink:href="'.$path.'?'.$fileChangeTime.$hash.'"></use></svg></i>';
    }
    ...
    public static function showPriceMatrix($arItem = array(), $arParams = array(), $strMeasure = '', $arAddToBasketData = array()){
        $html = '';
        if(
                                    ...
                                    <?if($arPrice["PRICE"] > $arPrice["DISCOUNT_PRICE"]){?>
                                        <div class="price font-bold <?=(($iCountPriceInterval > 1) ? 'font_xs' : ($arParams['MD_PRICE'] ? 'font_mlg' : 'font_mxs'));?>" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["DISCOUNT_PRICE"];?>">
                                            <?if(strlen($arPrice["DISCOUNT_PRICE"])):?>
                                                ...
                                                <span class="values_wrapper"><?=\Aspro\Functions\CAsproMaxItem::getCurrentPrice("DISCOUNT_PRICE", $arPrice, bUseSchema: $arParams['USE_SCHEMA'] && $bMinPrice && !$bShowPopupPrice);?></span><?if(($arParams["SHOW_MEASURE"]=="Y") && $strMeasure && $arPrice["DISCOUNT_PRICE"]):?><span class="price_measure">/<?=$strMeasure?></span><?endif;?>
                                            <?endif;?>
                                        </div>
                                    ...
                                    <?}else{?>
                                        <div class="price font-bold <?=(($iCountPriceInterval > 1) ? 'font_xs' : ($arParams['MD_PRICE'] ? 'font_mlg' : 'font_mxs dd'));?>" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["DISCOUNT_PRICE"];?>">
                                            <? if($arPrice['DISCOUNT_PRICE'] || $sMissingGoodsPriceDisplay === 'PRICE'): ?>
                                                <?if($arItem['SHOW_FROM_LANG'] == 'Y'):?><span><?=Loc::getMessage('FROM')?></span><?endif;?>
                                                <span><span class="values_wrapper"><?=\Aspro\Functions\CAsproMaxItem::getCurrentPrice("PRICE", $arPrice, bUseSchema: $arParams['USE_SCHEMA'] && $bMinPrice && !$bShowPopupPrice);?></span><?if(($arParams["SHOW_MEASURE"]=="Y") && $strMeasure && $arPrice["PRICE"]):?><span class="price_measure">/<?=$strMeasure?></span><?endif;?></span>
                                            <? elseif($sMissingGoodsPriceDisplay === 'TEXT'): ?>
                                                <?if($arParams['USE_SCHEMA']):?>
                                                    <?=Price::showPriceWithSchema();?>
                                                <?endif;?>
                                ...
                                <?if($bOtherPrice):?>
                                    <div class="price font-bold font_mxs">
                                        <div class="price_value_block values_wrapper">
                                            <?=\Aspro\Functions\CAsproMaxItem::getCurrentPrice("VALUE", $arItem['MIN_PRICE'], bUseSchema: $arParams['USE_SCHEMA']);?></div><?if (($arParams['SHOW_MEASURE'] == 'Y') && $arSetItem["MEASURE"][$arSetItem["ID"]]["MEASURE"]["SYMBOL_RUS"]):?><span class="price_measure">/<?=$arSetItem["MEASURE"][$arSetItem["ID"]]["MEASURE"]["SYMBOL_RUS"]?></span><?endif;?>
                                    </div>
                                ...
                                <?else:?>
                                    <div class="price font-bold font_mxs">
                                        <?if($arItem['ITEM_PRICES'][$priceSelected]["PRICE"] || $sMissingGoodsPriceDisplay === 'PRICE'):?>
                                            <div class="price_value_block values_wrapper">
                                                <?if($arItem['SHOW_FROM_LANG'] == 'Y'):?><span><?=Loc::getMessage('FROM')?></span><?endif;?>
                                                <?=\Aspro\Functions\CAsproMaxItem::getCurrentPrice("PRICE", $arItem['ITEM_PRICES'][$priceSelected], bUseSchema: $arParams['USE_SCHEMA']);?></div><?if (($arParams['SHOW_MEASURE'] == 'Y') && $arItem['CATALOG_MEASURE_NAME'] && $arItem['ITEM_PRICES'][$priceSelected]["PRICE"]):?><span class="price_measure">/<?=$arItem['CATALOG_MEASURE_NAME']?></span><?endif;?>
                                        <? elseif($sMissingGoodsPriceDisplay === 'TEXT'): ?>
                                            <?if($arParams['USE_SCHEMA']):?>
                                                <?=Price::showPriceWithSchema();?>
                                            <?endif;?>
                                            <?=$sMissingGoodsPriceDisplayText; ?>
                                        <? elseif($arParams['USE_SCHEMA']):?>
                                            <?=Price::showPriceWithSchema();?>
                                        <? endif; ?>
                                    </div>
    ...
    public static function GetQuantityArray(
        ...
        if($arQuantityRights["SHOW_QUANTITY"]){
            ...
            if($totalCount > 0)
            {
                ...
                $totalText = "<span class='store_view " . ($useStoreClick == "Y" ? 'dotted' : '') . "'>" .
                    $arQuantityOptions["EXPRESSION_FOR_EXISTS"] .
                    (!$arQuantityRights["SHOW_QUANTITY_COUNT"] ? $storesAmountHtml : '') .
                    '</span>';
                    }
            else
            {
            ...
            if($arQuantityRights["SHOW_QUANTITY_COUNT"] && $totalCount > 0)
            {
                if($arQuantityOptions["USE_WORD_EXPRESSION"] == "Y")
                {
                    ...
                    {
                        $totalText = "<span class='store_view " . ($useStoreClick == "Y" ? 'dotted' : '') . "'>".$totalAmount.'</span>'.$storesAmountHtml;
                    }
                }
                else
                {
                    $totalText .= (strlen($totalText) ? ": " : ($useStoreClick == "Y" ? "<span class='store_view dotted'>" : "")).
                        $totalCount . $storesAmountHtml.
                        (strlen($totalText) || $useStoreClick != "Y" ? "" : "</span>");
                }
            }
    ...
    public static function GetAddToBasketArray(&$arItem, $totalCount = 0, $defaultCount = 1, $basketUrl = '', $bDetail = false, $arItemIDs = array(), $class_btn = "small", $arParams=array()){
        ...
        $class_btn = htmlspecialcharsbx($class_btn);
        ...
        $partProp = htmlspecialcharsbx($arParams['PARTIAL_PRODUCT_PROPERTIES'] ?: '');
        $addProp = htmlspecialcharsbx($arParams['ADD_PROPERTIES_TO_BASKET'] ?: '');
        ...
        if ($bShowOutOfProduction) {
            ...
            if (isset($arItem['PROPERTIES']['PRODUCT_ANALOG_FILTER']) && $arItem['PROPERTIES']['PRODUCT_ANALOG_FILTER']['VALUE']) {
                ...
                $buttonHTML = "<a href='".htmlspecialcharsbx($arItem['PROPERTIES']['PRODUCT_ANALOG_FILTER']['VALUE'])."' class='btn btn-default btn-wide transition_bg catalog-item-analog-filter ".$class_btn." has-ripple btn--wrap-text fill-use-999' title='".htmlspecialcharsbx($buttonTEXT)."'>".(!$bDetail ? self::showSpriteIconSvg(SITE_TEMPLATE_PATH."/images/svg/catalog/catalog_icons.svg#similar-19-16", "", ['WIDTH' => 19,'HEIGHT' => 16]) : '')."<span>".htmlspecialcharsbx($buttonTEXT)."</span></a>";
            }elseif (!$bDetail) {
            ...
            if(!$bDetail && $arItem["OFFERS_MORE"] != "Y" && $type_sku != "TYPE_2"){
                ...
                $buttonHTML = '<span class="btn btn-default transition_bg '.$class_btn.' read_more1 to-cart animate-load" id="'.$arItemIDs['BUY_LINK'].'" data-offers="N" data-iblockID="'.$arItem["IBLOCK_ID"].'" data-item="'.$arItem["ID"].'">'.self::showIconSvg("fw ncolor colored", SITE_TEMPLATE_PATH."/images/svg/basket.svg", $buttonText[0]).'<span>'.$buttonText[0].'</span></span><a rel="nofollow" href="'.htmlspecialcharsbx($basketUrl).'" id="'.$arItemIDs['BASKET_LINK'].'" class="'.$class_btn.' in-cart btn btn-default transition_bg" data-item="'.$arItem["ID"].'"  style="display:none;">'.self::showIconSvg("fw ncolor colored", SITE_TEMPLATE_PATH."/images/svg/inbasket.svg", $buttonText[1]).'<span>'.$buttonText[1].'</span></a>';
            }
            ...
            if($priceValue > 0){
                ...
                if($totalCount > 0 && (isset($arItem["CAN_BUY"]) && $arItem["CAN_BUY"])){
                    ...
                    if((isset($arItem["CAN_BUY"]) && $arItem["CAN_BUY"])){
                        if($bDetail && $frontCatalog == "Y"){
                            ...
                            $buttonACTION = 'ADD';
                            $buttonText = array($arAddToBasketOptions['EXPRESSION_ADDTOBASKET_BUTTON_DEFAULT'], $arAddToBasketOptions['EXPRESSION_ADDEDTOBASKET_BUTTON_DEFAULT']);
                            $buttonHTML = '<span data-value="'.($minPrice["DISCOUNT_VALUE"] ?? $minPrice["PRICE"]).'" data-currency="'.$minPrice["CURRENCY"].'" class="'.$class_btn.' to-cart btn btn-default transition_bg animate-load" data-item="'.$arItem['ID'].'" data-float_ratio="'.htmlspecialcharsbx($float_ratio).'" data-ratio="'.$ratio.'" data-bakset_div="bx_basket_div_'.$arItem['ID'].(isset($arItemIDs['DOP_ID']) && $arItemIDs['DOP_ID'] ? '_'.$arItemIDs['DOP_ID'] : '').'" data-props="'.htmlspecialcharsbx($arItemProps).'" data-part_props="'.$partProp.'" data-add_props="'.htmlspecialcharsbx($addProp).'"  data-empty_props="'.htmlspecialcharsbx($emptyProp).'" data-offers="'.$isOffer.'" data-iblockID="'.$arItem["IBLOCK_ID"].'"  data-quantity="'.$quantity.'">'.self::showIconSvg("fw ncolor colored", SITE_TEMPLATE_PATH."/images/svg/basket.svg", $buttonText[0]).'<span>'.htmlspecialcharsbx($buttonText[0]).'</span></span><a rel="nofollow" href="'.htmlspecialcharsbx($basketUrl).'" class="'.$class_btn.' in-cart btn btn-default transition_bg" data-item="'.$arItem['ID'].'"  style="display:none;">'.self::showIconSvg("fw ncolor colored", SITE_TEMPLATE_PATH."/images/svg/inbasket.svg", $buttonText[1]).'<span>'.htmlspecialcharsbx($buttonText[1]).'</span></a>';
                        }
                    }
                    ...
                    if($bDetail && $frontCatalog == "Y"){
                        ...
                        if($arAddToBasketOptions["BUYMISSINGGOODS"] == "ADD" /*|| $arItem["CAN_BUY"]*/){
                            if($arItem["CAN_BUY"]){
                                $buttonText = array($arAddToBasketOptions['EXPRESSION_ADDTOBASKET_BUTTON_DEFAULT'], $arAddToBasketOptions['EXPRESSION_ADDEDTOBASKET_BUTTON_DEFAULT']);
                                $buttonHTML = '<span data-value="'.($minPrice["DISCOUNT_VALUE"] ?? minPrice["PRICE"]).'" data-currency="'.$minPrice["CURRENCY"].'" class="'.$class_btn.' to-cart btn btn-default transition_bg animate-load" data-item="'.$arItem['ID'].'" data-float_ratio="'.$float_ratio.'" data-ratio="'.$ratio.'" data-bakset_div="bx_basket_div_'.$arItem['ID'].'" data-props="'.htmlspecialcharsbx($arItemProps).'" data-part_props="'.$partProp.'" data-add_props="'.$addProp.'"  data-empty_props="'.$emptyProp.'" data-offers="'.htmlspecialcharsbx($arItem["IS_OFFER"]).'" data-iblockID="'.htmlspecialcharsbx($arItem["IBLOCK_ID"]).'" data-quantity="'.$quantity.'">'.self::showIconSvg("fw ncolor colored", SITE_TEMPLATE_PATH."/images/svg/basket.svg", $buttonText[0]).'<span>'.$buttonText[0].'</span></span><a rel="nofollow" href="'.htmlspecialcharsbx($basketUrl).'" class="'.$class_btn.' in-cart btn btn-default transition_bg" data-item="'.$arItem['ID'].'"  style="display:none;">'.self::showIconSvg("fw ncolor colored", SITE_TEMPLATE_PATH."/images/svg/inbasket.svg", $buttonText[1]).'<span>'.$buttonText[1].'</span></a>';
                            }else{
                ...
                if($bDetail && $frontCatalog == "Y"){
                    ...
                    if($arAddToBasketOptions["BUYNOPRICEGGOODS"] == "ORDER"){
                        ...
                        $buttonHTML = '<span class="'.$class_btn.' to-order btn btn-default animate-load" data-event="jqm" data-param-form_id="TOORDER" data-name="toorder" data-autoload-product_name="'.self::formatJsName($arItem["NAME"]).'" data-autoload-product_id="'.($arItem['ID']).'">'.self::showIconSvg("fw ncolor colored", SITE_TEMPLATE_PATH."/images/svg/mail_c.svg", (htmlspecialcharsbx($buttonText[0]))).'<span>'.htmlspecialcharsbx($buttonText[0]).'</span></span>';
                        if($arAddToBasketOptions['EXPRESSION_ORDER_TEXT']){
                            $buttonHTML .='<div class="more_text">'.$arAddToBasketOptions['EXPRESSION_ORDER_TEXT'].'</div>';
                        }
                    }
    ...
    */
    ...
    public static function getCanBuyRegion(array $arItem, float $totalCount = 0): bool
    {
        return ($totalCount && ($arItem['OFFERS'] || $arItem['CAN_BUY']))
            || (
                (!$totalCount && $arItem['CATALOG_QUANTITY_TRACE'] == 'N' )
                || (!$totalCount && $arItem['CATALOG_QUANTITY_TRACE'] == 'Y' && $arItem['CATALOG_CAN_BUY_ZERO'] == 'Y')
            );
    }
            ...
            <?if($arRegion):?>
                <?if($arRegion['PROPERTY_EMAIL_VALUE']):?>
                    <div <?=($class ? 'class="'.$class.'"' : '')?>>
                        ...
                        <?foreach($arRegion['PROPERTY_EMAIL_VALUE'] as $value):?>
                            <a href="mailto:<?=SolutionUtils::sanitizeValue($value);?>" target="_blank"><?=SolutionUtils::sanitizeValue($value);?></a>
                        <?endforeach;?>
                    </div>
                <?endif;?>
            <?else:?>
                ...
                <div class="phone wrap">
                    ...
                    <?for($i = 0; $i < $iCountPhones; ++$i):?>
                        ...
                        foreach ($messengerNames as $name) {
                            ...
                            if ($bShowMessenger) {
                                $messengerIcons .= '<a href="'.$link.substr($href, 4).'" target="_blank">'
                                    .'<span class="icon">'.\Aspro\Max\Iconset::showIcon($iconPath, false, false).'</span>'
                                    .'</a>';
                            }
                        }?>
                        ...
                        <div class="more_phone flexbox flexbox--row flexbox--gap flexbox--gap-16">
                            <a class="more_phone_a dark-color <?= strlen($description) ? '' : 'no-decript' ?> flexbox flexbox--row flexbox--gap flexbox--gap-8" rel="nofollow" href="<?=SolutionUtils::sanitizeHtml($href)?>">
                                <span class="phones__phone-icon <?= strlen($icon) ? 'has_icon' : 'no-icon' ?>">
                                    <?=$icon?>
                                </span>
            ...
            <div class="phone with_dropdown<?=($class ? ' '.$class : '')?>">
                <?if($bFooter):?>
                    <div class="wrap">
                        <div>
                <?endif;?>
                    <?=static::showSpriteIconSvg(SITE_TEMPLATE_PATH."/images/svg/header_icons_srite.svg".($bFooter ? "#phone_footer" : "#phone_black"), "svg-inline-phone ", ['WIDTH' => 5,'HEIGHT' => 13]);?><a rel="nofollow" href="<?=SolutionUtils::sanitizeHtml($href)?>"><?=SolutionUtils::sanitizeHtml($phone)?></a>
                <?if($bFooter):?>
                        </div>
                    </div>
                <?endif;?>
                    ...
                    <div class="dropdown <?=($bHaveIcons ? 'with_icons' : '')?>">
                        <div class="wrap scrollblock">
                            <?for($i = 0; $i < $iCountPhones; ++$i):?>
                                ...
                                $icon = ($arRegion ? $arRegion['PHONES'][$i]['ICON'] : $arBackParametrs['HEADER_PHONES_array_PHONE_ICON_'.$i]);
                                $icon = (strlen($icon) ? \Aspro\Max\Iconset::showIcon($icon, false, false) : '');
                                ...
                                <div class="more_phone flexbox flexbox--row flexbox--gap flexbox--gap-16">
                                    <a class="more_phone_a <?= strlen($description) ? '' : 'no-decript' ?> flexbox flexbox--row flexbox--gap flexbox--gap-8" rel="nofollow" href="<?=SolutionUtils::sanitizeHtml($href)?>">
                                        <span class="phones__phone-icon <?= strlen($icon) ? 'has_icon' : 'no-icon' ?>">
                                            <?=$icon?>
                                        </span>
                                        <span class="phones__phone-link-text flexbox dropdown--top-reverse flexbox--gap flexbox--gap-8">
    ...
    public static function showFooterPhone(){
        ...
        <?if($iCountPhones):?>
            ...
            <span class="phone_wrap">
                <span>
                    <a href="<?=SolutionUtils::sanitizeHtml($href)?>" rel="nofollow"><?=SolutionUtils::sanitizeHtml($phone)?></a>
                </span>
            </span>
        <?endif;?>
        <?
    ...
    public static function prepareItemMapHtml($arShop, $bStore = "N", $arParams = array(), $bBigBlock = "N"){
        ...
        if($arParams['TITLE_BLOCK_DETAIL_NAME'])
            $html .= '<div class="subtitle font_upper muted">'.SolutionUtils::sanitizeValue($arParams['TITLE_BLOCK_DETAIL_NAME']).'</div>';

        $html .= '<div class="title option-font-bold '.($bBigBlock != 'Y' ? 'font_mlg' : 'font_exlg').'">'.(strlen($arShop["URL"]) ? '<a class="dark_link" href="'.$arShop["URL"].'">' : '').SolutionUtils::sanitizeHtml(htmlspecialcharsBack($arShop["ADDRESS"])).(strlen($arShop["URL"]) ? '</a>' : '').'</div>';
        if($arShop['METRO'] || $arShop['SCHEDULE'] || $arShop['EMAIL'] || $arShop['PHONE'] || isset($arShop['QUANTITY'])){
            ...
            $html .= '<div class="properties">';
                ...
                $html .= ($arShop['METRO'] ? '<div class="property schedule"><div class="title-prop font_upper muted">'.($bStore == 'Y' ? Loc::getMessage('CONTACT_METRO') : $arShop['DISPLAY_PROPERTIES']['METRO']['NAME']).'</div><div class="value">'.SolutionUtils::sanitizeValue(htmlspecialcharsBack($arShop['METRO_PLACEMARK_HTML'])).'</div></div>' : '');
                $html .= (strlen($arShop['SCHEDULE']) ? '<div class="property schedule"><div class="title-prop font_upper muted">'.($bStore == 'Y' ? Loc::getMessage('CONTACT_SCHEDULE') : $arShop['DISPLAY_PROPERTIES']['SCHEDULE']['NAME']).'</div><div class="value">'.SolutionUtils::sanitizeHtml(htmlspecialcharsBack($arShop['SCHEDULE'])).'</div></div>' : '');

                if($arShop['PHONE']){
                    $phone = '';
                    if(is_array($arShop['PHONE'])){
                        foreach($arShop['PHONE'] as $value){
                            $phone .= '<div class="value"><a class="dark_link" rel= "nofollow" href="tel:'.SolutionUtils::sanitizeValue(preg_replace('/[^+\d]+/', '', $value)).'">'.SolutionUtils::sanitizeValue(htmlspecialcharsBack($value)).'</a></div>';
                        }
                    }
                    else{
                        $phone = '<div class="value"><a class="dark_link" rel= "nofollow" href="tel:'.preg_replace('/[^+\d]+/', '', SolutionUtils::sanitizeValue($arShop['PHONE'])).'">'.SolutionUtils::sanitizeValue(htmlspecialcharsBack($arShop['PHONE'])).'</a></div>';
                ...
                $html .= (strlen($arShop['EMAIL']) ? '<div class="property email"><div class="title-prop font_upper muted">'.($bStore == 'Y' ? Loc::getMessage('CONTACT_EMAIL') : $arShop['DISPLAY_PROPERTIES']['EMAIL']['NAME']).'</div><div class="value"><a class="dark_link" href="mailto:'.SolutionUtils::sanitizeValue(htmlspecialcharsBack($arShop['EMAIL'])).'">'.SolutionUtils::sanitizeValue(htmlspecialcharsBack($arShop['EMAIL'])).'</a></div></div>' : '');
            $html .= '</div>';
        }
        $html .= '</div>';
        return $html;
                ...
                if(isset($arShop['IBLOCK_ID'])){
                    ...
                    if($arShop['GPS_S'] && $arShop['GPS_N']){
                        ...
                        $arPlacemarks[] = array(
                            ...
                            "LON" => $arShop['GPS_N'],
                            "TEXT" => $html,
                            "HTML" => '<div class="title">'.(strlen($arShop["URL"]) ? '<a href="'.$arShop["URL"].'">' : '').SolutionUtils::sanitizeHtml($arShop["ADDRESS"]).(strlen($arShop["URL"]) ? '</a>' : '').'</div><div class="info-content">'.($arShop['METRO'] ? SolutionUtils::sanitizeValue($arShop['METRO_PLACEMARK_HTML']) : '').(strlen($arShop['SCHEDULE']) ? '<div class="schedule">'.SolutionUtils::sanitizeHtml($arShop['SCHEDULE']).'</div>' : '').$str_phones.(strlen($arShop['EMAIL']) ? '<div class="email"><a rel="nofollow" href="mailto:'.SolutionUtils::sanitizeValue($arShop['EMAIL']).'">'.SolutionUtils::sanitizeValue($arShop['EMAIL']).'</a></div>' : '').'</div>'.(strlen($arShop['URL']) ? '<a rel="nofollow" class="button" href="'.$arShop["URL"].'"><span>'.GetMessage('DETAIL').'</span></a>' : '')
                        );
                    }
                }
                else{
                    ...
                    if($arShop['PHONE'])
                    {
                        ...
                        {
                            $str_phones .= '<br><a rel="nofollow" href="tel:'.SolutionUtils::sanitizeValue(str_replace(array(' ', ',', '-', '(', ')'), '', $phone)).'">'.SolutionUtils::sanitizeValue($phone).'</a>';
                        }
                        $str_phones .= '</div>';
                    }
                    if($arShop['GPS_S'] && $arShop['GPS_N']){
            ...
            if(isset($arShop['IBLOCK_ID'])){
                ...
                if($arShop['GPS_S'] && $arShop['GPS_N']){
                    ...
                    if($arShop['PHONE'])
                    {
                        ...
                        foreach($arShop['PHONE'] as $phone)
                        {
                            $str_phones .= '<br><a rel="nofollow" href="tel:'.SolutionUtils::sanitizeValue(preg_replace('/[^+\d]+/', '', $phone)).'">'.SolutionUtils::sanitizeValue(htmlspecialcharsBack($phone)).'</a>';
                        }
                        $str_phones .= '</div>';
                    }
    ...
    public static function prepareRegionItemMapHtml($arRegion){
        ...
        $html .= '<div class="title option-font-bold font_mlg">'.SolutionUtils::sanitizeHtml($arRegion['PROPERTY_ADDRESS_VALUE']['TEXT'] ?? '').'</div>';
        if($arRegion['PROPERTY_REGION_TAG_METRO_VALUE'] || $arRegion['PROPERTY_REGION_TAG_SHEDULLE_VALUE'] || $arRegion['PHONES'] || $arRegion['PROPERTY_EMAIL_VALUE'] ){
            $html .= '<div class="properties">';
                ...
                $metroValue = is_array($arRegion['PROPERTY_REGION_TAG_METRO_VALUE']) ? implode(', ', $arRegion['PROPERTY_REGION_TAG_METRO_VALUE']) : $arRegion['PROPERTY_REGION_TAG_METRO_VALUE'];
                $html .= ($metroValue ? '<div class="property metro"><div class="title-prop font_upper muted">'.(Loc::getMessage('CONTACT_METRO')).'</div><div class="value">'.SolutionUtils::sanitizeValue($metroValue).'</div></div>' : '');
                ...
                $html .= (strlen($shedule) ? '<div class="property schedule"><div class="title-prop font_upper muted">'.(Loc::getMessage('CONTACT_SCHEDULE')).'</div><div class="value">'.SolutionUtils::sanitizeHtml($shedule).'</div></div>' : '');
                ...
                if($arRegion['PHONES']){
                    ...
                    if(is_array($arRegion['PHONES'])){
                        foreach($arRegion['PHONES'] as $value){
                            if(is_array($value)){
                                $phone .= '<div class="value"><a class="dark_link" rel= "nofollow" href="'.(strlen($value['HREF']) ? SolutionUtils::sanitizeValue($value['HREF']) : 'javascript:;').'">'.SolutionUtils::sanitizeValue($value['PHONE']).'</a></div>';
                            }
                            else{
                                $phone .= '<div class="value"><a class="dark_link" rel= "nofollow" href="tel:'.SolutionUtils::sanitizeValue(str_replace(array(' ', ',', '-', '(', ')'), '', $value)).'">'.SolutionUtils::sanitizeValue($value).'</a></div>';
                            }
                        }
                    }
                    else{
                        $phone = '<div class="value"><a class="dark_link" rel= "nofollow" href="tel:'.SolutionUtils::sanitizeValue(str_replace(array(' ', ',', '-', '(', ')'), '', $arRegion['PHONES'])).'">'.SolutionUtils::sanitizeValue($arRegion['PHONES']).'</a></div>';
                    }
                ...
                if($arRegion['PROPERTY_EMAIL_VALUE']){
                    ...
                    if(is_array($arRegion['PROPERTY_EMAIL_VALUE'])){
                        foreach($arRegion['PROPERTY_EMAIL_VALUE'] as $value){
                            $email .= '<div class="value"><a class="dark_link" href="mailto:'.SolutionUtils::sanitizeValue($value).'">'.SolutionUtils::sanitizeValue($value).'</a></div>';
                        }
                    }
                    else{
                        $email = '<div class="value"><a class="dark_link" href="mailto:'.SolutionUtils::sanitizeValue($arRegion['PROPERTY_EMAIL_VALUE']).'">'.SolutionUtils::sanitizeValue($arRegion['PROPERTY_EMAIL_VALUE']).'</a></div>';
                    }
    ...
    public static function drawShopsList($arShops, $arParams, $showMap = 'Y')
    {
        ...
        if(is_array($arShops)) {
            foreach($arShops as $i => $arShop) {
                if(isset($arShop['IBLOCK_ID'])) {
                    $arShop['TITLE'] = (in_array('NAME', $arParams['FIELD_CODE']) ? TSolution\Utils::sanitizeValue($arShop['~NAME']) : '');
                    ...
                    $arShop['IMAGE'] = ($imageID ? CFile::ResizeImageGet($imageID, ['width' => 450, 'height' => 300], BX_RESIZE_IMAGE_PROPORTIONAL) : []);
                    $arShop['ADDRESS'] = $arShop['DISPLAY_PROPERTIES']['ADDRESS']['VALUE'];
                    $arShop['ADDRESS'] = $arShop['TITLE'].((strlen($arShop['TITLE']) && strlen($arShop['ADDRESS'])) ? ', ' : '').$arShop['ADDRESS'];
                    $arShop['PHONE'] = $arShop['DISPLAY_PROPERTIES']['PHONE']['VALUE'];
                    $arShop['EMAIL'] = $arShop['DISPLAY_PROPERTIES']['EMAIL']['VALUE'];
                    ...
                    if($arShop['GPS_S'] && $arShop['GPS_N']) {
                        ...
                        if($arShop['PHONE']) {
                            foreach($arShop['PHONE'] as $phone) {
                                $str_phones .= '<div class="phone"><a rel="nofollow" href="tel:'.SolutionUtils::sanitizeValue(str_replace([' ', ',', '-', '(', ')'], '', $phone)).'">'.SolutionUtils::sanitizeValue($phone).'</a></div>';
                            }
                        }
                    ...
                    <div class="contacts_map ">
                        <?if($arParams['MAP_TYPE'] != '0'):?>
                            <?$APPLICATION->IncludeComponent(
                                ...
                                [
                                    ...
                                    'MAP_DATA' => serialize(['google_lat' => $mapLAT, 'google_lon' => $mapLON, 'google_scale' => 15, 'PLACEMARKS' => $arPlacemarks]),
                                    'MAP_WIDTH' => '100%',
                                    'MAP_HEIGHT' => '550',
                                    'CONTROLS' => [
                                    ],
                        ...
                        <?else:?>
                            <?$APPLICATION->IncludeComponent(
                                ...
                                [
                                    'INIT_MAP_TYPE' => 'ROADMAP',
                                    'MAP_DATA' => serialize(['yandex_lat' => $mapLAT, 'yandex_lon' => $mapLON, 'yandex_scale' => 4, 'PLACEMARKS' => $arPlacemarks]),
                                    'MAP_WIDTH' => '100%',
                                    'MAP_HEIGHT' => '550',
                                    'CONTROLS' => [
                                        0 => 'ZOOM',
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 col-sm-8 col-xs-12 left-block-contacts">
                                            <?if($arShop['IMAGE'] && strlen($arShop['IMAGE']['src'])):?>
                                                ...
                                                <?if($bDetailLink):?>
                                                    <a href="<?=SolutionUtils::sanitizeValue($arShop['URL']);?>">
                                                <?endif; ?>
                                                        <img src="<?=Aspro\Functions\CAsproMax::showBlankImg($arShop['IMAGE']['src']);?>"
                                                            data-src="<?=$arShop['IMAGE']['src'];?>"
                                                            alt="<?=SolutionUtils::sanitizeValue($arShop['ADDRESS']);?>"
                                                            title="<?=SolutionUtils::sanitizeValue($arShop['ADDRESS']);?>"
                                            ...
                                            <div class="top-wrap">
                                                <div class="title font_mxs darken">
                                                    ...
                                                    <?endif; ?>
                                                            <?=SolutionUtils::sanitizeHtml($arShop['ADDRESS']); ?>
                                                    <?if($bDetailLink):?>
                                                        </a>
                                                    <?endif; ?>
                                                </div>
                                                <div class="middle-prop">
                                                    ...
                                                    <?if ($arShop['METRO'] && is_array($arShop['METRO'])):?>
                                                        <?foreach ($arShop['METRO'] as $metro):?>
                                                            <div class="metro font_upper"><?=static::showIconSvg('metro colored', SITE_TEMPLATE_PATH.'/images/svg/Metro.svg'); ?><span class="text muted777"><?=SolutionUtils::sanitizeValue($metro); ?></span></div>
                                                        <?endforeach; ?>
                                                    <?endif; ?>
                                                </div>
                                                <?if($arShop['SCHEDULE']):?>
                                                    <div class="schedule"><?=static::showIconSvg('clock colored', SITE_TEMPLATE_PATH.'/images/svg/WorkingHours.svg'); ?><span class="text font_xs muted777"><?=SolutionUtils::sanitizeHtml($arShop['SCHEDULE']); ?></span></div>
                                                <?endif; ?>
                                            </div>
                                        </div>
                                                ...
                                                <div class="row">
                                                    <?if($arShop['PHONE']):?>
                                                        <div class="phones col-md-6 col-sm-12 col-xs-12">
                                                            <?foreach($arShop['PHONE'] as $phone):?>
                                                                <?$phone = SolutionUtils::sanitizeHtml(htmlspecialcharsBack($phone));?>
                                                                <div class="phone font_sm darken">
                                                                    <a href="tel:<?=preg_replace('/[^+\d]+/', '', $phone);?>" class="black">
                                                                        <?=$phone;?>
                                                                    </a>
                                                                </div>
                                                            <?endforeach; ?>
                                                    ...
                                                    <?if(strlen($arShop['EMAIL'])):?>
                                                        <div class="emails col-md-6 col-sm-12 col-xs-12">
                                                            <div class="email font_sm">
                                                                <a class="dark-color" href="mailto:<?=SolutionUtils::sanitizeValue($arShop['EMAIL']); ?>"><?=SolutionUtils::sanitizeHtml($arShop['EMAIL']);?></a>
                                                            </div>
                                                        </div>
                                                    <?endif; ?>
                                                </div>
            ...
            if(isset($arShop['IBLOCK_ID'])){
                ...
                if($arShop['GPS_S'] && $arShop['GPS_N']){
                    ...
                    if($arShop['PHONE'])
                    {
                        foreach($arShop['PHONE'] as $phone)
                        {
                            $str_phones .= '<div class="phone"><a rel="nofollow" href="tel:'.SolutionUtils::sanitizeValue(str_replace(array(' ', ',', '-', '(', ')'), '', $phone)).'">'.SolutionUtils::sanitizeValue($phone).'</a></div>';
                        }
                    }
                ...
                if($arShop['PHONE'])
                {
                    foreach($arShop['PHONE'] as $phone)
                    {
                        $str_phones .= '<div class="phone"><a rel="nofollow" href="tel:'.SolutionUtils::sanitizeValue(preg_replace('/[^+\d]+/'
                        , '', $phone)).'">'.SolutionUtils::sanitizeValue(htmlspecialcharsBack($phone)).'</a></div>';
                    }
                }
                if($arShop['GPS_S'] && $arShop['GPS_N']){
                    $mapLAT += $arShop['GPS_N'];
                            ...
                            <div class="top_block">
                                <?if(strlen($arShop['ADDRESS'])):?>
                                    <div class="address">
                                        ...
                                        <div class="value darken"><?=SolutionUtils::sanitizeHtml(htmlspecialcharsBack($arShop['ADDRESS']))?></div>
                                    </div>
                                <?endif;?>
                                <?if($arPhotos):?>
                                    <!-- noindex-->
                                    <div class="gallery_wrap swipeignore">
                                        ...
                                        <div class="big-gallery-block text-center">
                                            ...
                                            <?foreach($arPhotos as $i => $arPhoto):?>
                                                <div class="item">
                                                <a href="<?=$arPhoto['ORIGINAL']?>" class="fancy" data-fancybox="item_slider" target="_blank" title="<?=TSolution\Utils::sanitizeValue($arPhoto['DESCRIPTION'])?>">
                                    ...
                                    <?if($arShop["METRO"]):?>
                                        <?foreach($arShop["METRO"] as $metro):?>
                                            <div class="property metro">
                                                ...
                                                <div class="value darken"><?=SolutionUtils::sanitizeValue(htmlspecialcharsBack($metro));?></div>
                                            </div>
                                        <?endforeach;?>
                                    <?endif;?>
                                    <?if($arShop["SCHEDULE"]):?>
                                        <div class="property schedule">
                                            ...
                                            <div class="value darken"><?=SolutionUtils::sanitizeHtml(htmlspecialcharsBack($arShop["SCHEDULE"]));?></div>
                                        </div>
                                    <?endif;?>
                                </div>
                                <div class="col-md-6 col-sm-6">
                                        ...
                                        <div class="property phone">
                                            ...
                                            <?foreach($arShop["PHONE"] as $phone):?>
                                                <div class="value phone darken">
                                                    <a href="tel:<?=SolutionUtils::sanitizeValue(preg_replace('/[^+\d]+/', '', $phone));?>" rel="nofollow" class="black"><?=SolutionUtils::sanitizeHtml(htmlspecialcharsBack($phone));?></a>
                                                </div>
                                            <?endforeach;?>
                                        </div>
                                    <?endif?>
                                    <?if(strlen($arShop["EMAIL"])):?>
                                        <div class="property email">
                                            ...
                                            <div class="value darken"><a class="dark-color" rel="nofollow" href="mailto:<?=SolutionUtils::sanitizeValue(htmlspecialcharsBack($arShop["EMAIL"]));?>"><?=SolutionUtils::sanitizeValue(htmlspecialcharsBack($arShop["EMAIL"]));?></a></div>
                                        </div>
                                    <?endif;?>
                                </div>
                            ...
                            <div class="feedback item">
                                <div class="wrap">
                                    <?if($arShop['DESCRIPTION']):?>
                                        <div class="previewtext muted777"><?=SolutionUtils::sanitizeHtml($arShop['DESCRIPTION']);?></div>
                                    <?endif;?>
                                    <?//if(1 || $bUseFeedback):?>
