...
use Aspro\Max\Traits\Events as EventsTraits;
...
class CMaxEvents
{
    use EventsTraits\Comments;
    use EventsTraits\User;
    ...
    public static function onBeforeUserRegisterHandler()
    {
        if (Validation::isSolutionForm()) {
            if (Validation::isAgreementsRequired()) {
                $licenseField = $_REQUEST['order']['licenses_popup'] ?? $_REQUEST['licenses_popup'] ?? '';
                if (empty($licenseField)) {
                    $GLOBALS['APPLICATION']->ThrowException(Loc::getMessage('ERROR_FORM_LICENSE'));
...
<!DOCTYPE html>', '<!DOCTYPE html>', $content);
    ...
    public static function onBeforeResultAddHandler($WEB_FORM_ID, &$arFields, &$arrVALUES)
    {
        ...
        if (Validation::isSolutionForm()) {
            if (Validation::isAgreementsRequired()) {
                if (!isset($_REQUEST['licenses_popup']) && !isset($_REQUEST['licenses_inline'])){
    ...
    public static function OnBeforeSubscriptionAddHandler()
    {
        if (Validation::isSolutionForm()) {
            global $APPLICATION;
            $hasError = false;
            ...
            if (Validation::isCaptchaOnSubscribeRequired()) {
                if (empty($_REQUEST['captcha_word']) || !$APPLICATION->CaptchaCheckCode($_REQUEST['captcha_word'], $_REQUEST['captcha_sid'])) {
                    $APPLICATION->ThrowException(Loc::getMessage('ERROR_FORM_CAPTCHA'));
    ...
    public static function onBeforeProductSubscribeHandler(Entity\Event $event)
    {
        global $APPLICATION, $USER;

        if (!$USER->IsAuthorized()) {
            if (Validation::isAgreementsRequired()) {
                if (!isset($_REQUEST['licenses_popup'])) {
                    $result = new Entity\EventResult();
                    $result->addError(
                        new Entity\EntityError(
                            Loc::getMessage('ERROR_FORM_LICENSE'),
                            'ERROR_FORM_LICENSE'
                        )
                    );
            ...
            if (Validation::isCaptchaOnSubscribeRequired()) {
                if (empty($_REQUEST['captcha_word']) || !$APPLICATION->CaptchaCheckCode($_REQUEST['captcha_word'], $_REQUEST['captcha_sid'])) {
                    ...
                    $result->addError(
                        new Entity\EntityError(
                            Loc::getMessage('ERROR_FORM_CAPTCHA'),
                            'ERROR_FORM_CAPTCHA'
                        )
                    );
                    return $result;
