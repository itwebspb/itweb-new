        ...
        if (!strlen($errorMessage)) {
            ...
            foreach(
                ...
                if (!Loader::includeModule($moduleId)) {
                    $errorMessage = Loc::getMessage('ICONSET_ERROR_MODULE_NOT_INCLUDED', ['#MODULE_NAME#' => $moduleId]);
                }
            }
        }
                    ...
                    if (strlen($id)) {
                        foreach($iconset->getItems() as $item) {
                            if ($item['id'] == $id) {
                                ...
                                ?>
                                <div class="iconset_item iconset_item--added <?=$bSelected ? 'iconset_item--selected' : '';?>" data-id="<?=htmlspecialcharsbx($item['id']);?>" data-value="<?=htmlspecialcharsbx($item['path']);?>" title="<?=htmlspecialcharsbx($item['name']);?>">
...
<?else:?>
    ...
    <?if (!strlen($errorMessage)):?>
        <?php
        $GLOBALS['APPLICATION']->SetAdditionalCSS('/bitrix/css/aspro.max/iconset.css');
        $GLOBALS['APPLICATION']->AddHeadScript('/bitrix/js/aspro.max/iconset.js');
        ...
        <div id="iconset-<?=$rand;?>" class="iconset<?=$bCanAdd && $bCanChange ? ' iconset--can_add' : '';?>" data-code="<?=$code;?>" style="display:none;">
            ...
            <div class="iconset_wrap">
                ...
                <?foreach($iconset->getItems() as $item):?>
                    <?$bSelected = $item['path'] === $value;?>
                    <div class="iconset_item<?=$bSelected ? ' iconset_item--selected' : '';?>" data-id="<?=$item['id'];?>" data-value="<?=htmlspecialcharsbx($item['path']);?>" title="<?=htmlspecialcharsbx($item['name']);?>">
                        <?if ($bCanDelete && $bCanChange && !$item['default']):?>
                            <div class="iconset_btn--delete" title="<?=Loc::getMessage('ICONSET_BUTTON_DELETE');?>"></div>
                        <?endif;?>
                        <div class="iconset_item_middle">
            ...
            <?if ($bCanAdd && $bCanChange):?>
                <?$$varTabControl->BeginNextTab();?>
                <form name="iconset_form" class="iconset_form" action="<?=htmlspecialcharsbx($APPLICATION->GetCurPage())?>?lang=<?=htmlspecialcharsbx(LANGUAGE_ID)?>" method="post" enctype="multipart/form-data">
