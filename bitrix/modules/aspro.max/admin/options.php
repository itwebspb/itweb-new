                    ...
                    foreach ($arBlock['OPTIONS'] as $optionCode => $arOption) {
                        if ($arOption['TYPE'] === 'array') {
                            ...
                            if ($optionCode == 'CUSTOM_FONT') {
                                $newVal = htmlspecialcharsback(htmlspecialcharsbx(trim(str_replace(['>', '<', 'rel="stylesheet"'], '', $_REQUEST[$optionCode.'_'.$optionsSiteID]))));
                            } else {
                                $newVal = $_REQUEST[$optionCode.'_'.$optionsSiteID];
                            }
                            if ($optionCode == 'PRIORITY_SECTION_DESCRIPTION_SOURCE') {
                                $priority = isset($_REQUEST[$optionCode.'_'.$optionsSiteID]) ? (is_array($_REQUEST[$optionCode.'_'.$optionsSiteID]) ? htmlspecialcharsbx(implode(',', $_REQUEST[$optionCode.'_'.$optionsSiteID])) : htmlspecialcharsbx($_REQUEST[$optionCode.'_'.$optionsSiteID])) : '';
                                if (!strlen($priority)) {
                            ...
                            if ($arOption['TYPE'] == 'checkbox') {
                                    ...
                                    foreach ($arOption['LIST'] as $keyh => $arValueH) {
                                        if ($arValueH['ADDITIONAL_OPTIONS']) {
                                            foreach ($arValueH['ADDITIONAL_OPTIONS'] as $keyh2 => $arValueH2) {
                                                $value = $_REQUEST[$keyh2.'_'.$keyh.'_'.$optionsSiteID] ?? '';
                                                if (!$value && $arValueH2['TYPE'] == 'checkbox') {
                                                    $value = 'N';
                                                }
                                                Option::set('aspro.max', $keyh2.'_'.$keyh, $value, $optionsSiteID);
                                            }
                                        }
                                    }
                                }
                                if (isset($arOption['DEPENDENT_PARAMS']) && $arOption['DEPENDENT_PARAMS']) {
                                    foreach ($arOption['DEPENDENT_PARAMS'] as $keyh2 => $arValueH2) {
                                        $value = $_REQUEST[$keyh2.'_'.$optionsSiteID] ?? '';
                                        if (!$value && $arValueH2['TYPE'] == 'checkbox') {
                                            $value = 'N';
                                        }
                                        Option::set('aspro.max', $keyh2, $value, $optionsSiteID);
                                    }
                                }
                            ...
                            if ($arOption['TYPE'] != 'file') {
                                $arTab['OPTIONS'][$optionCode] = htmlspecialcharsback(htmlspecialcharsbx($newVal));
                            }

                            Option::set($moduleID, $optionCode, $newVal, $optionsSiteID);

                            if ($optionCode == 'CUSTOM_FONT') {
                                $path = Application::getDocumentRoot().'/bitrix/components/aspro/theme.'.Solution::solutionName.'/css/user_font_'.$optionsSiteID.'.css';
                                $content = '';
                                if ($newVal) {
                                    if (preg_match('/family=([^:&]+)/', $newVal, $matches)) {
                                        if (is_array($matches) && !empty($matches[1])) {
                                            $content = "body,h1, h2, h3, h4, h5, h6, .h1, .h2, .h3, .h4, .h5, .h6, .popup-window,body div.bx-yandex-map,.fancybox-title{font-family: '".htmlspecialcharsbx(str_replace('+', ' ', $matches[1]))."', sans-serif;}";
                                        }
                                    }
                                }
                                IO\File::putFileContents(htmlspecialcharsbx($path), htmlspecialcharsbx($content));
                            }
                        }
                    }
                }
        ...
        <form method="post" class="max_options views" enctype="multipart/form-data" action="<?=$APPLICATION->GetCurPage();?>?mid=<?=urlencode($mid);?>&amp;lang=<?=LANGUAGE_ID;?>">
            ...
            if ($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) && check_bitrix_sessid()) {
                if (strlen($Update) && strlen($_REQUEST["back_url_settings"])) {
                    $back = (string)($_REQUEST['back_url_settings'] ?? '');
                    if ($back !== '' && $back[0] === '/' && substr($back, 0, 2) !== '//') {
                        LocalRedirect($back);
                    } else {
            ...
            <script type="text/javascript">
                ...
                function searchParams() {
                    var _this = $(this).closest('.tabs-wrapper').find('.search_wrapper input'),
                        val = _this.val(),
            ...
            <script type="text/javascript">
                ...
                $('[name^="VK_API_CLIENT_ID"] + .adm-btn--popup').on('click', function() {
                    const nodeInput = this.previousElementSibling;
                    let clientId = nodeInput.value;
                    if (!clientId) {
                        return;
                    }
                    ...
                    const siteOrigin = '<?=($_SERVER['HTTPS'] ? 'https://' : 'http://').$_SERVER['SERVER_NAME'];?>';

                    const authWindow = window.open(`<?=VKID::getRedirectUrl();?>?site_id=${siteId}&client_id=${clientId}`, '_blank', 'popup,width=640,height=480');
                    ...
                    const callback = (event) => {
                        ...
                        if (data.type === 'vk_auth_callback') {
                            clearInterval(checkWindowInterval);
                            this.disabled = false;

                            const nodeInfoMessage = this.closest('.inner_wrapper').querySelector('.adm-info-message');
                            if (nodeInfoMessage) {
                                nodeInfoMessage.innerHTML = nodeInfoMessage.dataset.message.replace('#ID#', clientId);
                                nodeInfoMessage.classList.remove('_adm-hidden');
                            }
				...
				$('input[name^="AGREEMENT_USE_UNIQUE"]').change(function() {
					...
					if(!_this.prop('checked') || !licenceTypeBitrix){
                        ...
                        tab.find('[data-optioncode^="AGREEMENT_COMMENT"]').addClass('aspro-admin-hidden');
                        ...
                        tab.find('[data-optioncode^="AGREEMENT_COMMENT"]').removeClass('aspro-admin-hidden');
					}
				});
