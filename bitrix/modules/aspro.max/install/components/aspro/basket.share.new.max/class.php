...
use Aspro\Max\ShareBasketItemTable;
use Aspro\Max\ShareBasketTable;
...
class CAsproBasketShareNewMax extends CBitrixComponent
{
    ...
    public function onPrepareComponentParams($arParams)
    {
        ...
        if (isset($arParams['SHARE_SOCIALS']) && is_array($arParams['SHARE_SOCIALS'])) {
            $arTmp = array_intersect(['VKONTAKTE', 'FACEBOOK', 'ODNOKLASSNIKI', 'MOIMIR', 'TWITTER', 'VIBER', 'WHATSAPP', 'SKYPE', 'TELEGRAM', 'MAX'], $arParams['SHARE_SOCIALS']);
            $arParams['SHARE_SOCIALS'] = array_values($arTmp);
        } else {
            $arParams['SHARE_SOCIALS'] = ['VKONTAKTE', 'ODNOKLASSNIKI', 'MAX'];
        }
    ...
    protected function includeModules()
    {
        ...
        if (!Loader::includeModule(Solution::moduleID)) {
            $this->setError(Loc::getMessage('BSN_C_ERROR_MODULE_SOLUTION_NOT_INSTALL', ['#SOLUTION_MODULE_ID#' => Solution::moduleID]));
        }
    ...
    protected function makeResult()
    {
        ...
        $arProducts = $arProductsIds = $arSectionsIDs = $arSections = [];
        ...
        $shareBasket = ShareBasketTable::getList([
            'filter' => [
                '=HASH' => $hash,
                '=USER_ID' => $userId,
                '=SITE_ID' => $siteId,
    ...
    protected static function generateCode()
    {
        do {
            ...
            $result = ShareBasketTable::getList([
                'filter' => ['=CODE' => $code],
                'limit' => 1,
                'select' => ['ID'],
            ]);
