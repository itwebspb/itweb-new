...
{
    public const PRESET_MODULE_ID = 'aspro.sharepreset';
    public const PRESET_FILE_EXTENSION = 'json';
    public const PRESET_FILE_MAX_SIZE = 102400;
    ...
    public function importFromLinkAction($sessid, $siteId, $siteDir, $front, $link)
    {
        ...
        if ($arUrl = parse_url($link)) {
            if (empty($arUrl['host'])) {
                throw new SystemException(Loc::getMessage('TA_C_ERROR_BAD_URL_LINK') ?: 'Bad URL');
            }

            $host = mb_strtolower((string)$arUrl['host']);

            // Only allow requests to the same server
            $serverName = mb_strtolower((string)$_SERVER['HTTP_HOST']);
            if ($host !== $serverName) {
                throw new SystemException(Loc::getMessage('TA_C_ERROR_BAD_URL_LINK') ?: 'Remote host not allowed');
            }

            $ip = gethostbyname($host);
            if (!filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE)) {
                throw new SystemException(Loc::getMessage('TA_C_ERROR_BAD_URL_LINK') ?: 'Private address blocked');
            }

            $scheme = in_array($arUrl['scheme'], ['http', 'https']) ? $arUrl['scheme'] : 'https';
    ...
    public function importFromPresetAction($sessid, $siteId, $siteDir, $front, $preset)
    {
        ...
        if (
            ...
            if (
                ...
                throw new SystemException(Loc::getMessage('TA_C_ERROR_LINK_ANOTHER_MODULE', ['#MODULE_ID#' => Solution::moduleID]));
            }
                ...
                if (is_dir($file['tmp_name'])) {
                    ...
                    if (
                        ...
                        if (
                            ...
                            throw new SystemException(Loc::getMessage('TA_C_ERROR_FILE_ANOTHER_MODULE', ['#MODULE_ID#' => Solution::moduleID]));
                        }
    ...
    protected function includeModules()
    {
        if (
            ...
            throw new SystemException(Loc::getMessage('TA_C_ERROR_MODULE_NOT_INSTALLED', ['#MODULE_ID#' => Solution::moduleID]));
        }
    }
    ...
    protected function checkCanSave($siteId, $siteDir, $front)
    {
        if ($front) {
            $arFrontParametrs = Solution::GetFrontParametrsValues($siteId, $siteDir, false);
            if ($arFrontParametrs['THEME_SWITCHER'] !== 'Y' || !$GLOBALS['USER']->IsAuthorized()) {
                throw new SystemException(Loc::getMessage('TA_C_ERROR_SWITCHER_NOT_ACTIVE'));
            }
        } else {
            $RIGHT = $GLOBALS['APPLICATION']->GetGroupRight(Solution::moduleID);
