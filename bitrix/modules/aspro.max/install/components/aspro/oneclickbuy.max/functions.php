...
function placeOrder($registeredUserID, $basketUserID, $newOrder, $arOrderDat, $POST)
{
    ...
    $propsList = ['EMAIL', 'PHONE', 'FIO'];
    foreach ($propsList as $propCode) {
        $prop = htmlspecialcharsbx($_POST['ONE_CLICK_BUY'][$propCode] ?? '');
        if (!$prop) {
            continue;
        }
...
function safeUserLogout(string $sessid = ''): void
{
    global $USER;
    $USER->Logout();
