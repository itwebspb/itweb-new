...
if (!$request->isPost()) {
    ...
    exit(Json::encode(['STATUS' => 'N', 'message' => 'Forbidden']));
}

if (!check_bitrix_sessid()) {
    exit(Json::encode(['STATUS' => 'N', 'MESSAGE' => 'Bad session']));
}

$isUploadFiles = $request->getPost('uploadfiles') === 'Y';
if ($isUploadFiles) {
    Loader::includeModule('sale');

    $orderID = htmlspecialcharsbx($request->getPost('orderID'));
    if (!$orderID) {
        exit(Json::encode(['STATUS' => 'N', 'message' => 'Undefined order ID']));
    }

    $arOrderQuery = CSaleOrder::GetList([], ['ID' => $orderID], false, false, ['ID', 'PERSON_TYPE_ID', 'DATE_INSERT'])->Fetch();
    if (!$arOrderQuery) {
        exit(Json::encode(['STATUS' => 'N', 'message' => 'Order not found']));
    }

    $now = new Bitrix\Main\Type\DateTime();
    if ($now->getTimeStamp() - MakeTimeStamp($arOrderQuery['DATE_INSERT']) > UPLOAD_FILES_TTL) {
        exit(Json::encode(['STATUS' => 'N', 'MESSAGE' => 'Timeout']));
    }

    global $USER;
    if ($USER->IsAuthorized()) {
        $order = Bitrix\Sale\Order::load($orderID);
        if (!$order || (int) $order->getUserId() !== (int) $USER->GetID()) {
            exit(Json::encode(['STATUS' => 'N', 'MESSAGE' => 'Access denied']));
        }
    } else {
        $uploadToken = (string) $request->getPost('uploadToken');
        $storedToken = $_SESSION['ocb_upload_tokens'][$orderID] ?? null;

        if (
            empty($uploadToken)
            || empty($storedToken)
            || time() > (int) $storedToken['expires']
            || !hash_equals($storedToken['token'], $uploadToken)
        ) {
            exit(Json::encode(['STATUS' => 'N', 'MESSAGE' => 'Access denied']));
        }

        unset($_SESSION['ocb_upload_tokens'][$orderID]);
    }
    ...
    if ($res->SelectedRowsCount() > 0) {
        ...
        foreach ($_FILES as $key => $arFile) {
            ...
            $error = CFile::CheckFile($arFile, 5 * 1024 * 1024, false, $allowedExtensions);
            if ($error !== '') {
                continue;
            }
    ...
    if (!function_exists('getJson')) {
        function getJson($message, $res = 'N', $error = '', $ext = false)
        {
            ...
            $result = [
                'result' => $res === 'Y' ? 'Y' : 'N',
                'message' => htmlspecialcharsbx($GLOBALS['APPLICATION']->ConvertCharset($message, SITE_CHARSET, 'utf-8')),
            ];
            ...
            if ($error) {
                ...
                $error = SolutionUtils::sanitizeHtml(html_entity_decode($error));
                $result['err'] = $GLOBALS['APPLICATION']->ConvertCharset($error, SITE_CHARSET, 'utf-8');
            }
    ...
    $SITE_ID = htmlspecialcharsbx($_REQUEST['SITE_ID']);
    ...
    if (!$USER->IsAuthorized()) {
                ...
                if ($_POST['ONE_CLICK_BUY']['PHONE']) {
                    ...
                    if ($bPhoneAuthShow) {
                        ...
                        if ($nUserCount == 1) {
                            ...
                            $registeredUserID = htmlspecialcharsbx($ar_user['USER_ID']);
                        ...
                        if ($nUserCount == 1) {
                            ...
                            $registeredUserID = htmlspecialcharsbx($ar_user['ID']);
        ...
        if ($user_registered && !$user_exists) {
            ...
            if ($bPhoneAuthSupported && $bPhoneAuthShow) {
                ...
                if ($arUserByPhone) {
                    exit(getJson(GetMessage('TOO_MANY_USERS_WITH_PHONE', ['#PHONE#' => htmlspecialcharsbx($phoneNumber)])));
                }
            ...
            if ($newUser['TYPE'] == 'ERROR') {
                ...
                $registeredUserID = htmlspecialcharsbx($newUser['ID']);

                if (!empty($_POST['ONE_CLICK_BUY']['PHONE'])) {
                    $USER->Update($registeredUserID, ['PERSONAL_PHONE' => htmlspecialcharsbx($_POST['ONE_CLICK_BUY']['PHONE'])]);
                }
                if (!empty($username[2])) {
                            ...
                            if (
                                ...
                                if (!$arUserByPhone['DATE_SENT'] || ($timeDiff > CUser::PHONE_CODE_RESEND_INTERVAL)) {
                                    ...
                                    if (!$smsResult->isSuccess()) {
                                        exit(getJson('', 'Y', htmlspecialcharsback(htmlspecialcharsbx(implode(
                                            '<br />',
                                            array_merge(
                                                $arResult['ERRORS'],
                                                $smsResult->getErrorMessages()
                                            )
                                        ))), ['CODE' => 'SHOW_SMS_FIELD', 'SIGNED_DATA' => $signedData, 'RESEND_INTERVAL' => CUser::PHONE_CODE_RESEND_INTERVAL]));
                                    }
    ...
    $newOrder = [
        ...
        'CANCELED' => 'N',
        'STATUS_ID' => 'N',
        'USER_ID' => htmlspecialcharsbx($registeredUserID),
        'PERSON_TYPE_ID' => intval($_POST['PERSON_TYPE_ID']) > 0 ? $_POST['PERSON_TYPE_ID'] : 1,
        'DELIVERY_ID' => $deliveryId,
        'PAY_SYSTEM_ID' => intval($_POST['PAY_SYSTEM_ID']) > 0 ? htmlspecialcharsbx($_POST['PAY_SYSTEM_ID']) : 1,
        'USER_DESCRIPTION' => htmlspecialcharsbx($_POST['ONE_CLICK_BUY']['COMMENT']),
    ...
    if ($bAllBasketBuy) {
        ...
        if ($arBasketItems) {
            ...
            $arOrderDat['USER_ID'] = htmlspecialcharsbx($registeredUserID);

            $arMinPriceCheck = Solution::checkAllowDelivery($newOrder['PRICE'], $newOrder['CURRENCY']);
            if ($arMinPriceCheck['ERROR']) {
                exit(getJson(GetMessage('ORDER_CREATE_FAIL'), 'N', $arMinPriceCheck['TEXT']));
            }
            ...
            if ($orderID == false) {
                ...
                if ($user_registered) {
                    safeUserLogout($anonymousSessid);
                }
        ...
        if ($arErrors) {
            ...
            exit(getJson(GetMessage('ORDER_CREATE_FAIL'), 'N', htmlspecialcharsback(htmlspecialcharsbx(implode('<br />', (array) $arErrors)))));
        }
    ...
    $uploadToken = null;
    if ($user_registered && $orderID) {
        $uploadToken = bin2hex(random_bytes(16));
        $_SESSION['ocb_upload_tokens'][$orderID] = [
            'token' => $uploadToken,
            'expires' => time() + UPLOAD_FILES_TTL,
        ];
    }
    ...
    try {
        $ocbOrderProps = Json::decode($_POST['PROPERTIES']);
    } catch (Throwable $th) {
        $ocbOrderProps = [];
    }

    if (!empty($ocbOrderProps)) {
        $ocbOrderProps = array_map('strval', $ocbOrderProps);
    }

    if (!in_array('LOCATION', $ocbOrderProps)) {
        $ocbOrderProps[] = 'LOCATION';
    }

    $res = CSaleOrderProps::GetList([], ['@CODE' => $ocbOrderProps, 'PERSON_TYPE_ID' => $personType]);

    while ($prop = $res->Fetch()) {
        $propValue = htmlspecialcharsbx($_POST['ONE_CLICK_BUY'][$prop['CODE']] ?? '');
        if (empty($propValue)) {
            continue;
        }
        ...
        $dbP = CSaleOrderPropsValue::GetList([], ['ORDER_ID' => $orderID, 'ORDER_PROPS_ID' => $prop['ID']]);
        if ($arP = $dbP->Fetch()) {
            CSaleOrderPropsValue::Update($arP['ID'], ['VALUE' => $propValue]);
        } else {
            CSaleOrderPropsValue::Add(['ORDER_ID' => $orderID, 'NAME' => $prop['NAME'], 'ORDER_PROPS_ID' => $prop['ID'], 'CODE' => $prop['CODE'], 'VALUE' => $propValue]);
        }
    }
    ...
    if ($orderID) {
        ...
        $arMessageFields = [
            'RS_ORDER_ID' => $orderID,
            'CLIENT_NAME' => htmlspecialcharsbx($_POST['ONE_CLICK_BUY']['FIO'] ?: $_POST['ONE_CLICK_BUY']['CONTACT_PERSON']),
            'ACCOUNT_NUMBER' => $arOrderQuery['ACCOUNT_NUMBER'],
            'PHONE' => htmlspecialcharsbx($_POST['ONE_CLICK_BUY']['PHONE']),
            ...
            'COMMENT' => htmlspecialcharsbx($_POST['ONE_CLICK_BUY']['COMMENT']),
        ];
        if ($_POST['ONE_CLICK_BUY']['EMAIL']) {
    ...
    $ext = [];
    if ($uploadToken !== null) {
        $ext['uploadToken'] = $uploadToken;
    }

    exit(getJson($orderID, 'Y', '', $ext));
}
