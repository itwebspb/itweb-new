...
$templateData['BANNERS_COUNT'] = count($arResult['ITEMS']);
$templateData['CURRENT_BANNER_INDEX'] = $currentBannerIndex;
$templateData['CURRENT_BANNER_COLOR'] = '';
        ...
        <div class="swiper-wrapper main-slider__wrapper">
            ...
            <?foreach ($arResult['ITEMS'][$arParams['BANNER_TYPE_THEME']]['ITEMS'] as $i => $arItem):?>
                ...
                <div class="swiper-slide main-slider__item box <?=$arItem['PROPERTIES']['TEXTCOLOR']['VALUE_XML_ID'] ? ' '.$arItem['PROPERTIES']['TEXTCOLOR']['VALUE_XML_ID'] : '';?><?=$arItem['PROPERTIES']['TEXT_POSITION']['VALUE_XML_ID'] ? ' '.$arItem['PROPERTIES']['TEXT_POSITION']['VALUE_XML_ID'] : ' left';?><?=$bShowVideo ? ' wvideo' : '';?>"
                    ...
                    <?endif;?>
                    >
                    <div id="<?=$this->GetEditAreaId($arItem['ID']);?>" class="<?=Utils::implodeClasses($opacityClasses);?>">
                            ...
                            <table>
                                <tbody>
                                    <?if ($arResult['BIGBANNER_MOBILE'] == 3) {
                                        ...
                                        $mobileImageClass = $arItem['PROPERTIES']['TABLET_IMAGE']['VALUE'] ? 'with-mobile-image' : '';
                                    }?>
                                    <tr class="main_info js-notice-block <?=$mobileImageClass;?>"
                                        <?if ($bSwiperLazy):?>
                                            ...
                                            <?ob_start();?>
                                                <td class="text <?=$arItem['PROPERTIES']['TEXT_POSITION']['VALUE_XML_ID'];?>">
                                                    <?if ($arItem['PROPERTIES']['LINK_ITEM']['VALUE']):?>
                                                        ...
                                                        $arSelect = ['ID', 'IBLOCK_ID', 'NAME', 'DETAIL_PAGE_URL', 'PROPERTY_vote_count', 'PROPERTY_rating', 'PROPERTY_vote_sum', 'CATALOG_TYPE', 'PROPERTY_EXTENDED_REVIEWS_RAITING', 'PROPERTY_EXTENDED_REVIEWS_COUNT'];
                                                        if ($hitProp) {
                                                        ...
                                                        $arProduct = CMaxCache::CIBLockElement_GetList(['CACHE' => ['MULTI' => 'N', 'TAG' => CMaxCache::GetIBlockCacheTag($arItem['PROPERTIES']['LINK_ITEM']['LINK_IBLOCK_ID'])]], ['IBLOCK_ID' => $arItem['PROPERTIES']['LINK_ITEM']['LINK_IBLOCK_ID'], 'ACTIVE' => 'Y', 'ACTIVE_DATE' => 'Y', 'ID' => $arItem['PROPERTIES']['LINK_ITEM']['VALUE']], false, false, $arSelect);
                                                        ...
                                                        <div class="banner_title item_info">
                                                            ...
                                                            if ($bHasOffers) {
                                                                $arSelect = ['ID', 'IBLOCK_ID', 'NAME', 'CATALOG_QUANTITY'];
                                                                $arOffers = CMaxCache::CIBLockElement_GetList(['CACHE' => ['MULTI' => 'Y', 'TAG' => CMaxCache::GetIBlockCacheTag($arItem['PROPERTIES']['LINK_ITEM']['LINK_IBLOCK_ID'])]], ['PROPERTY_CML2_LINK' => $arProduct['ID'], 'ACTIVE' => 'Y', 'ACTIVE_DATE' => 'Y'], false, false, $arSelect);
                                                            ...
                                                            $arQuantityData = CMax::GetQuantityArray($totalCount, ['ID' => $arProduct['ID']], 'N', false);
                                                            $strMeasure = '';

                                                            if ($arProduct['CATALOG_MEASURE'] && $arParams['SHOW_MEASURE'] !== 'N') {
                                                                $arMeasure = CCatalogMeasure::getList([], ['ID' => $arProduct['CATALOG_MEASURE']], false, false, [])->GetNext();
                                                                    ...
                                                                    <?if ($arItem['PROPERTIES']['SHOW_RATING']['VALUE'] == 'Y'):?>
                                                                        <div class="ratings">
                                                                            <?if ($arParams['REVIEWS_VIEW'] == 'EXTENDED'):?>
                                                                                <?$message = $arProduct['PROPERTY_EXTENDED_REVIEWS_RAITING_VALUE'] ? GetMessage('VOTES_RESULT', ['#VALUE#' => $arProduct['PROPERTY_EXTENDED_REVIEWS_RAITING_VALUE']]) : GetMessage('VOTES_RESULT_NONE');?>
