			...
			<div class="item" itemprop="review" itemscope="itemscope" itemtype="http://schema.org/Review">
				<div class="top_block_wrapper">
					<div class="n-product-review-user_profile">
						...
						<?else:?>
                            <?php
                            $authorName = htmlspecialcharsbx((string)$arItem['author']);
                            $avatarUrl = htmlspecialcharsbx((string)($arItem['authorInfo']['avatarUrl'] ?? ''));
                            ?>
							<?if ($arItem['authorInfo'] && strlen($avatarUrl)):?>
								<div class="img">
									<img class="n-product-review-user_avatar" src="<?=$avatarUrl;?>" alt="<?=$authorName;?>">
								</div>
							<?endif;?>
							<span class="n-product-review-user_name" itemprop="author"><?=$authorName;?></span>
				...
				<?if($arItem["pro"]):?>
					<div class="middle_block_wrapper">
						...
						<div class="n-product-review-item__text"><?=htmlspecialcharsbx($arItem['pro']);?></div>
					</div>
				<?endif;?>
				<?if($arItem["contra"]):?>
					<div class="middle_block_wrapper">
						<div class="n-product-review-item__title"><?=Loc::getMessage("YMFAIL")?></div>
						<div class="n-product-review-item__text"><?=htmlspecialcharsbx($arItem['contra']);?></div>
					</div>
				<?endif;?>
				<?if($arItem["text"]):?>
					<div class="middle_block_wrapper">
						<div class="n-product-review-item__title"><?=Loc::getMessage("YMCOMMENT")?></div>
						<div class="n-product-review-item__text"><?=htmlspecialcharsbx($arItem['text']);?></div>
					</div>
				<?endif;?>
