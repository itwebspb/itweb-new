...
if (!isset($arProperty['NUM_AMOUNT'])) {
    $arSelect = ['ID', 'PRODUCT_AMOUNT', 'ADDRESS', 'SORT', 'TITLE', 'GPS_N', 'GPS_S', 'UF_METRO', 'SCHEDULE', 'EMAIL', 'UF_PHONES'];
    if ($arParams['SHOW_GENERAL_STORE_INFORMATION'] != 'Y') {
        foreach ($arResult['STORES'] as $pid => $arProperty) {
            $arStore = CCatalogStore::GetList(['TITLE' => 'ASC', 'ID' => 'ASC'], ['ACTIVE' => 'Y', 'PRODUCT_ID' => $arParams['ELEMENT_ID'], 'ID' => $arProperty['ID']], false, false, $arSelect)->Fetch();
            ...
            if ($arResult['STORES'][$pid]['PHONE']) {
                $storePhones = array_filter(explode(',', $arResult['STORES'][$pid]['PHONE'] ?? ''));
                $ufPhones = array_filter((array) Solution::unserialize($arStore['UF_PHONES']) ?: []);

                if (!empty($ufPhones)) {
                    $arResult['STORES'][$pid]['PHONE'] = implode(', ', array_merge($storePhones, $ufPhones));
                }
        ...
        $filter = ['ACTIVE' => 'Y', 'PRODUCT_ID' => $arParams['ELEMENT_ID'], '+SITE_ID' => SITE_ID, 'ISSUING_CENTER' => 'Y', 'ID' => (array) $arParams['STORES']];
        $rsProps = CCatalogStore::GetList(['TITLE' => 'ASC', 'ID' => 'ASC'], $filter, false, false, $arSelect);
