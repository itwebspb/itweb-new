				...
				<div class="slide-block__body">
					<ul class="menu dropdown">
						...
						<?foreach($arSections as $sId => $arSection):?>
							...
							?>
							<li class="full item<?=($bCurrent ? ' current' : '')?><?=($bCollapsed ? ' collapsed' : '')?>"><a href="<?=$APPLICATION->GetCurPageParam('section_id='.$sId, $arDeleteParams)?>" class="rounded2 bordered"><span class="item_title"><?=htmlspecialcharsbx($arSection['NAME'])?></span><span class="item_count muted"><?=$arAllSections[$sId]['COUNT']?></span></a></li>
						<?endforeach;?>
