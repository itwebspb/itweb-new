...
foreach ($arResult["MESSAGES"] as $res):
	$iCount++;
    $authorName = htmlspecialcharsbx($res["AUTHOR_NAME"]);
	?><table class="reviews-post-table <?=($iCount == 1 ? "reviews-post-first " : "")?><?
		...
		?>" bx-author-id="<?=$res["AUTHOR_ID"]?>" bx-author-name="<?=$authorName;?>" id="message<?=$res["ID"]?>" itemprop="review" itemscope itemtype="http://schema.org/Review">
		<thead><tr><td>
		<?
...
<div>
		...
		if (intVal($res["AUTHOR_ID"]) > 0 && !empty($res["AUTHOR_URL"])):
			?><a href="<?=htmlspecialcharsbx($res['AUTHOR_URL']);?>" class="dark-link"><span itemprop="author"><?=$authorName;?></span></a><?
		else:
			?><span itemprop="author" class="darken"><?=$authorName;?></span><?
		endif;
		?>
