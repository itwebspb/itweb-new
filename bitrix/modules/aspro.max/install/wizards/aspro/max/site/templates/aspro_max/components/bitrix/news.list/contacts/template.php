									...
									<div class="row">
										<div class="col-md-6 col-sm-8 col-xs-12 left-block-contacts">
											...
											<div class="top-wrap">
                                                ...
                                                <?endif;?>
														<?=SolutionUtils::sanitizeHtml($arItem['~NAME']);?><?=SolutionUtils::sanitizeHtml($address);?>
                                                <?if($showLink):?>
													</a>
                                                <?endif;?>
												</div>
												<div class="middle-prop">
													...
													<?if($arItem['PROPERTIES']['METRO']['~VALUE']):?>
														<?foreach($arItem['PROPERTIES']['METRO']['~VALUE'] as $metro):?>
															<div class="metro font_upper"><?=CMax::showIconSvg("metro colored", SITE_TEMPLATE_PATH."/images/svg/Metro.svg");?><span class="text muted777"><?=SolutionUtils::sanitizeHtml($metro);?></span></div>
														<?endforeach;?>
													<?endif;?>
												</div>
												<?if($arItem['PROPERTIES']['SCHEDULE']['~VALUE']['TEXT'] ?? ''):?>
													<div class="schedule"><?=CMax::showIconSvg("clock colored", SITE_TEMPLATE_PATH."/images/svg/WorkingHours.svg");?><span class="text font_xs muted777"><?=SolutionUtils::sanitizeHtml($arItem['PROPERTIES']['SCHEDULE']['~VALUE']['TEXT']);?></span></div>
												<?endif;?>
														...
														<div class="phones col-md-4 col-sm-12 col-xs-12">
															<?foreach($arItem['PROPERTIES']['PHONE']['~VALUE'] as $phone):?>
																<div class="phone font_sm darken">
																	<a href="tel:<?=SolutionUtils::sanitizeValue(str_replace(array(' ', ',', '-', '(', ')'), '', $phone));?>" class="black"><?=SolutionUtils::sanitizeHtml($phone);?></a>
																</div>
															<?endforeach;?>
														</div>
													<?endif?>
													<?if($arItem['PROPERTIES']['EMAIL']['~VALUE']):?>
														<div class="emails col-md-4 col-sm-12 col-xs-12">
															<div class="email font_sm">
																<a class="dark-color" href="mailto:<?=SolutionUtils::sanitizeHtml($arItem['PROPERTIES']['EMAIL']['~VALUE']);?>"><?=SolutionUtils::sanitizeHtml($arItem['PROPERTIES']['EMAIL']['~VALUE']);?></a>
															</div>
														</div>
													<?endif?>
