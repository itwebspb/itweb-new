...
<? if($arResult['ITEMS']): ?>
	<?
        ...
        $showProps = ($arParams['SHOW_PROPS'] ?? "Y") === "Y";
        ...
        $sClassList = match((string) $elementInRow) {
            '4' => 'col-lg-3 col-md-4',
            default => 'col-lg-4',
        };
        $rowClass = ['flexbox items row row-'.$elementInRow];
        if ($bMobileScrolledItems) {
		    $rowClass[] = 'normal swipeignore mobile-overflow mobile-margin-16 mobile-compact has-bottom-nav';
	    }

        $titleBlock = $arParams['TITLE_BLOCK'] ?: null;
        $textAllElements = $arParams['TITLE_BLOCK_ALL'] ?: null;
        $linkAllElements = $arParams['ALL_URL'] ?: null;
	...
	<? if(!$isAjax): ?>
        ...
        <?if ($bMaxWidthWrap):?>
            <div class="content_wrapper_block">
                <div class="maxwidth-theme only-on-front">
                    <div class="tab_slider_wrapp specials">
        <?endif;?>
        ...
        <?if($titleBlock) :?>
            <div class="top_block">
                <?=Aspro\Functions\CAsproMax::showTitleH($titleBlock);?>
            <?if($linkAllElements && $textAllElements) :?>
                <a href="<?=SITE_DIR.$linkAllElements;?>" class="pull-right font_upper muted"><?= $textAllElements ?></a>
            <?endif;?>
            </div>
        <?endif;?>

		<div class="<?=TSolution\Utils::implodeClasses($rowClass);?>">
			<?// show section items?>
	<? endif; ?>
			<?
			...
			<? foreach($arResult['ITEMS'] as $i => $arItem): ?>
				...
				?>
				<div class="item__column <?=$sClassList?> col-sm-6 col-xs-12 item-width-261 js-notice-block <?=$dop_class;?>" data-ref="mixitup-target">
					<div class="item_wrap colored_theme_hover_bg-block box-shadow height-100 <?=($arParams['BORDERED']=='Y' ? 'bordered-block ' : '')?>" >
							...
							<div class="body-info<?=$bShowSectionName? ' with-section': '';?><?=($bDetailLink) ? ' has-link':'';?>">
								<div class="body-info__top">
									...
									<?if(strlen($arItem['FIELDS']['PREVIEW_TEXT'])):?>
										<div class="item__preview-text previewtext font_xs muted777 line-h-165<?= $bMobileScrolledItems ? ' linecamp-3' : '' ?>">
            ...
            <?if($bMobileScrolledItems):?>
                    ...
                    <?endif;?>
                            ...
                            <div class="bottom_nav mobile_slider animate-load-state block-type<?=($bHasNav ? '' : ' hidden-nav');?>" data-parent=".item-views" data-append=".items.row" <?=($isAjax ? "style='display: none; '" : "");?>>
						<?if ($bHasNav):?>
