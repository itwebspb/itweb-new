BX?.Aspro?.Utils?.Ajax?.appendSessid?.();
...
$(document).on("click", '.front_page .logo-row .logo', function (e) {
  e.preventDefault();
          ...
          if (window.matchMedia("(min-width:992px)").matches) {
            ...
            if (isHeaderFixed || !headerCanFix) {
              if (!headerCanFix || scrollTop <= headerDiffHeight + headerTop) {
                ...
                $("#headerfixed .region_wrapper .dropdown").hide().removeClass("opened show active");
                $("#headerfixed .dropdown-select .dropdown-select__list").hide();
                $(".inline-search-block.show").hide().removeClass("show opened");
