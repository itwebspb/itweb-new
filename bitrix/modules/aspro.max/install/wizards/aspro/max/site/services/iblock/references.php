...
unset($_SESSION["MAXIMUM_HBLOCK_AUTHORS_ID"]);

$filter = [
    'LOGIC' => 'OR',
    ['=NAME' => 'AsproMaxContactReference'],
    ['=TABLE_NAME' => 'b_hlbd_sposobyoplaty'],
];

$dbHblock = HL\HighloadBlockTable::getList(array("filter" => $filter));
if(!$dbHblock->Fetch()){
...
$filter = [
    'LOGIC' => 'OR',
    ['=NAME' => 'AsproMaxAuthors'],
    ['=TABLE_NAME' => 'b_hlbd_aspromaxauthors'],
];
$dbHblock = HL\HighloadBlockTable::getList(array("filter" => $filter));
if(!$dbHblock->Fetch()){
	$data = array('NAME' => 'AsproMaxAuthors', 'TABLE_NAME' => 'b_hlbd_aspromaxauthors');
	$result = HL\HighloadBlockTable::add($data);
	$HL_ID = $result->getId();
	$_SESSION["MAXIMUM_HBLOCK_AUTHORS_ID"] = $HL_ID;
	...
	if($HL_ID){
		...
		$arUserFields = array (
			...
			array (
				...
				'FIELD_NAME' => 'UF_FULL_DESCRIPTION',
				'USER_TYPE_ID' => 'string',
				'XML_ID' => 'UF_AUTHOR_FULL_DESCRIPTION',
				'SORT' => '600',
				'MULTIPLE' => 'N',
				'MANDATORY' => 'N',
				'SHOW_FILTER' => 'N',
