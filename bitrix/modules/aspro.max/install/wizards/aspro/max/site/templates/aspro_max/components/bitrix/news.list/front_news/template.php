...
<?if($arResult['ITEMS']):?>
				...
				<?endif;?>
					...
					<?endif;?>
						<div class="inner-text<?=($bShowSection ? ' with-section' : '');?><?=($bActiveDate ? ' with-date' : '');?><?=($arParams['TITLE_SHOW_FON'] == 'Y' ? ' with-fon' : '');?>">
							...
							<?// date active period?>
							<?if($bActiveDate):?>
                                <?$isSmallFont = $arParams['HALF_BLOCK'] == 'Y' && ($indexItem || $arParams['IS_AJAX']);?>
								<div class="period-block<?=($bShowSection && !$bLineImg ? '' : ' muted');?> <?=$isSmallFont ? 'font_xxs' : 'font_xs';?>">
	...
	<?endif;?>
		...
		<div class="bottom_nav_wrapper<?=($bSlider ? ' hidden-slider-nav' : '');?>">
			<div class="bottom_nav animate-load-state<?=($arResult['NAV_STRING'] ? ' has-nav' : '');?>" <?=($arParams['IS_AJAX'] ? "style='display: none; '" : "");?> data-parent=".item-views" data-scroll-class=".swipeignore.mobile-overflow" data-append="<?=($arParams['HALF_BLOCK'] != 'Y' ? '.items > .row' : '.items > .row > .item-wrapper.line_img .half-wrapper');?>">
