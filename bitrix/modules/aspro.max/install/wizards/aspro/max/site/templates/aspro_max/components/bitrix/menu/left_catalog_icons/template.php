...
$iVisibleItemsMenu = $arTheme['MAX_VISIBLE_ITEMS_MENU']['VALUE'] ?: 10;
...
<div class="menu_top_block catalog_block <?=$bShowOverlay ? 'dark_overlay' : '';?>"<?=$templateData['USE_AJAX_SUBMENU'] ? ' data-visible-count="'.(int)$iVisibleItemsMenu.'"' : '';?>>
    <ul class="menu dropdown">
        <?foreach ($arResult as $key => $arItem):?>
            ...
            ?>
            <li class="full <?=$arItem['CHILD'] ? 'has-child' : '';?> <?=$arItem['SELECTED'] ? 'current opened' : '';?> <?=$classMenuPosition;?> v_hover<?=$templateData['USE_AJAX_SUBMENU'] && $arItem['CHILD'] ? ' ajax-submenu-parent' : '';?>"<?=$templateData['USE_AJAX_SUBMENU'] && $arItem['CHILD'] ? ' data-section-id="'.(int)$arItem['ID'].'"' : '';?>>
                <a class="icons_fa <?=$arItem['CHILD'] ? 'parent' : '';?>" title="<?=$arItem['NAME'];?>" href="<?=$arItem['SECTION_PAGE_URL'];?>">
                    <span class="image">
                        <?if ($arItem['IMAGES']):?>
                            <?if (strpos($arItem['IMAGES']['src'], '.svg') !== false && TSolution::GetFrontParametrValue('COLORED_CATALOG_ICON') === 'Y'):?>
                                <?=TSolution\Functions::showSVG(['PATH' => $arItem['IMAGES']['src']]);?>
                            <?else:?>
                                <img class="lazy" data-src="<?=$arItem['IMAGES']['src'];?>" src="<?=TSolution\Functions::showBlankImg($arItem['IMAGES']['src']);?>" alt="<?=$arItem['NAME'];?>" title="<?=$arItem['NAME'];?>" />
                            <?endif;?>
                        <?else:?>
                            <?php $arItem['IMAGES'] = ['src' => SITE_TEMPLATE_PATH.'/images/svg/noimage_product.svg'];?>
                            <img class="lazy no_image" data-src="<?=$arItem['IMAGES']['src'];?>" src="<?=TSolution\Functions::showBlankImg($arItem['IMAGES']['src']);?>" alt="<?=$arItem['NAME'];?>" title="<?=$arItem['NAME'];?>" />
                        <?endif;?>
                    </span>
                </a>
                <?if ($arItem['CHILD']):?>
                    <div class="dropdown-block">
                        <div class="title">
                            <a class="dark-color" href="<?=$arItem['SECTION_PAGE_URL'];?>">
                                <?=$arItem['NAME'];?>
                            </a>
                        </div>
                            ...
                            <div class="dropdown <?=$arItem['UF_MENU_BANNER'] ? 'with_right_block' : '';?>">
                                <ul class="left-menu-wrapper">
                                    <?foreach ($arItem['CHILD'] as $arChildItem):?>
                                        ...
                                        ?>
                                        <li class="<?=!$templateData['USE_AJAX_SUBMENU'] && $arChildItem['CHILD'] ? 'has-childs' : '';?> <?=$arChildItem['SELECTED'] ? 'current' : '';?>"<?=$templateData['USE_AJAX_SUBMENU'] ? ' data-l2-id="'.(int)$arChildItem['ID'].'"' : '';?>>
                                            <?if ($arChildItem['IMAGES']):?>
                                                <span class="image">
                                                    <a href="<?=$arChildItem['SECTION_PAGE_URL'];?>">
                                                        ...
                                                        <?else:?>
                                                            <img class="lazy" data-src="<?=$arChildItem['IMAGES']['src'];?>" src="<?=TSolution\Functions::showBlankImg($arChildItem['IMAGES']['src']);?>" alt="<?=$arChildItem['NAME'];?>" title="<?=$arChildItem['NAME'];?>" />
                                                        <?endif;?>
                                                    </a>
                                                </span>
                                            <?endif;?>
                                            ...
                                            <?if (!$templateData['USE_AJAX_SUBMENU'] && $arChildItem['CHILD']):?>
                                                <?php $iCountChilds = count($arChildItem['CHILD']);?>
                                                <ul class="dropdown toggle_menu">
                                                    <?foreach ($arChildItem['CHILD'] as $key => $arChildItem1):?>
                                                        <li class="menu_item <?=$arChildItem1['SELECTED'] ? 'current' : '';?> <?=$key + 1 > $iVisibleItemsMenu ? 'collapsed' : '';?>">
