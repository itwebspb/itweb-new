...
$bShowAuthor = !empty($arResult['AUTHOR']['ID']);
?>
			...
			<div class="period-block darken ncolor font_xs <?=($arResult['ACTIVE_TO'] ? 'red' : '');?> flexbox flexbox--row flexbox--gap flexbox--gap-40">
                <?if($bShowAuthor):?>
                    <div class="author flexbox flexbox--row flexbox--gap flexbox--gap-12">
                        <?if(!empty($arResult['AUTHOR']['UF_FILE']['src'])):?>
                            <img class="author__image rounded" data-src="" src="<?=$arResult['AUTHOR']['UF_FILE']['src'];?>" alt="<?=$arResult['AUTHOR']['UF_NAME'];?>" title="<?=$arResult['AUTHOR']['UF_NAME']?>" />
                        <?endif;?>
                        <div class="author__title"><a href="<?=$arResult['AUTHOR']['URL']?>"><?=$arResult['AUTHOR']['UF_NAME']?></a></div>
                    </div>
                <?endif; ?>
                <?if($bActiveDate):?>
                    <div class="date_wrap">
						...
						<div class="bottom-soc-props social_props">
							<!-- noindex -->
								<?foreach($arResult['SOCIAL_PROPS'] as $arProp):?>
									<a href="<?=TSolution\Utils::getAbsoluteExternalUrl($arProp['VALUE']);?>" target="_blank" rel="nofollow" class="value <?=strtolower($arProp['CODE']);?>">
...
$formOrderText = trim(ob_get_clean());
?>
