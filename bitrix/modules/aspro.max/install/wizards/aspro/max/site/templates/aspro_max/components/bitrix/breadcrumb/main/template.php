...
if($arResult) {
    ...
    for($index = 0; $index < $cnt; ++$index) {
        ...
        if($arItem['LINK'] != '' && $arItem['LINK'] != GetPagePath() && GetPagePath() != $arItem['LINK'].'index.php' || $arSubSections) {
            ...
            $strReturn .= '<span class="breadcrumbs__item'.($bMobileBreadcrumbs ? ' breadcrumbs__item--mobile' : '').'" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><link href="'.GetPagePath().'" /><span><span itemprop="name" class="breadcrumbs__item-name font_xs">'.$title.'</span><meta itemprop="position" content="'.($index + 1).'"></span></span>';
        }
    }
