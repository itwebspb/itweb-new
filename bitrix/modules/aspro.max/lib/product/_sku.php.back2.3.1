...
class Sku {
    ...
    function __construct(array $arPost)
    {
        ...
        if($this->params["IS_DETAIL"]){
            $featureProps = \Bitrix\Iblock\Model\PropertyFeature::getDetailPageShowPropertyCodes( $this->params['IBLOCK_ID'], array('CODE' => 'Y') );
        } else {
            $featureProps = \Bitrix\Iblock\Model\PropertyFeature::getListPageShowPropertyCodes( $this->params['IBLOCK_ID'], array('CODE' => 'Y') );
        }
        if( $featureProps ) {
    ...
    public function getOfferInfo(){
        ...
        if($obElement = $rsElements->GetNextElement())
        {
            ...
            \CIBlockPriceTools::clearProperties($arItem['DISPLAY_PROPERTIES'], $this->params['OFFER_TREE_PROPS']);
            static::clearDisplayProps($arItem['DISPLAY_PROPERTIES']);
    ...
    public function getOfferInfoDetail(){
        ...
        \CIBlockPriceTools::clearProperties($dopOfferInfo['DISPLAY_PROPERTIES'], $this->params['OFFER_TREE_PROPS']);
        static::clearDisplayProps($dopOfferInfo['DISPLAY_PROPERTIES']);
    ...
    public function getMeasureName(){
        ...
        if (0 > $this->offerInfo['CATALOG_MEASURE'])
            $this->offerInfo['CATALOG_MEASURE'] = 0;
        if (0 < $this->offerInfo['CATALOG_MEASURE'])
        {
            $rsMeasures = \CCatalogMeasure::getList(
                array(),
                array('=ID' => $this->offerInfo['CATALOG_MEASURE']),
                false,
                false,
                array('ID', 'SYMBOL_RUS')
            );
    ...
    public function getCurrencyParams(){
        ...
        if ("Y" == $this->params["CONVERT_CURRENCY"])
        {
    ...
    public function getPropsGroup(){
        ...
        if($this->params["IS_DETAIL"]){
            $bPropsGroup = \Bitrix\Main\Config\Option::get(Solution::moduleID, 'GRUPPER_PROPS', 'NOT', $this->params["SITE_ID"]) === "ASPRO_PROPS_GROUP";
            if($bPropsGroup){
                ob_start();
    ...
    public function getGallery(){
        if($this->params["SHOW_GALLERY"] == "Y")
        {
            ...
            if($arItem['MAIN_ITEM'] && 'Y' == $this->params['ADD_DETAIL_TO_SLIDER'])
            {
                ...
                $mainItemGallery = Solution::getSliderForItemExt($arItem['MAIN_ITEM'], $this->params["ADD_PICT_PROP"], 'Y' == $this->params['ADD_DETAIL_TO_SLIDER']);
                ...
                if($arItem["GALLERY"]){
                    foreach($arItem["GALLERY"] as $i => $arImage){
                        if(isset($arImage["ID"])){
                            ...
                            $arItem["GALLERY"][$i]["SMALL"] = \CFile::ResizeImageGet($arImage["ID"], array("width" => $this->params['GALLERY_WIDTH'], "height" => $this->params['GALLERY_HEIGHT']), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
                            $arItem["GALLERY"][$i]["THUMB"] = \CFile::ResizeImageGet($arImage["ID"], array("width" => 52, "height" => 52), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
                        }
                    }
                }
            } else {
                array_splice($arItem["GALLERY"], $this->params["MAX_GALLERY_ITEMS"]);
                $arItem["GALLERY_HTML"] = SolutionItem::showSectionGallery( array('ITEM' => $arItem, 'RETURN' => true, 'RESIZE' => $this->arResize) );
            }
            $this->offerInfo = $arItem;
        }
    }
    ...
    public function getAdditionalGalleryOffer(){
        $this->offerInfo["ADDITIONAL_GALLERY"] = array();		
    ...
    public function getMainItemInfo(){
        ...
        if($this->maiItemID)
        {
            $arFilterTmp = array("ID" => $this->maiItemID, "IBLOCK_ID" => $this->params['IBLOCK_ID_PARENT']);
            $arSelectTmp = array("ID", "NAME", 'PREVIEW_PICTURE', 'DETAIL_PICTURE', 'PROPERTY_POPUP_VIDEO', 'PROPERTY_CML2_ARTICLE', 'PROPERTY_'.$this->params["ADD_PICT_PROP"]);
            ...
            $arElement = SolutionCache::CIblockElement_GetList(array('CACHE' => array('TAG' => SolutionCache::GetIBlockCacheTag($this->params['IBLOCK_ID_PARENT']), 'MULTI' => 'N')), $arFilterTmp, false, false, $arSelectTmp);
            ...
            if(is_array($arElement) && !empty($arElement)){
                ...
                if(!empty($arElement['PROPERTY_'.$this->params["ADD_PICT_PROP"].'_VALUE'])) {
                    $arMainItem["PROPERTIES"][$this->params["ADD_PICT_PROP"]] = array(
                        "PROPERTY_TYPE" => "F",
    ...
    public function getDiscountTime(){
        if($this->params["SHOW_DISCOUNT_TIME"] == "Y" && $this->params["SHOW_COUNTER_LIST"] != "N")
        {
            ...
            $active_to = '';
            $arDiscounts = \CCatalogDiscount::GetDiscountByProduct($this->offerInfo['ID'], $arUserGroups, "N", array(), $this->params["SITE_ID"]);
            if($arDiscounts)
            {
    ...
    public function formatPriceMatrix(){
        $this->offerInfo['ITEM_PRICES'] = array();
    ...
    public function getButtons(){
        ...
        $arAddToBasketData = Solution::GetAddToBasketArray($this->offerInfo, $this->offerInfo["TOTAL_COUNT"], $this->params["DEFAULT_COUNT"], $this->params["BASKET_URL"], $this->offerInfo['IS_DETAIL'], array(), $basketButtonClass, $this->params);
    ...
    public static function clearDisplayProps(array &$arProps){
        $arProps = array_map(function($arProp){
            $arPropTmp = array_filter($arProp, function($key){
                $arNeedKeys = ['DISPLAY_VALUE', 'NAME', 'HINT'];
        ...
        <?if($arPost["PARAMS"]):?>
            ...
            else
            {
                ...
                if(\Bitrix\Main\Config\Option::get("main", "component_cache_on", "Y") != "N")
                {
                    ...
                    $obCache->EndDataCache(array("arOfferInfo" => $obSKU->offerInfo, "arResize" => $obSKU->arResize));
                }
                /**/
            }
            ...
            if($obSKU->params["IS_DETAIL"]){
                ...
                else
                {
                    ...
                    if(\Bitrix\Main\Config\Option::get("main", "component_cache_on", "Y") != "N")
                    {
                        ...
                        $obCache->EndDataCache(array("arOfferInfoDetail" => $obSKU->offerInfoDetail));
                    }
                }
