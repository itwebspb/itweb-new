...
class Iconset {
	...
	public static function showIcon($id, $bSvgUrlReplace = false, $bInlineSvg = true){
		if(strlen($id)){
			...
			if($bSvg = self::isSvgIcon($path) && $bInlineSvg){
			...
			elseif(file_exists($_SERVER['DOCUMENT_ROOT'].$path)){
				...
				return '<img class="iconset_icon iconset_icon--img" src="'.str_replace($_SERVER['DOCUMENT_ROOT'], '', $path).'" title="'.htmlspecialcharsbx($name).'" '.($style ? 'style="'.$style.'"' : '').' />';
			}
		}
