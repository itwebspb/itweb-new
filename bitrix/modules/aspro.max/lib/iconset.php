...
use Aspro\Max\Property\RegionPhone;
...
class Iconset
{
    public const ICONSET_DIR = 'iconset';
    protected $code;
    protected $config;
    ...
    public function __construct($code)
    {
        if (strlen($code)) {
            ...
            if (!$this->config) {
                throw new \Bitrix\Main\ArgumentException(Loc::getMessage('ICONSET_ERROR_UNKNOWN_CODE', ['#CODE#' => $code]));
            }
        } else {
    ...
    public function getItems()
    {
        ...
        $dbRes = \CFile::GetList(['ID' => 'DESC'], ['MODULE_ID' => self::getModuleId()]);
    ...
    private function getDefaultItems()
    {
        ...
        foreach (
            (array) glob($path.$this->config['glob_pattern'], GLOB_BRACE) as $file
        ) {
            $arIcons[] = $file;
        }
                ...
                foreach ($this->getItems() as $item) {
                    if ($item['id'] == $id) {
                        if ($item['default']) {
                            ...
                            $safeModuleId = $helper->forSql(self::getModuleId());
                            $safeLike = str_replace(['\\', '%', '_'], ['\\\\', '\%', '\_'], $helper->forSql($searchValue));
                            $options = $connection->query("(SELECT MODULE_ID,NAME,SITE_ID,VALUE FROM b_option WHERE MODULE_ID = '".$safeModuleId."' AND VALUE LIKE '".$safeLike."%') UNION (SELECT MODULE_ID,NAME,SITE_ID,VALUE FROM b_option_site WHERE MODULE_ID = '".$safeModuleId."' AND VALUE LIKE '".$safeLike."%')");
                                ...
                                if ($arPropertyHandlers = RegionPhone::OnIBlockPropertyBuildList()) {
                                    if ($arPropertyHandlers['USER_TYPE']) {
                                        ...
                                        $dbRes = \CIBlockProperty::GetList(
                                            [],
                                            ['USER_TYPE' => $arPropertyHandlers['USER_TYPE']]
                                        );
                                        while ($arProperty = $dbRes->Fetch()) {
                                            ...
                                            $arElementFilter[] = ['%PROPERTY_'.$arProperty['ID'] => $searchValue];
                                        }
    ...
    public static function showIcon($id, $bSvgUrlReplace = false, $bInlineSvg = true)
    {
        ...
        if ($bSvg && $bInlineSvg && !$bExternal) {
            ...
            if ($bSvgUrlReplace) {
                $svgContent = str_replace(['svg_mask_', 'svg_paint_'], ['svg_mask_re_', 'svg_paint_re_'], $svgContent);
            }
        ...
        return '<img class="iconset_icon iconset_icon--img" data-src src="'.str_replace($documentRoot, '', $path).'" '.($style ? 'style="'.$style.'"' : '').' />';
    }
}
