    ...
    class CAsproMax{
                    ...
                    <?if($arAddToBasketData["ACTION"] !== "NOTHING"):?>
                        ...
                        <?if($arAddToBasketData["ACTION"] == "ADD" && $arAddToBasketData["CAN_BUY"] && $arParams["SHOW_ONE_CLICK_BUY"]!="N"):?>
                            <div class="wrapp-one-click">
                                <span class="btn btn-transparent-border-color <?=htmlspecialcharsbx($class);?> type_block transition_bg one_click" data-item="<?=htmlspecialcharsbx($arResult["ID"])?>" data-iblockID="<?=htmlspecialcharsbx($arParams["IBLOCK_ID"])?>" data-quantity="<?=htmlspecialcharsbx($arAddToBasketData["MIN_QUANTITY_BUY"]);?>" onclick="oneClickBuy('<?=htmlspecialcharsbx($arResult["ID"])?>', '<?= htmlspecialcharsbx($arParams["IBLOCK_ID"]); ?>', this)">
        ...
        public static function shemaOrganization(){
            ...
            <?if($arRegion):
                ...
                $phone = SolutionUtils::sanitizeValue(implode(', ', array_column($arRegion['PHONES'], 'PHONE')));
                $email = SolutionUtils::sanitizeValue($arRegion['PROPERTY_EMAIL_VALUE']['0'] ?? '');
            else:
        ...
        public static function replacePropsParams(&$arParams, $arReplaceCode = [])
        {
            $arReplaceCodeDefault = [
                ...
                'PRICE_VAT_INCLUDE' => 'PRICE_VAT_INCLUDE',
                'DISPLAY_VAT_INFO' => 'DISPLAY_VAT_INFO',
            ];
            ...
            if(\CMax::GetFrontParametrValue('PRICE_VAT_INCLUDE') != 'FROM_COMPONENT') {
                $arParams[$arReplaceCode["PRICE_VAT_INCLUDE"]] = \CMax::GetFrontParametrValue('PRICE_VAT_INCLUDE');
            }
            $arParams[$arReplaceCode["DISPLAY_VAT_INFO"]] = \CMax::GetFrontParametrValue('DISPLAY_VAT_INFO');
        }
