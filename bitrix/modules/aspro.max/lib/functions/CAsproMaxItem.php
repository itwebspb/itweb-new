...
if(!class_exists("CAsproMaxItem"))
{
    class CAsproMaxItem{
        ...
        public static function getCurrentPrice($price_field, $arPrice, $bFromPrice = true, $bUseSchema = false){
            $val = '';
            if($bFromPrice)
            {
            ...
            if ($bUseSchema) {
                $val .= '<meta itemprop="priceCurrency" content="' . htmlspecialchars($arPrice['CURRENCY']) . '">';
                $val .= '<meta itemprop="price" content="' . htmlspecialchars($bFromPrice ? $arPrice[$price_field] : $price_field) . '">';
            }
        ...
        ?>
                <?ob_start();?>
                    ...
                    ?>
                    <div class="like_icons <?=$class;?>" data-size="<?=$arItem["ICONS_SIZE"];?>">
                    ...
                    <?if($arPrice["CAN_ACCESS"]):?>
                        ...
                        <div class="price_matrix_wrapper <?=($arDiscountPrices && $iCountPriceGroup > 1 ? (isset($arDiscountPrices[$arPriceGroup['ID']]) ? 'strike_block' : '') : '');?>">
                            <?if($arPrice["VALUE"] > $arPrice["DISCOUNT_VALUE"]):?>
                                <div class="prices-wrapper">
                                    <div class="price font-bold <?=($arParams['MD_PRICE'] ? 'font_mlg' : 'font_mxs');?>" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["DISCOUNT_VALUE"];?>">
                                        <?if(strlen($arPrice["PRINT_DISCOUNT_VALUE"])):?>
                                            <?=(($arParams["MESSAGE_FROM"] ?? '') ?: '')?><span class="values_wrapper">
                                                <?=self::getCurrentPrice("DISCOUNT_VALUE", $arPrice, bUseSchema: $arParams['USE_SCHEMA'] && $bMinPrice && !$bUsePopup);?>
                                ...
                                <div class="price font-bold <?=($arParams['MD_PRICE'] ? 'font_mlg' : 'font_mxs');?>" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["VALUE"];?>">
                                    <? if($arPrice['VALUE'] || $sMissingGoodsPriceDisplay === 'PRICE'): ?>
                                        <?if(strlen($arPrice["PRINT_VALUE"])):?>
                                            <?=($arParams["MESSAGE_FROM"] ? $arParams["MESSAGE_FROM"] : '')?><span class="values_wrapper"><?=self::getCurrentPrice("VALUE", $arPrice, bUseSchema: $arParams['USE_SCHEMA'] && $bMinPrice && !$bUsePopup);?></span><?if(($arParams["SHOW_MEASURE"]=="Y") && $strMeasure):?><span class="price_measure">/<?=$strMeasure?></span><?endif;?>
                                        <?endif;?>
                                    <? elseif($sMissingGoodsPriceDisplay === 'TEXT'): ?>
                                        <?if($arParams['USE_SCHEMA'] && !$bUsePopup):?>
                                            <?=Price::showPriceWithSchema();?>
                                        <?endif;?>
                                        <?= $sMissingGoodsPriceDisplayText; ?>
                                    <? elseif($sMissingGoodsPriceDisplay === 'NOTHING' && $arParams['USE_SCHEMA'] && !$bUsePopup):?>
                                        <?=Price::showPriceWithSchema();?>
                                    <? endif; ?>
                                </div>
                            <?endif;?>
        ...
        public static function getIconsCount(): int
        {
            $params = [
                'CATALOG_COMPARE',
                'CATALOG_DELAY',
            ...
            foreach ($params as $param) {
                if (\CMax::GetFrontParametrValue($param) !== 'N') {
                    $icons++;
                }
            }
