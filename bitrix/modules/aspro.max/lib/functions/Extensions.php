...
class Extensions
{
    protected static function getList(string $templatePath = ''): array
    {
        $templateExtensions = static::getTemplateExtensions($templatePath);
        $moduleExtensions = static::getModuleExtensions();

        return array_merge($templateExtensions, $moduleExtensions);
    }

    protected static function getTemplateExtensions(string $templatePath = ''): array
    {
        $path = $templatePath ?: SITE_TEMPLATE_PATH;
        ...
        return [
            ...
            'authors' => [
                'css' => SITE_TEMPLATE_PATH.'/css/authors.min.css',
            ],
            'banners' => [
            ...
            'comment_like' => [
                'js' => SITE_TEMPLATE_PATH.'/js/conditional/comment_like.js',
            ],
            'chip' => [
            ...
            'filter_search' => [
                'css' => SITE_TEMPLATE_PATH.'/css/conditional/filter_search.css',
                'js' => SITE_TEMPLATE_PATH.'/js/conditional/filter_search.js',
            ],
            'font_awesome' => [
            ...
            'section_pagination' => [
                'js' => SITE_TEMPLATE_PATH.'/js/conditional/section_pagination.js',
            ],
    ...
    protected static function getModuleExtensions(): array
    {
        $path = BX_ROOT.'/js/'.Solution::moduleID;

        return [
            'gesture' => [
                'js' => [
                    $path.'/gesture/TinyGesture.js',
                    $path.'/gesture/swipe.js',
                ],
            ],
            'header.menu.left_catalog.aim' => [
                'js' => $path.'/header/menu/left_catalog/aim.js',
                'rel' => ['jquery3'],
            ],
            'header.menu.left_catalog.ajax_submenu' => [
                'js' => $path.'/header/menu/left_catalog/ajax_submenu.js',
            ],
        ];
    }
    ...
    {
        $arConfig = static::getList($templatePath);

        foreach ($arConfig as $ext => $arExt) {
            \CJSCore::RegisterExt(static::getRealCode($ext), array_merge($arExt, ['skip_core' => true]));
        }
    }
    ...
    public static function getRealCode(string $ext): string
    {
