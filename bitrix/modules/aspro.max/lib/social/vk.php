...
{
    public const API_VERSION = 5.199;
    public const API_URL = 'https://api.vk.ru/method/';

    public function __construct(?string $token = '', int $count = 10)
    {
        if (Solution::getFrontParametrValue('VK_API_METHOD') === 'ID') {
            $token = (new OAuth\VKID(SITE_ID))->getAccessToken();
        }
    ...
    {
        return $this->getRequest($this->prepareURLString($arOptions).'&count='.$this->count_post.'&v='.static::API_VERSION);
    }
