...
class SvgSprite
{
    ...
    public static function getSanitizedContent(string $path): string
    {
        $content = IO\File::getFileContents($path);

        $content = preg_replace('#<script\b[^>]*>.*?</script>#is', '', $content);
        $content = preg_replace('#\son\w+\s*=\s*(["\']).*?\1#is', '', $content);
    ...
    {
        return IO\File::isFileExists($path);
    }
    ...
    public function add($filename)
    {
        ...
        $in = new \DOMDocument();
        $content = IO\File::getFileContents($filename);
        if (\PHP_VERSION_ID < 80000 && function_exists('libxml_disable_entity_loader')) {
            libxml_disable_entity_loader(true);
        }
        $in->loadXML($content, LIBXML_NONET);
        ...
        $this->fullFilePath = static::getFullPath(self::$uploadPath.$this->imgName);
