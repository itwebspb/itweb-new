...
class CrossSales {
	...
	public function __set($name, $value){
		switch($name){
			case 'productId':
				...
				break;
			case 'arParams':
				$this->setProduct($this->productId, $value);
				break;
			case 'siteId':
				$this->siteId = $value;
				...
				if($propertyProductsFilterIblockId == $this->productIblockId){
					...
					if($arRulesTmp = Cache::CIBLockElement_GetList(
						...
						if($arRulesIDs){
							...
							$cachePath = '/'.Cache::class.'/iblock/CIBlockElement_GetList/'.$cacheTag.'/';
								...
								foreach($arRulesTmp as $i => &$arRule){
									if(in_array($arRule['ID'], $arRulesIDs)){
										...
										if (!isset($arRule['PROPERTY_SHOW_PLACE_ENUM_ID'])) {
											$arRule['PROPERTY_SHOW_PLACE_ENUM_ID'] = array_keys((array) $arRule['PROPERTY_SHOW_PLACE_VALUE']);
										}
						...
						if($arRulesTmp){
							...
							$cacheTag = Cache::GetIBlockCacheTag($this->iblockId);
							$cachePath = '/'.Cache::class.'/iblock/CIBlockElement_GetList/'.$cacheTag.'/';
							$cacheID = 'CIBlockElement_GetList_'.$cacheTag.md5($this->productId.serialize($this->arProductSelect));
							if ($obCache->InitCache($cacheTime, $cacheID, $cachePath)) {
								$res = $obCache->GetVars();
								$this->arProduct = $res['arProduct'];
							}
							else {
								$arProductSelectFields = $arProductSelectPropertiesIds = $arProductSelectPropertiesCodes = [];

								foreach ($this->arProductSelect as $field) {
									if (str_starts_with($field, 'PROPERTY_')) {
										$prop = preg_replace('/^PROPERTY_/', '', $field);

										if (preg_match('/[^\d]/', $prop)) {
											$arProductSelectPropertiesCodes[] = $prop;
										}
										else {
											$arProductSelectPropertiesIds[] = $prop;
										}
									}
									else {
										$arProductSelectFields[] = $field;
									}
								}
								...
								$this->arProduct = Cache::CIBLockElement_GetList(
									...
									false,
									false,
									$arProductSelectFields ?: $this->arProductSelect
								);

								if (
									$this->arProduct &&
									(
										$arProductSelectPropertiesIds ||
										$arProductSelectPropertiesCodes
									)
								) {
									if ($arProductSelectPropertiesIds) {
										$arTmp = [$this->productId => []];
										\CIBlockElement::GetPropertyValuesArray(
											$arTmp,
											$this->productIblockId,
											['ID' => $this->productId],
											['ID' => $arProductSelectPropertiesIds]
										);

										if (!empty($arTmp[$this->productId])) {
											foreach ($arTmp[$this->productId] as $arProductPropertyValue) {
												$this->arProduct['PROPERTY_'.$arProductPropertyValue['ID'].'_VALUE'] = $arProductPropertyValue['VALUE'];
												if (array_key_exists('PROPERTY_VALUE_ID', $arProductPropertyValue)) {
													$this->arProduct['PROPERTY_'.$arProductPropertyValue['ID'].'_VALUE_ID'] = $arProductPropertyValue['PROPERTY_VALUE_ID'];
												}
												if (array_key_exists('VALUE_ENUM_ID', $arProductPropertyValue)) {
													$this->arProduct['PROPERTY_'.$arProductPropertyValue['ID'].'_ENUM_ID'] = $arProductPropertyValue['VALUE_ENUM_ID'];
												}
											}
										}
									}

									if ($arProductSelectPropertiesCodes) {
										$arTmp = [$this->productId => []];
										\CIBlockElement::GetPropertyValuesArray(
											$arTmp,
											$this->productIblockId,
											['ID' => $this->productId],
											['CODE' => $arProductSelectPropertiesCodes]
										);

										if (!empty($arTmp[$this->productId])) {
											foreach ($arTmp[$this->productId] as $arProductPropertyValue) {
												$this->arProduct['PROPERTY_'.$arProductPropertyValue['CODE'].'_VALUE'] = $arProductPropertyValue['VALUE'];
												if (array_key_exists('PROPERTY_VALUE_ID', $arProductPropertyValue)) {
													$this->arProduct['PROPERTY_'.$arProductPropertyValue['CODE'].'_VALUE_ID'] = $arProductPropertyValue['PROPERTY_VALUE_ID'];
												}
												if (array_key_exists('VALUE_ENUM_ID', $arProductPropertyValue)) {
													$this->arProduct['PROPERTY_'.$arProductPropertyValue['CODE'].'_ENUM_ID'] = $arProductPropertyValue['VALUE_ENUM_ID'];
												}
											}
										}
									}
								...
								$obCache->EndDataCache(['arProduct' => $this->arProduct]);
							}
							...
							foreach($arRulesTmp as $i => &$arRule){
								if($this->_checkParsedCondition($arRule['PROPERTY_PRODUCTS_FILTER_VALUE'])){
									...
									if($arRule['PROPERTY_EXT_PRODUCTS_FILTER_VALUE']){
										...
										foreach((array) $arRule['PROPERTY_SHOW_PLACE_ENUM_ID'] as $showPlaceID){
	...
	public function getItems($showPlace = '', $sort = 'rand', $order = 'ASC'){
		if($arFilter = $this->getItemsFilter($showPlace)){
			$displayElementSlider = ($this->arParams["DISPLAY_LINKED_ELEMENT_SLIDER_CROSSLINK"]
			? $this->arParams["DISPLAY_LINKED_ELEMENT_SLIDER_CROSSLINK"]
			: ($this->arParams['DISPLAY_ELEMENT_SLIDER'] ? $this->arParams['DISPLAY_ELEMENT_SLIDER'] : 10));
				...
				else
				{
					if (($ind = strpos($name, 'CondIBProp')) !== false)
					{
						...
						if(!array_key_exists($propertyId, $arPropertiesCodes)){
							...
							/* The filter for main products does not work in cross-selling by property with a lowercase character code
			...
			foreach($arFilter as $key => $value){
				if(is_array($value)){
					...
					elseif(strpos($value, 'CondCrossIBProp') !== false){
						...
						if($bNotEqual || $bSIn){
							if($iblock){
								if(!isset($arMultipleListPropertiesIDs)){
									// multiple properties
									...
									$dbRes = \CIBlockProperty::GetList(
										array(),
										array('IBLOCK_ID' => $iblock, 'ACTIVE' => 'Y', 'PROPERTY_TYPE' => 'S:directory', 'MULTIPLE' => 'Y')
									);
									while($arProperty = $dbRes->Fetch()){
										$arMultipleListPropertiesIDs[$arProperty['ID']] = true;
									}
						...
						if(!isset($find) || !$find){
							// if the current product does not have any value for this property, then we do not filter by this property
							unset($arFilter[$key]);
