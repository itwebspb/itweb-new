...
trait Admin {
		...
		if(array_key_exists($optionCode, $arTab["OPTIONS"]) || $arOption["TYPE"] == 'note' || $arOption["TYPE"] == 'includefile')
		{
								...
								<?for($itemIndex = 0; $itemIndex <= $itemsCount; ++$itemIndex):?>
									...
									<?if($bNew):?><?ob_start();?><?endif;?>
										<div class="wrapper">
											<div class="inner_wrapper">
												<?foreach($arOptionsKeys as $_optionKey):?>
                                                    <?if($_optionKey !== 'PHONE_MESSENGER_TELEGRAM' && $_optionKey !== 'PHONE_MESSENGER_WHATSAPP'):?>
                                                    ...
                                                    <?endif;?>
												<?endforeach;?>
												<div class="phone_messenger remove" title="<?=Loc::getMessage("REMOVE_ITEM")?>"></div>
												<div class="phone_messenger drag" title="<?=Loc::getMessage("TRANSFORM_ITEM")?>"></div>
											</div>
										</div>
			...
			else
			{
				...
				else
				{
					...
					?>
					<div class="item js_block <?=$optionType;?> <?=((isset($arOption["WITH_HINT"]) && $arOption["WITH_HINT"] == "Y") ? 'with-hint' : '');?> <?=((isset($arOption["BIG_BLOCK"]) && $arOption["BIG_BLOCK"] == "Y") ? 'big-block' : '');?> <?=isset($arOption['CLASS']) ? $arOption['CLASS'] : '';?>" data-class="<?=$optionCode;?>" data-search="<?=$searchClass;?>">
	...
	public static function ShowAdminRow($optionCode, $arOption, $arTab, $arControllerOption, $btable = false){
		...
		$isArrayItem = (strpos($optionCode, '_array_') !== false && strpos($optionCode, 'PHONE_MESSENGER') === false);
		?>
		...
		<?elseif($optionType == "note"):?>
			...
			<?endif;?>
		<?else:?>
            <?if ($optionCode === 'VK_API_CLIENT_ID'):?>
                <?php
                $bShowAdminMessage = false;
                $adminMessage = '';

                $tokenData = Option::get(self::moduleID, '_VK_API_TOKEN_DATA', '{}', $arTab['SITE_ID']);
                if ($tokenData) {
                    $tokenData = Json::decode($tokenData);
                    $bShowAdminMessage = !empty($tokenData) && $tokenData['access_token'];
                    $adminMessage = str_replace('#ID#', $tokenData['client_id'], $arOption['IFRAME']['SUCCESS_MESSAGE']);
                }

                $admMessageClass = 'adm-info-message adm-info-message--conditional';
                if (!$bShowAdminMessage) {
                    $admMessageClass .= ' _adm-hidden';
                }
                ?>
                <div class="<?=$admMessageClass;?>" data-message="<?=$arOption['IFRAME']['SUCCESS_MESSAGE'];?>">
                    <?=$adminMessage;?>
                </div>
            <?endif;?>
			...
			<?endif;?>
				...
				<?elseif($optionType == "text" || $optionType == "password"):?>
                    ...
                    <?if ($optionCode === 'VK_API_CLIENT_ID') {
                        ?>
                        <button type="button" class="adm-btn adm-btn--popup adm-btn--vk-auth" data-site-id="<?=$arTab['SITE_ID'];?>" title="<?=$arOption['IFRAME']['BUTTON_TITLE'];?>">
                            <span class="adm-btn__title"><?=$arOption['IFRAME']['BUTTON_TITLE'];?></span>
                        </button>
                        <?
                    }
                    ?>
