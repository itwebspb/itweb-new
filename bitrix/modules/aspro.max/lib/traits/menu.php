                ...
                foreach ($childs as $i => $item) {
                    if ($item['PARAMS'] && isset($item['PARAMS']['LINK_REGION'])) {
                        if ($item['PARAMS']['LINK_REGION']) {
                            if (!in_array($GLOBALS['arRegion']['ID'], (array) $item['PARAMS']['LINK_REGION'])) {
                                unset($childs[$i]);
                            }
                        } else {
                            unset($childs[$i]);
        ...
        if ($arSections && is_array($arSections)) {
            foreach ($arSections as $arSection) {
                if ($arSection['IBLOCK_SECTION_ID'] == $PSID) {
                    if (!$bMenu) {
                        $arItem = [$arSection['NAME'], $arSection['SECTION_PAGE_URL'], [], ['FROM_IBLOCK' => 1, 'ID' => $arSection['ID'], 'DEPTH_LEVEL' => $arSection['DEPTH_LEVEL'], 'IBLOCK_ID' => $arSection['IBLOCK_ID']]];
