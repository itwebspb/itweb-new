...
class GS
{
    public const OPTION_ENABLED = 'GS_ENABLED';
    public const OPTION_COPY = 'GS_COPY';
    public const OPTION_SK = 'GS_SK';
    public const OPTION_LAST_PACKET = 'GS_LAST_PACKET';
    public const NOTIFY_TAG = 'GS_#MODULE_ID#';
    public const PACKET_DIR = 'gs';
    public const PACKET_CHARSET = 'utf-8';
    public const PACKET_TYPE_REGISTER = 1;
    public const PACKET_TYPE_FILE = 2;
    public const PACKET_TYPE_POST = 3;
    public const SEND_URL = 'https://analytics.aspro.ru/add/';
    public const SEND_TIMEOUT = 3;
    public const SEND_USER_AGENT = 'gs-packet/bx/1';
    ...
    public static function clear()
    {
        ...
        $dbRes = \CFile::GetList([], ['MODULE_ID' => static::getModuleId()]);
    ...
    {
        $arFilesIds = [];
        $dbRes = \CFile::GetList(['ID' => 'DESC'], ['MODULE_ID' => static::getModuleId()]);
        while ($arFile = $dbRes->Fetch()) {
            if (
        ...
        $arFilesSizes = [];
        $arDirs = [$_SERVER['DOCUMENT_ROOT'].'/upload/'.static::getModuleId().'/'.static::PACKET_DIR.'/'];
        $i = 0;
        while ($arDirs && ++$i < 100) {
            $dir = array_pop($arDirs);
            $arDirExclude = [$dir.'.', $dir.'..'];
            $bHasChilds = false;
            foreach (
                (array) glob($dir.'{,.}*', GLOB_NOSORT | GLOB_BRACE) as $file
            ) {
                if (is_dir($file)) {
    ...
    {
        static::sendData(static::mkData(['sites', 'options']));
    }
    ...
    {
        if ($arData = static::mkData(['server', 'registry'])) {
    ...
    public static function showNotify()
    {
        if (static::isEnabled()) {
            $ID = \CAdminNotify::Add([
                'MESSAGE' => Loc::getMessage('GS_ENABLED_NOTIFY_MESSAGE', [
    ...
    {
        $dbRes = \CAdminNotify::GetList([], ['MODULE_ID' => static::getModuleId(), 'TAG' => static::getNotifyTag()]);
    ...
    private static function sendPacket($arPacket)
    {
        ...
        $client->query(HttpClient::HTTP_POST, static::SEND_URL, [
            'packet' => $arPacket,
        ]);
    ...
    public static function mkData($arParts = [])
    {
        $arData = [
            'copy' => static::getCopy(),
        ...
        if (
            ...
            if (
                ...
                if (in_array('sites', $arParts)) {
                    $arData['sites'] = $arOriginalTemplates = [];
                }
                ...
                while ($arSite = $dbRes->Fetch()) {
                    ...
                    foreach ($arHosts as $i => &$host) {
                        $host = trim(str_replace(['https://', 'http://'], '', $host));
                        if (!strlen($host)) {
                            unset($arHosts[$i]);
                        }
                    }
            ...
            if (
                ...
                foreach (
                    ...
                    foreach (
                        (array) glob($_SERVER['DOCUMENT_ROOT'].$dir.'*', GLOB_ONLYDIR) as $moduleDir
                    ) {
                        ...
                        while ($arOption = $options->fetch()) {
                            if (strlen($arOption['NAME'])) {
                                ...
                                if (!isset($arData['options'][$arOption['MODULE_ID']])) {
                                    $arData['options'][$arOption['MODULE_ID']] = [];
                                }
                                ...
                                if (strpos($arOption['NAME'], 'HEADER_PHONES_array_') === 0) {
                                    if (!isset($arData['options'][$arOption['MODULE_ID']]['HEADER_PHONES_VALUE'])) {
                                        ...
                                        $arData['options'][$arOption['MODULE_ID']]['HEADER_PHONES_DESCRIPTION'] = [];
                                    }
                                ...
                                if (is_array($arOption['VALUE'])) {
                                    foreach ($arOption['VALUE'] as $subCode => $subValue) {
                                        ...
                                        if (!isset($arData['options'][$arOption['MODULE_ID']][$subName])) {
                                            $arData['options'][$arOption['MODULE_ID']][$subName] = [];
                                        }
                                    ...
                                    if (!isset($arData['options'][$arOption['MODULE_ID']][$arOption['NAME']])) {
                                        $arData['options'][$arOption['MODULE_ID']][$arOption['NAME']] = [];
                                    }
                            ...
                            while ($arOption = $options->fetch()) {
                                if (strlen($arOption['NAME'])) {
                                    if (!isset($arData['options'][$moduleId][$arOption['NAME']])) {
                                        $arData['options'][$moduleId][$arOption['NAME']] = [];
                                    }
