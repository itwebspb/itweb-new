...
class PropertyUrlHandler extends AbstractUrlHandler
{
    ...
    protected $isReplaceSymbolString = false;
      ...
      )
    {
        ...
        $this->conditionTreeResult = new Condition\ConditionResult($this->iblockId, $this->condition, [], $this->siteId);
    }
    ...
    public function setSettings(array $settings)
    {
        ...
        if($settings['REPLACE_SPECIAL_CHAR_STRING']) {
            $this->isReplaceSymbolString = $settings['REPLACE_SPECIAL_CHAR_STRING'];
        }
    ...
    {
        $replaceParams = [
            'replace_space' => $this->isReplaceSymbolString ? $this->paramReplaceSpace : '-',
            'replace_other' => $this->isReplaceSymbolString ? $this->paramReplaceOther : '-',
        ];
        ...
        foreach ($values as $value) {
            $value = trim($value);
            $newValues[] = \Cutil::translit($value, 'ru', $replaceParams);
            $originValues[] = $value;
        }
    ...
    protected function getAllPropertyFields(): array
    {
        $all = array_merge($this->getPropertyFields(), $this->getSkuPropertyFields());

        usort($all, function ($a, $b) {
            $sortA = $a['PROPERTY_SORT'] ?? 0;
            $sortB = $b['PROPERTY_SORT'] ?? 0;
