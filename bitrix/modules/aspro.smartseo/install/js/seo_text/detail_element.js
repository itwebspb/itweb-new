...
engineFormProperty.syncEditorFlags = function () {
  const form = document.getElementById('form_seo_text_properties');
  if (!form) return;

  form.querySelectorAll('input[type="hidden"][data-editor-name]').forEach(input => {
      const checkbox = document.getElementById(input.dataset.editorName);

      if(checkbox) input.value = checkbox?.checked ? 'Y' : 'N';
  });
};
...
BX.ready(function () {
    ...
    (function () {
        ...
        form.onBeforeSend = function () {
            engineFormProperty.syncEditorFlags();
