...
class FilterRuleDetailController extends Controller
{
    ...
    private function getListIblocks($siteId, $iblockTypeId)
    {
        ...
        if ($setting->isOnlyCatalog() && $this->isCatalogModule) {
            ...
            $catalogIds = array_intersect(array_column($rows, 'IBLOCK_ID'), $iblockIds);
        }
        ...
        $arIblocks = array_column($arIblocks, NULL, 'ID');
        $arIblocks = array_map(function($iblockInfo) use ($catalogIds) {
            $iblockInfo['IS_CATALOG'] = in_array($iblockInfo['ID'], $catalogIds);
            return $iblockInfo;
        }, $arIblocks);

        \Bitrix\Main\Type\Collection::sortByColumn($arIblocks, array('IS_CATALOG' => SORT_DESC, 'SORT' => array(SORT_NUMERIC, SORT_ASC)));
