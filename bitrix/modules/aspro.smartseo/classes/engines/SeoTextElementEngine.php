...
class SeoTextElementEngine extends \Aspro\Smartseo\Engines\Engine
{
    ...
    protected function getElementUpdateFields($element)
    {
        ...
        if ($this->isRewrite) {
            ...
            foreach ($elementFields['FIELDS'] as $fieldCode => $fieldValue) {
                if (trim($element[$fieldCode] ?? '') || !strlen($fieldValue)) {
                    continue;
                }
        ...
        if (isset($result['PREVIEW_TEXT'])) {
            $result['PREVIEW_TEXT_TYPE'] = $this->getPropertyType('PREVIEW_TEXT');
        }
        if (isset($result['DETAIL_TEXT'])) {
            $result['DETAIL_TEXT_TYPE'] = $this->getPropertyType('DETAIL_TEXT');
        }

        return $result;
    }

    private function getPropertyType(string $fieldCode): string
    {
        return $this->isHtmlEditor($fieldCode) ? self::TYPE_HTML : self::TYPE_TEXT;
    }

    private function isHtmlEditor(string $fieldCode): bool
    {
        return isset($this->dataUpdate[$fieldCode . '_editor'])
            && $this->dataUpdate[$fieldCode . '_editor'] === 'Y';
    }
    ...
    protected function getElementUpdateProperties($element)
    {
        ...
        if ($this->isRewrite) {
            ...
            foreach ($elementFields['PROPERTIES'] as $fieldCode => $fieldValue) {
                $fieldValue = is_array($fieldValue) ? ($fieldValue['VALUE']['TEXT'] ?? '') : $fieldValue;

                if (
                    trim($element['PROPERTY_' . $fieldCode . '_VALUE']['TEXT'] ?? '') || !strlen($fieldValue)
                ) {
                    continue;
                }
        ...
        foreach ($elementFields['PROPERTIES'] as $fieldCode => $fieldValue) {
            if(is_array($fieldValue)) {
                $fieldValue['VALUE']['TYPE'] = $this->getPropertyType($fieldCode);
    ...
    private function fillElementValueByTemplate($entityTemplate, array &$data)
    {
        foreach ($data as &$value) {
            if (is_array($value)) {
                $value['VALUE']['TEXT'] = str_replace('section', 'sections', empty($value['VALUE']['TEXT']) ? '' : $value['VALUE']['TEXT']);
