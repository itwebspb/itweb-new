...
use	Aspro\Popup\Cache as Cache,
	\Bitrix\Main\Web\Json;

class MarketingPopup{
	const ALL_USERS_GROUP_ID = 2;
	const IBLOCK_TYPE = 'aspro_popup_adv';
	const IBLOCK_CODE = 'aspro_popup_marketings';
	...
	public function setInfo($arParams = array()){
		...
		if($bAdminSection && $this->siteId == 'ru') //admin page
			$this->siteId = 's1';
			...
			if($this->iblockId){
				...
				if($arRulesTmp = Cache::CIBLockElement_GetList(
					...
					if($arRulesIDs){
						...
						$cachePath = '/AsproPopupCache/iblock/CIBlockElement_GetList/'.$cacheTag.'/';
		...
		if($arParsedCondition && is_array($arParsedCondition))
		{
			foreach($arParsedCondition as $key => $value)
			{
				...
				else
				{
					...
					global $APPLICATION;
					if($value['NAME'] === 'PAGE'){
						$find = $APPLICATION->GetCurPage();
					}
					elseif($value['NAME'] === 'SERVER_NAME'){
						$find = $_SERVER['SERVER_NAME'];
					}elseif($value['NAME'] === 'GET_PARAM'){
						$context = \Bitrix\Main\Application::getInstance()->getContext();
						$request = $context->getRequest();
						$find = $request->getServer()->get('QUERY_STRING') ?? '';
					}
