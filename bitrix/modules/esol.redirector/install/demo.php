...
function esol_redirector_demo_expired() {
	$DemoMode = CModule::IncludeModuleEx("esol.redirector");
	if ($DemoMode==MODULE_DEMO) {
		$now=time();
		if (defined("esol_redirector_OLDSITEEXPIREDATE")) {
			if ($now>=esol_redirector_OLDSITEEXPIREDATE || esol_redirector_OLDSITEEXPIREDATE>$now+3000000) {
				return true;
			}
		} else{ 
			return true;
...
function esol_redirector_show_demo($bAjax = false) {
	$moduleId = 'esol.redirector';
	$DemoMode = CModule::IncludeModuleEx($moduleId);
	$activateText = GetMessage("ESOL_REDIRECTOR_DEMO_MESSAGE_ACTIVATE_MODULE",array("#LANG#"=>LANGUAGE_ID));
	if ($DemoMode==MODULE_DEMO) {
		$now=time();
		if (defined("esol_redirector_OLDSITEEXPIREDATE")) {
			if ($now<esol_redirector_OLDSITEEXPIREDATE) {
				print BeginNote();
				$expire_arr = getdate(esol_redirector_OLDSITEEXPIREDATE);
				...
				print GetMessage("ESOL_REDIRECTOR_DEMO_MESSAGE_DAYS_REMAIN", array("#DAYS#"=>$days, "#ACTIVATE#"=>$activateText));
				print EndNote();
			} else {
				print BeginNote();
				print GetMessage("ESOL_REDIRECTOR_DEMO_MESSAGE_EXPIRED", array("#ACTIVATE#"=>$activateText));
				print EndNote();
			}
		} else{ 
			print BeginNote();
	...
	else
	{
		if(!$bAjax)
		{
			print BeginNote('id="esol-rr-updates-message"');
			print '<div id="esol-rr-updates-message-inner"></div>';
			print EndNote();
		}
		else
		{
			...
			if($updateEnd && is_callable(array('CUpdateClientPartner', 'GetLicenseKey')))
			{
				...
				echo '<div id="esol-rr-updates-message-inner">'.GetMessage("ESOL_REDIRECTOR_DEMO_MESSAGE_UPDATE_END", array("#LCKEY#"=>$lckey, "#LANG#"=>LANGUAGE_ID)).'</div>';
			}
		}
	}
